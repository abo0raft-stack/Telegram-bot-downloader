...
async def start_handler(event):
    await event.respond(f"🚀 **بوت التحميل النفاث ({VERSION})**\nأرسل رابط صفحة الويب أو الفيديو للبدء.")
...
async def url_handler(event):
    ...
    if len(urls) == 1:
        url = urls[0]
        checking_msg = await event.respond("🌐 **جاري تحليل الرابط بآلية عالية السرعة...**")
        ...
        await event.respond(f"🌐 **تم تحليل الصفحة!**\n📁 **الاسم:** `{default_name}`", buttons=buttons)
...
async def start_execution(chat_id, url, filename_title, as_doc=False):
    status_msg = await bot.send_message(chat_id, "🔍 **جاري التهيئة بروابط فائقة السرعة...**")
    msg_id = status_msg.id
    ...
    try:
        ...
        await status_msg.edit("🚀 **جاري التنزيل بأقصى سرعة (Turbo 16x)...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{msg_id}")])
        ...
        def progress_hook_ytdlp(d):
            ...
            if d['status'] == 'downloading':
                ...
                text = (
                    f"🚀 **جاري التنزيل المتوازي السريع...**\n"
                    f"📊 النسبة: `{p}`\n"
        ...
        # خيارات معززة بالكامل للتنزيل بأقصى سرعة
        ydl_opts = {
            ...
            # --- خيارات التسريع الفائق ---
            'concurrent_fragment_downloads': 16, # زيادة الاتصالات المتوازية إلى 16
            'hls_use_mpegts': True,
            'http_chunk_size': 10485760,         # طلب كتل بحجم 10MB لتخطي خنق السيرفرات
            'external_downloader': 'aria2c',     # استخدام aria2c كـ محرك خارجي إن وجد
        ...
        if not os.path.exists(file_path):
            if os.path.exists(f"{file_path}.mp4"): file_path = f"{file_path}.mp4"
            elif os.path.exists(f"{file_path}.mkv"): file_path = f"{file_path}.mkv"
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
            ...
            raise Exception("فشل التنزيل أو لم يتم العثور على مقطع صالح داخل الصفحة.")
