...
VERSION = "v99.6-BRQZ-QualityBtnFix"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **إصلاح أزرار منصات التواصل (v99.6):** حل مشكلة الضغط على أزرار اختيار الجودة والتحميل لمنصات التواصل الاجتماعي مع الحفاظ على جميع الخصائص.\n"
            ...
            "⚡ **اختصارات الأوامر (v99.2):** دعم تنفيذ `/start` بـ `/` و `/settings` بـ `//`.\n\n"
...
@bot.on(events.CallbackQuery(pattern=r"^(q_|prof_)"))
...
async def handle_user_input_and_urls(event):
    ...
    for u in urls:
        ...
        if is_profile_url(clean_u):
            ...
            await event.respond("🌐 **تم التعرف على رابط منشور منصات تواصل. اختر خيار التحميل:**", buttons=buttons)
...
async def main():
    await init_db()
    print(f"🚀 تم تشغيل البوت بنجاح ({VERSION})...")
