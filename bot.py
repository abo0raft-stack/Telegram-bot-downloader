...
VERSION = "v96.3-Metadata-FastStart-Fix"
...
async def convert_to_mp4(input_path):
    ...
    # محاولة سريعة بالترميز المباشر والتسريع الموانع للمشاكل
    cmd = [
        'ffmpeg', '-y', '-i', input_path,
    ...
    # في حال كان الملف بالأصل mp4 ولم يتحول بالترميز الكامل نطبق fix_video_metadata
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    """تنزيل وتحميل ديليموشن مع دعم التقسيم لملفات > 2GB وتسكين الفهرس (moov atom)"""
    chat_id = event.chat_id
    ...
    try:
        ...
        for idx, part_file in enumerate(split_files, start=1):
            ...
            # التأكد التام من تسكين الفهرس وتصحيح الميتا داتا قبل الإرسال
        ...
        try:
            ...
            for vid_idx, vid in enumerate(videos, start=1):
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    # تسكين الميتا داتا والفهرس moov atom في بداية الملف لمنع التعامل معه كـ Stream
