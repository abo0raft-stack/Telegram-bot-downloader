...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        ...
        if is_m3u8 or not is_direct_mp4:
            await status_msg.edit("⚡ **جاري السحب الفائق وتجميع بث M3U8...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
            ...
            def progress_hook_ytdlp(d):
                ...
                if d['status'] == 'downloading':
                    ...
                    text = (
                        f"📡 **جاري تحميل M3U8 بالتوازي (16 قناة)...**\n"
                        f"📊 النسبة: `{p}`\n"
            ...
            ydl_opts = {
                ...
                'concurrent_fragment_downloads': 16, # تحميل 16 جزء متوازي لتجنب انتهاء التوكن الزمنية
                'hls_use_mpegts': True,
        ...
        if os.path.exists(file_path):
            ...
            if file_size < 50 * 1024: 
                raise Exception("الملف المحمل صغير جداً أو انتهت صلاحية الرابط أثناء السحب.")
        ...
        if "CANCELLED_BY_USER" in err_msg:
            ...
            # تحسين إخراج وصف الخطأ
            detailed_err = getattr(e, 'msg', err_msg)
