...
# --- الإعدادات ---
VERSION = "v68.0-FFmpeg-ImageFix-Complete"
...
def sanitize_and_fix_image(file_path):
    """إعادة تشفير قسرية باستخدام ffmpeg تجرد الصور من Metadata وتجعلها 100% قياسية لتليجرام"""
    try:
        base_name, _ = os.path.splitext(file_path)
        new_path = f"{base_name}_fixed.jpg"
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        # --- معالجة وإعادة بناء كافة الصور عبر FFmpeg ---
        downloaded_files = []
        for root, _, files in os.walk(task_dir):
            for file in files:
                fpath = os.path.join(root, file)
                if os.path.isfile(fpath) and os.path.getsize(fpath) > 500 and not file.endswith('_thumb.jpg') and not file.endswith('_fixed.jpg'):
                    ...
                    # تنظيف وتعديل أبعاد الصورة لضمان التوافق المطلق مع Telethon
        ...
        # 1. إرسال الصور (مع آلية احتياطية تفادياً لخطأ الـ Album)
        if photos:
            try:
                if len(photos) == 1:
                ...
                print(f"Album upload error, falling back to individual send: {e}")
