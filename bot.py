...
def update_and_install_libraries():
    try:
        import subprocess
        subprocess.run(["pip", "install", "-U", "yt-dlp", "gallery-dl", "Pillow", "aiohttp", "aiosqlite", "telethon", "requests", "psutil", "beautifulsoup4", "googletrans==4.0.0-rc1", "curl_cffi"], check=True)
...
# --- ترويسات متطورة لتجاوز حظر 403 Forbidden ---
BROWSER_HEADERS = {
...
def generate_ffmpeg_thumbnail(file_path, task_id):
    thumb_path = os.path.join(os.path.dirname(file_path), f"thumb_{task_id}.jpg")
    try:
        cmd = ['ffmpeg', '-y', '-ss', '00:00:01', '-i', file_path, '-vframes', '1', '-q:v', '2', thumb_path]
        import subprocess
...
def download_direct_url(self, url: str, task_id: str, loop: asyncio.AbstractEventLoop) -> dict:
    """دالة مخصصة لتحميل الفيديوهات المباشرة وتفادي خطأي HTTP 403 Forbidden و 429"""
    out_dir = "downloads"
    ...
    # محاولات ذكية للتغلب على 403 Forbidden و 429
    max_attempts = 5
    for attempt in range(max_attempts):
        try:
            ...
            with opener.open(req, timeout=30) as response, open(file_path, 'wb') as out_file:
            ...
            if e.code in [403, 429] and attempt < max_attempts - 1:
                ...
                logger.warning(f"⚠️ HTTP {e.code} detected during direct download. Retrying in {sleep_time:.1f}s with updated headers...")
...
# --- محرك التنزيل لـ yt-dlp المحدث والمحسّن مع معالجة تجاوز 403 Forbidden ---
...
async def tiktok_multi_engine(url, task_dir):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            headers = get_random_headers(referer="https://www.tikwm.com/")
...
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    ...
    # التنزيل المباشر باستخدام الدالة المخصصة
    loop = asyncio.get_event_loop()
    ...
    async def update_progress_task():
        ...
        try:
            while True:
                item = await q.get()
                msg, downloaded, total_size, start_time, unit = item
                now = time.time()
                if now - last_update >= 1.8 or downloaded == total_size:
                    last_update = now
                    elapsed = now - start_time
    ...
    p_task = asyncio.create_task(update_progress_task())
    try:
        res = await loop.run_in_executor(None, download_direct_url, None, url, task_id, loop)
        if res.get("file_path") and os.path.exists(res["file_path"]):
            if filepath != res["file_path"]:
                shutil.move(res["file_path"], filepath)
    finally:
        p_task.cancel()
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🛡️ **تجاوز حظر HTTP 403 Forbidden (v99.5):** إضافة ترويسات صفائح المتصفح التكيفية (Sec-Ch-Ua & Sec-Fetch) وتفعيل ميزة انتحال المتصفح (Chrome 120 Impersonate) لحل الحظر المباشر نهائياً.\n"
            "⚡ **التحميل المباشر التلقائي (v99.4):** تحديث دالة `download_direct_url` للتعامل مع أخطاء HTTP 429 وتفعيل التنزيل الفوري للروابط المباشرة بدون أزرار خيارات التنسيق والمستندات.\n"
            "🚀 **حل مشكلة الحظر (v99.3):** إضافة نظام التخطي الذكي وإعادة المحاولة التلقائية (Exponential Backoff) لحل مشكلة '429 Too Many Requests' أثناء التنزيل المباشر من سيرفرات مثل Shahid وغيرها.\n"
            "⚡ **تحديث أختصارات الأوامر (v99.2):** دعم تنفيذ أمر التشغيل `/start` باستخدام `/` فقط، وتنفيذ أمر الإعدادات `/settings` باستخدام `//` فقط.\n"
            "🔧 **إصلاح خيارات التنزيل (v99.1):** إعادة هيكلة خيارات `ydl_opts` لمنع أخطاء التوافق مع ffmpeg ودعم الدمج التلقائي لصيغة MP3 و MP4 بحسب الجودة الحالية.\n"
            "🎵 **تحديث شامل لمحرك TikTok (v99):** بناء محرك هجين متعدد المصادر لضمان تنزيل الفيديوهات والصور (Photo Mode) مع استخراج النص والترجمة للغة العربية.\n"
            "🎬 **إصلاح نوع إرسال الفيديو (v98):** ضمان ضبط أبعاد وخواص الفيديو بدقة لمنع التليجرام من تحويل المقاطع إلى صور.\n"
            "✨ **تنظيف شرح فيديوهات التواصل (v96):** إلغاء كتابة اسم الملف ورأس الجملة وإظهار نص المنشور المجلوب والترجمة العربية مباشرة أسفل الفيديو لجميع المنصات.\n"
...
async def profile_callback_handler(event):
    ...
    if not url:
        await event.answer("⚠️ انتهت صلاحية هذه الجلسة، أرسل الرابط مجدداً.", alert=True)
        return

    chat_id = event.chat_id
    await event.delete()
    
    status_msg = await bot.send_message(chat_id, "👤 **جاري جلب الصورة الشخصية بأعلى جودة...**")
...
async def quality_callback_handler(event):
    ...
    if task_type == "dailymotion":
        asyncio.create_task(download_dailymotion_video(event, url, quality_choice, None))
    else:
        status_msg = await bot.send_message(chat_id, f"⏳ **جاري تحضير الفيديو بالجودة المحددة ({quality_choice}p)...**")
...
async def main():
    await init_db()
    print(f"🚀 تم تشغيل البوت بنجاح - الإصدار الخاص: {VERSION}")
