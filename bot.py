...
# --- الإعدادات ---
VERSION = "v59.0-Instagram-Multi-Engine-Fix"
...
def download_instagram_gallery_dl(url, task_dir):
    """المحرك 2: gallery-dl المتخصص في معارض الصور"""
    try:
        cmd = ["gallery-dl", "--directory", task_dir, "--filename", "{id}_{num}.{ext}"]
...
def instagram_api_cobalt(url, task_dir):
    """المحرك 3: API Cobalt الخارجي الحاسم لتجاوز حظر IP"""
    try:
...
def instagram_web_scraper(url, task_dir):
    """المحرك 4: Scraper مباشر لهيكل JSON الخاص بإنستغرام"""
    try:
        ...
        if res.status_code == 200:
            ...
            carousel = media.get('edge_sidecar_to_children', {}).get('edges', []) or media.get('carousel_media', [])
            
            if carousel:
                for node in carousel:
                    n = node.get('node', node)
                    if n.get('is_video') or 'video_versions' in n:
                        v_url = n.get('video_url') or (n.get('video_versions', [{}])[0].get('url'))
                        if v_url: items.append((v_url, 'mp4'))
                    else:
                        img_url = n.get('display_url') or (n.get('image_versions2', {}).get('candidates', [{}])[0].get('url'))
                        if img_url: items.append((img_url, 'jpg'))
            else:
                if media.get('is_video') or 'video_versions' in media:
                    v_url = media.get('video_url') or (media.get('video_versions', [{}])[0].get('url'))
                    if v_url: items.append((v_url, 'mp4'))
                else:
                    img_url = media.get('display_url') or (media.get('image_versions2', {}).get('candidates', [{}])[0].get('url'))
                    if img_url: items.append((img_url, 'jpg'))

            for idx, (m_url, ext) in enumerate(items, start=1):
                r = session.get(m_url, timeout=15)
                if r.status_code == 200:
                    fpath = os.path.join(task_dir, f"scrape_media_{idx}.{ext}")
    ...
    try:
        ...
        if target_url and is_complex_url(target_url):
            ...
            if 'instagram.com' in target_url or 'instagr.am' in target_url:
                ...
                # المحرك 2: Gallery-dl
                if not downloaded:
                    await loop.run_in_executor(None, download_instagram_gallery_dl, target_url, task_dir)
                    downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                    
                # المحرك 3: Cobalt API
                if not downloaded:
                    await loop.run_in_executor(None, instagram_api_cobalt, target_url, task_dir)
                    downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                    
                # المحرك 4: Scraper Direct
                if not downloaded:
                    await loop.run_in_executor(None, instagram_web_scraper, target_url, task_dir)
        ...
        # جمع الملفات المُنَزَّلة من المجلد الرئيسي أو أي مجلد فرعي
        downloaded_files = []
        for root, _, files in os.walk(task_dir):
            for f in files:
                fpath = os.path.join(root, f)
                if os.path.isfile(fpath) and os.path.getsize(fpath) > 0 and not f.endswith('_thumb.jpg'):
...
def main():
    ...
    print("🤖 البوت يعمل بكفاءة عالية وتم حل مشكلة صور إنستغرام والدعم المتعدد!")
