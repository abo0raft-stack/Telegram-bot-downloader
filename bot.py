...
def update_and_install_libraries():
    try:
        import subprocess
        subprocess.run(["pip", "install", "-U", "yt-dlp", "gallery-dl", "Pillow", "aiohttp", "aiosqlite", "telethon", "requests", "psutil", "instaloader"], check=True)
...
# --- دالة التنزيل والرفع المخصصة لـ Dailymotion ---
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    try:
        ...
        post_caption = ""
        def run_dl():
            nonlocal post_caption
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(url, download=True)
                title = info.get('title', '')
                description = info.get('description', '')
                post_caption = f"🎥 **{title}**\n\n{description}".strip() if description else f"🎥 **{title}**"
        ...
        final_cap = post_caption[:1000] if post_caption else "🎥 **تم التنزيل بنجاح من Dailymotion!**"
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if info:
                ...
                info_json_path = os.path.join(task_dir, "post_caption.txt")
...
# --- محرك TikTok المحدث كلياً v85.1 لحل مشكلة الصورة الأولى كمستند ---
                    ...
                    for idx, media_url in enumerate(urls_to_dl, start=1):
                        ...
                        async with session.get(media_url, headers=BROWSER_HEADERS, timeout=30) as r:
                            if r.status == 200:
                                ...
                                if is_vid:
                                    filepath = os.path.join(task_dir, f"tt_video_{idx}.mp4")
                                else:
                                    # إعطاء أرقام مرتبة بوضوح لمنع تداخل الصوت والغلاف مع ألبوم الصور
                                    filepath = os.path.join(task_dir, f"tt_photo_{img_counter:02d}.jpg")
                                    img_counter += 1
...
async def instagram_carousel_and_photo_engine(url, task_dir):
    try:
        ...
        print(f"IG Gallery-dl fallback error: {e}")
    ...
    try:
        ...
        print(f"Instagram engine Cobalt error: {e}")

    try:
        embed_url = url.rstrip('/') + '/embed/captioned/'
        async with aiohttp.ClientSession() as session:
            async with session.get(embed_url, headers=BROWSER_HEADERS, timeout=15) as resp:
                if resp.status == 200:
                    html_content = await resp.text()
                    image_urls = re.findall(r'class="EmbeddedMediaImage"\s+src="([^"]+)"', html_content)
                    if not image_urls:
                        image_urls = re.findall(r'https://scontent[^\s"]+\.jpg[^\s"]*', html_content)
                    
                    for idx, img_u in enumerate(set(image_urls), start=1):
                        clean_img_url = img_u.replace('&amp;', '&')
                        async with session.get(clean_img_url, headers=BROWSER_HEADERS, timeout=20) as img_resp:
                            if img_resp.status == 200:
                                filepath = os.path.join(task_dir, f"ig_direct_photo_{idx}.jpg")
                                with open(filepath, 'wb') as f:
                                    f.write(await img_resp.read())
    except Exception as e:
        print(f"Direct IG embed scraper failed: {e}")

# --- دالة إصلاح الصور وترميمها v85.1 لحل مشكلة الصورة الأولى المعطوبة كمستند ---
async def deep_sanitize_image(file_path):
    try:
        ...
        # 1. القراءة بـ PIL وإعادات التنسيق النظيف المباشر
        with Image.open(file_path) as img:
            ...
            rgb_img.save(cleaned_path, 'JPEG', quality=98)

        # 2. إزالة أية بيانات حزم خربانة بالـ FFmpeg
        ...
        print(f"Image deep sanitize error on {file_path}: {e}")
        return file_path

# --- استئناف وتنفيذ التنزيل المباشر الذكي مع جلب الاسم الحقيقي ---
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    ...
    if real_filename and real_filename != os.path.basename(filepath):
        try:
            new_path = os.path.join(os.path.dirname(filepath), real_filename)
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if target_url and is_social:
                ...
                filepath = await download_direct_async(bot, chat_id, target_url, filepath, status_msg, cancel_event, task_id)
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    file_disp_name = os.path.basename(vid_file)
                    part_caption = f"📁 **اسم الملف:** `{file_disp_name}`"
                    if len(video_files) > 1:
                        part_caption += f" (Part {idx}/{len(video_files)})"
            ...
            downloaded_files = []
            post_caption_text = ""
            caption_file = os.path.join(task_dir, "post_caption.txt")
            if os.path.exists(caption_file):
                try:
                    with open(caption_file, "r", encoding="utf-8") as f:
                        post_caption_text = f.read().strip()
                except Exception: pass

            for root, _, files in os.walk(task_dir):
                for file in files:
                    fpath = os.path.join(root, file)
                    if os.path.isfile(fpath) and os.path.getsize(fpath) > 500 and not file.endswith('_thumb.jpg') and not file.endswith('_clean.jpg') and not file.endswith('_final.jpg') and not file.startswith('frame_') and file != "post_caption.txt":
                ...
                if len(photos) > 1 and not as_doc:
                    try:
                        for chunk_idx in range(0, len(photos), 10):
                            ...
                            album_caption = post_caption_text[:1000] if (post_caption_text and chunk_idx == 0) else f"📸 **ألبوم الصور ({chunk_idx+1}-{chunk_idx+len(chunk_photos)}/{len(photos)}):**"
                            await bot.send_file(
                                chat_id, 
                                chunk_photos, 
                    ...
                    for idx, p in enumerate(photos, start=1):
                        try:
                            # توحيد التسمية والترميم النهائي
                            p = await deep_sanitize_image(p)
                            base_p = os.path.splitext(p)[0] + ".jpg"
                            if p != base_p and os.path.exists(p):
                                os.rename(p, base_p)
                                p = base_p
                            
                            if as_doc:
                                doc_caption = f"📄 **مستند صورة ({idx}/{len(photos)}):** `{os.path.basename(p)}`"
                                await bot.send_file(chat_id, p, caption=doc_caption, force_document=True)
                            else:
                                cap = post_caption_text[:1000] if post_caption_text else (f"📸 **صورة ({idx}/{len(photos)}):**" if len(photos) > 1 else "📸 **تم تنزيل الصورة بنجاح!**")
                                await bot.send_file(chat_id, p, caption=cap, force_document=False)
                        except Exception as p_err:
                            print(f"Failed to send image {p}: {p_err}")
            ...
            for vid in videos:
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    ...
                    vid_caption = post_caption_text[:1000] if post_caption_text else f"🎥 **تم تنزيل الفيديو بنجاح!**{part_caption}\n📁 `{os.path.basename(part_vid)}`"
...
async def clean_handler(event):
    ...
    await event.respond(f"🧹 **تم تنظيف وحذف جميع ملفات مجلد التحميلات بنجاح!**\n💾 المساحة المحررة: `{format_size(freed)}`")
...
async def disk_status_handler(event):
    ...
    dl_size, dl_count = get_downloads_folder_size()
    
    msg = (
        "📊 **حالة السيرفر وقرص التحميلات الحالي:**\n\n"
        f"📂 **استهلاك مجلد التحميلات (downloads):**\n"
        f"• حجم الملفات الحالية: `{format_size(dl_size)}`\n"
        f"• عدد الملفات: `{dl_count}` ملف\n\n"
        f"💾 **مساحة قرص التخزين الإجمالية:**\n"
        f"• الإجمالي: `{format_size(disk.total)}`\n"
        f"• المستهلك: `{format_size(disk.used)}` ({disk.percent}%)\n"
    ...
    buttons = [[Button.inline("🧹 تصفير قرص التحميلات الآن", data="clear_downloads_now")]]
...
async def build_settings_buttons(chat_id):
    ...
    buttons = [
        ...
        [Button.inline(f"💾 حجم القرص: {format_size(dl_size)} ({dl_count} ملفات)", data="refresh_disk_info")],
        [Button.inline("🧹 تصفير قرص التحميلات وحذف الملفات", data="clear_downloads_now")],
...
async def send_settings_menu(event):
    ...
    msg = (
        "⚙️ **لوحة التحكم والإعدادات الخاصّة بالبوت:**\n\n"
        "قم بالضغط على الأزرار أدناه لتعديل خيارات التحميل أو تصفير قرص التحميلات.\n\n"
        f"📊 **معلومات الاستهلاك الحالية:**\n"
        f"• مستهلك التحميلات: `{format_size(dl_size)}` (`{dl_count}` ملفات)\n"
        f"• المساحة المتبقية بالقرص: `{format_size(disk.free)}`"
    )
...
@bot.on(events.CallbackQuery(pattern=r"^(toggle_snaps|toggle_social_snaps|change_qual|change_font|refresh_disk_info|clear_downloads_now|close_settings)$"))
async def settings_callback_handler(event):
    ...
    elif data == "refresh_disk_info":
        await event.answer("🔄 تم تحديث معلومات استهلاك القرص!")

    elif data == "clear_downloads_now":
        freed = clean_download_folder()
        await event.answer(f"🧹 تم تصفير القرص بنجاح وحذف {format_size(freed)}!", alert=True)
...
async def start_handler(event):
    ...
    welcome_text = (
        ...
        "• `/df` - فحص مساحة القرص ومجلد التحميلات والـ RAM.\n"
        "• `/clean` - مسح وحذف كافة الملفات المؤقتة.\n"
        "• `/restart` - إعادة تشغيل البوت.\n"
        "• `/trim [بدء] [نهاية] [رابط]` - قص المقطع.\n\n"
        "⚙️ لتعديل الإعدادات والتحكم بالقرص أرسل: `/settings`"
    )
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "📸 **إصلاح صور تيكتوك (v85.1):** حل جذري لعطل الصورة الأولى عند التحميل كمستندات وتنظيف الهيدر للـ JPG.\n"
            "📸 **إصلاح صور الانستغرام:** حل كامل لمشكلة عدم تنزيل الصور الفردية والألبومات.\n"
            "🎛️ **التحكم بقرص التحميلات:** إضافة معلومات الحجم واستهلاك الملفات وحالة القرص تفصيلياً.\n"
            "🧹 **تصفير وحذف الملفات:** إمكانية حذف وتصفير مجلد التحميلات بضغطة زر من الإعدادات والأوامر.\n"
            "📝 **وصول وصف المنشور:** استخراج وإرفاق نص المنشور المكتوب تلقائياً مع الميديا.\n"
            "🏷️ **اسم الملف للروابط المباشرة:** استخراج وعرض اسم الملف الحقيقي بدقة.\n"
            "📺 **إصلاح Dailymotion الشامل:** معالجة مسار التوجيه وإسناد الـ Referer المطلوبة.\n\n"
