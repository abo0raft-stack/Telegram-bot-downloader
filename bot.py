...
async def callback_handler(event):
    ...
    if data == "ask_rename":
        ...
        if user_id in USER_STATES:
            ...
            # البدء المباشر بعد اختيار الصيغة
            if chat_id not in DOWNLOAD_QUEUES:
                DOWNLOAD_QUEUES[chat_id] = []
    ...
    elif data == "convert_doc" or data == "type_doc":
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, fmt='mp4'):
    ...
    try:
        if media_msg:
            await status_msg.edit("📥 **جاري تنزيل الملف من تليجرام...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
        ...
        # المعالجة والتحويل إن لزم الأمر
        ...
        if fmt == 'mp3':
            await bot.send_file(chat_id, final_file, voice_note=False)
        else:
            await bot.send_file(chat_id, final_file, force_document=as_doc)
