...
VERSION = "v99.8-TikTokEngineUpdate"
...
def force_release_memory():
    """تفريغ الذاكرة وإرجاعها لنظام التشغيل قسراً"""
    gc.collect()
    try:
...
def get_dir_size(path):
    """حساب الحجم الحقيقي للمجلد بالبايت"""
    total = 0
...
# --- محرك رفع الملفات آمن ضد خطأ أجزاء الرفع Failed to upload file part ---
async def upload_file_smart(client, filepath, progress_callback=None):
    """رفع أجزاء الملف بضمان التخطي والتمرير التلقائي لتجنب أخطاء upload file part"""
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if "upload file part" in str(e).lower() or attempt < max_retries - 1:
...
async def get_video_metadata_and_thumb(file_path):
    ...
    try:
        ...
        if os.path.exists(thumb_path):
            if os.path.getsize(thumb_path) < 200:
...
async def split_video_file(filepath, max_size_mb=1850):
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    try:
        ...
        safe_thumb = thumb_path if (thumb_path and os.path.exists(thumb_path) and os.path.getsize(thumb_path) > 200) else None

        file_to_send = await upload_file_smart(bot, file_path, progress_callback=upload_progress_callback)
        await bot.send_file(
            chat_id,
            file_to_send,
            caption=caption_out,
...
# --- محرك التنزيل لـ yt-dlp المحدث والمحسّن ---
...
# --- محرك TikTok الشامل الهجين والمطور لجميع أنواع الوسائط ---
async def inspect_tiktok_media(url):
    """التعرف المسبق والتلقائي على وسائط TikTok لفتح اللوحة المناسبة (صور أو فيديو)"""
    try:
        async with aiohttp.ClientSession() as session:
            headers = {'User-Agent': BROWSER_HEADERS['User-Agent']}
            async with session.post("https://www.tikwm.com/api/", data={"url": url}, headers=headers, timeout=12) as res:
                if res.status == 200:
                    res_json = await res.json()
                    data = res_json.get("data", {})
                    if data.get("images"):
                        return "image"
                    if data.get("play") or data.get("hdplay"):
                        return "video"
    except Exception: pass
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(api_url, json={"url": url}, headers=headers, timeout=12) as res:
                if res.status == 200:
                    data = await res.json()
...
async def tiktok_multi_engine(url, task_dir):
    """محرك متكامل وهجين لتنزيل صور وفيديوهات تيكتوك مع الوصف وبدائل متعددة لضمان الاستخراج بدون أخطاء"""
    
    # 1. التنزيل عبر TikWM API (تنسيق الصور والفيديو HD)
    try:
        async with aiohttp.ClientSession() as session:
            headers = {'User-Agent': BROWSER_HEADERS['User-Agent']}
            async with session.post("https://www.tikwm.com/api/", data={"url": url, "hd": 1}, headers=headers, timeout=25) as res:
                if res.status == 200:
                    res_json = await res.json()
                    data = res_json.get("data", {})
                    ...
                    if images:
                        for idx, img_url in enumerate(images, start=1):
                            async with session.get(img_url, headers=headers, timeout=20) as r:
                                if r.status == 200:
                                    filepath = os.path.join(task_dir, f"tt_photo_{idx}.jpg")
                                    with open(filepath, 'wb') as f:
                                        f.write(await r.read())
                        if any(os.path.isfile(os.path.join(task_dir, f)) and not f.endswith('.txt') for f in os.listdir(task_dir)):
                            return
                    ...
                    if play_url:
                        ...
                        async with session.get(play_url, headers=headers, timeout=35) as r:
                            if r.status == 200:
                                filepath = os.path.join(task_dir, "tt_video.mp4")
                                with open(filepath, 'wb') as f:
                                    f.write(await r.read())
                                return
    except Exception as e:
        print(f"TikTok TikWM API error: {e}")

    # 2. التجربة باستخدام gallery-dl
    await gallery_dl_fallback_engine(url, task_dir, "tt")
    if any(os.path.isfile(os.path.join(task_dir, f)) and not f.endswith('.txt') for f in os.listdir(task_dir)):
        return

    # 3. التجربة عبر Cobalt API كخيار هجين إضافي
    try:
        api_url = "https://api.cobalt.tools/api/json"
        headers = {
            "Accept": "application/json",
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(api_url, json={"url": url}, headers=headers, timeout=20) as res:
                if res.status == 200:
                    data = await res.json()
                    ...
                    for idx, media_url in enumerate(urls_to_dl, start=1):
                        ...
                        async with session.get(media_url, headers=BROWSER_HEADERS, timeout=30) as r:
                            if r.status == 200:
                                ...
                                filepath = os.path.join(task_dir, f"tt_media_{idx}.{ext}")
                                with open(filepath, 'wb') as f:
                                    f.write(await r.read())
    except Exception as e:
        print(f"TikTok Cobalt API error: {e}")

    # 4. التنزيل الاحتياطي النهائي باستخدام yt-dlp
    if not any(os.path.isfile(os.path.join(task_dir, f)) and not f.endswith('.txt') for f in os.listdir(task_dir)):
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(None, download_with_ytdlp, url, task_dir, 'mp4', 'best')
        ...
        try:
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.get(url, ssl=False, allow_redirects=True) as response:
                    if response.status == 429:
                        retry_after = int(response.headers.get("Retry-After", 2 ** attempt + 3))
                        try:
                            await status_msg.edit(f"⚠️ **السيرفر مزدحم (429).**\nإعادة المحاولة ({attempt+1}/{MAX_RETRIES}) بعد `{retry_after}` ثانية...", buttons=cancel_btn)
                        except: pass
                        ...
                        if response.status in [403, 406] and 'Range' in headers:
                            ...
                            async with session.get(url, ssl=False, allow_redirects=True) as retry_res:
                                if retry_res.status == 429:
                                    retry_after = int(retry_res.headers.get("Retry-After", 2 ** attempt + 3))
                                    try:
                                        await status_msg.edit(f"⚠️ **السيرفر مزدحم (429).**\nإعادة المحاولة...", buttons=cancel_btn)
                                    except: pass
                                    await asyncio.sleep(retry_after)
                                    continue
            ...
            if "429" in str(e) or (hasattr(e, 'status') and e.status == 429):
                if attempt == MAX_RETRIES - 1:
                    raise Exception("فشل التنزيل بسبب حظر كثرة الطلبات (429 Too Many Requests) بعد عدة محاولات.")
                retry_after = 2 ** attempt + 3
                try: await status_msg.edit(f"⚠️ **تم تجاوز حد الطلبات (429).**\nإعادة المحاولة ({attempt+1}/{MAX_RETRIES}) بعد `{retry_after}` ثانية...", buttons=cancel_btn)
                except: pass
                await asyncio.sleep(retry_after)
            else:
                if attempt == MAX_RETRIES - 1:
        ...
        try:
            ...
            if is_avatar_task:
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    ...
                    file_to_send = await upload_file_smart(bot, vid_file, progress_callback=upload_progress_callback)
                    
                    if as_doc:
                        await bot.send_file(chat_id, file_to_send, force_document=True, caption=caption_text)
                    else:
                        ...
                        if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                            ...
                            safe_thumb = thumb_path if (thumb_path and os.path.exists(thumb_path) and os.path.getsize(thumb_path) > 200) else None
                            await bot.send_file(chat_id, file_to_send, caption=caption_text, thumb=safe_thumb, attributes=attr)
                            ...
                            await bot.send_file(chat_id, file_to_send, caption=caption_text)
                        else:
                            await bot.send_file(chat_id, file_to_send, caption=caption_text, force_document=True)
            ...
            if os.path.exists(desc_file_path):
                try:
                    with open(desc_file_path, "r", encoding="utf-8") as df:
                        ...
                        if post_desc_text:
                            translated_desc_text = await translate_to_arabic(post_desc_text)
                except Exception: pass
            ...
            if post_desc_text:
                ...
                if translated_desc_text:
                    ...
                    if len(clean_trans) > 800:
                        clean_trans = clean_trans[:800] + "..."
                ...
                if len(photos) > 1 and not as_doc:
                    try:
                        ...
                        for idx, p in enumerate(photos, start=1):
                            ...
                            file_p = await upload_file_smart(bot, p)
                            await bot.send_file(chat_id, file_p, caption=cap)
                else:
                    for idx, p in enumerate(photos, start=1):
                        try:
                            ...
                            if as_doc:
                                ...
                                if idx == 1 and post_caption_formatted: cap += f"\n\n{post_caption_formatted}"
                                await bot.send_file(chat_id, file_p, caption=cap, force_document=True)
                            else:
                                ...
                                if idx == 1 and post_caption_formatted: cap += f"\n\n{post_caption_formatted}"
                                await bot.send_file(chat_id, file_p, caption=cap, force_document=False)
                        except Exception:
                            file_p = await upload_file_smart(bot, p)
                            await bot.send_file(chat_id, file_p, force_document=True)
            ...
            for vid_idx, vid in enumerate(videos, start=1):
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    ...
                    file_v = await upload_file_smart(bot, part_vid)
                    safe_thumb = thumb_path if (thumb_path and os.path.exists(thumb_path) and os.path.getsize(thumb_path) > 200) else None
            ...
            for aud in audio_files:
                ...
                file_a = await upload_file_smart(bot, aud)
                await bot.send_file(chat_id, file_a, caption=aud_cap)

            for oth in other_files:
                if cancel_event.is_set(): raise Exception("CANCELLED")
                oname = os.path.basename(oth)
                file_o = await upload_file_smart(bot, oth)
                await bot.send_file(chat_id, file_o, caption=f"📄 `{oname}`", force_document=True)
...
async def start_handler(event):
    ...
    welcome_text = (
        ...
        "⚙️ لتعديل الإعدادات أو إدارة الأعضاء والسرعة أرسل: `//` أو `/settings`"
    )
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **تحديث محرك تيكتوك والوسائط (v99.8):** دعم تفكيك وسائط تيكتوك تلقائياً وعرض لوحات تحكم منفصلة للصور والفيديو مع الحفاظ على التنزيل عالي الجودة والاستخراج بدون أخطاء.\n"
            "✨ **حل مشكلة SendMediaRequest (v99.7):** معالجة الخطأ المسبب لعدم جلب مقاطع منصات التواصل وحل أخطاء إرسال المعاينات التالفة.\n"
            "✨ **إصلاح أزرار منصات التواصل (v99.6):** حل مشكلة الضغط على أزرار اختيار الجودة والتحميل لمنصات التواصل الاجتماعي.\n"
            "🛠️ **حل مشكلة Upload File Part (v99.5):** بناء دالة الرفع المباشر `upload_file_smart` لمنع فشل الرفع عند الأجزاء العالية.\n\n"
...
# --- معالجة الضغط على أزرار اختيار الجودة واللوحات التفاعلية ---
@bot.on(events.CallbackQuery(pattern=r"^(q_|prof_|fb_|img_|doc_)"))
async def task_callback_handler(event):
    if not await is_allowed(event):
        await event.answer("⚠️ ليس لديك صلاحية استخدام هذا البوت.", alert=True)
        return
    ...
    if data.startswith("prof_"):
        parts = data.split("_")
        mode = parts[1] # img or doc
        task_key = "_".join(parts[2:])
        url, _ = await pop_task(task_key)
        if not url:
            await event.answer("⚠️ انتهت صلاحية هذه المهمة أو تم تنفيذها بالفعل.", alert=True)
            return

        await event.delete()
        status_msg = await bot.send_message(chat_id, "⏳ **جاري التنزيل والرفع...**")
        asyncio.create_task(
            start_direct_execution(
                chat_id=chat_id,
                url=url,
    ...
    elif data.startswith("img_") or data.startswith("doc_"):
        parts = data.split("_")
        mode = parts[0] # img or doc
        ...
        await event.delete()
        status_msg = await bot.send_message(chat_id, "⏳ **جاري جلب وتحضير ألبوم الصور...**")
        asyncio.create_task(
            start_direct_execution(
                chat_id=chat_id,
                url=url,
    ...
    elif data.startswith("q_") or data.startswith("fb_"):
        parts = data.split("_")
        quality_choice = parts[1] # 1080, 720, 480, mp3
        task_key = "_".join(parts[2:])
        url, task_type = await pop_task(task_key)
        ...
        if task_type == "dailymotion":
            status_msg = await bot.send_message(chat_id, "⏳ **جاري بدء عملية التحميل...**")
            asyncio.create_task(download_dailymotion_video(event, url, quality_choice, status_msg))
        else:
            status_msg = await bot.send_message(chat_id, "⏳ **جاري بدء عملية التحميل...**")
...
async def handle_user_input_and_urls(event):
    ...
    for u in urls:
        ...
        u = clean_url(u)
        task_key = f"tk_{int(time.time() * 1000)}"

        if is_profile_url(u):
            await save_task(task_key, u, "profile")
            buttons = [
                [Button.inline("🖼️ الصورة الشخصية (معاينة)", data=f"prof_img_{task_key}")],
                [Button.inline("📁 الصورة الشخصية (مستند)", data=f"prof_doc_{task_key}")]
            ]
            await event.respond("👤 **تم التعرف على رابط ملف شخصي! اختر طريقة جلب الصورة:**", buttons=buttons)

        elif is_tiktok_url(u):
            # التعرف التلقائي على وسائط TikTok لفتح اللوحة المخصصة
            media_type = await inspect_tiktok_media(u)
            
            if media_type == "image":
                await save_task(task_key, u, "tiktok_photo")
                buttons = [
                    [
                        Button.inline("🖼️ صور كألبوم", data=f"img_{task_key}"),
                        Button.inline("📁 صور كمستند", data=f"doc_{task_key}")
                    ]
                ]
                await event.respond("📷 **تم التعرف على ألبوم صور TikTok! اختر طريقة العرض:**", buttons=buttons)
            else:
                await save_task(task_key, u, "tiktok_video")
                buttons = [
                    [
                        Button.inline("🎬 1080p", data=f"q_1080_{task_key}"),
                        Button.inline("🎬 720p", data=f"q_720_{task_key}")
                    ],
                    [Button.inline("🎵 صوت MP3 (خامة 320k)", data=f"q_mp3_{task_key}")]
                ]
                await event.respond("🎬 **تم التعرف على فيديو TikTok! اختر الجودة أو التنسيق المطلوب:**", buttons=buttons)

        elif is_dailymotion_url(u):
            await save_task(task_key, u, "dailymotion")
            buttons = [
                [
                    Button.inline("🎬 1080p", data=f"q_1080_{task_key}"),
                    Button.inline("🎬 720p", data=f"q_720_{task_key}"),
            ...
            await event.respond("📹 **تم التعرف على فيديو Dailymotion! اختر الجودة أو التنسيق المطلوب:**", buttons=buttons)

        elif is_complex_url(u):
            await save_task(task_key, u, "social")
            buttons = [
                [
                    Button.inline("🎬 1080p", data=f"q_1080_{task_key}"),
                    Button.inline("🎬 720p", data=f"q_720_{task_key}"),
            ...
            await event.respond("🌐 **تم التعرف على رابط من منصة تواصل! اختر الجودة أو التنسيق:**", buttons=buttons)

        else:
            status_msg = await event.respond("⏳ **جاري تحضير التنزيل المباشر...**")
            user_config = await get_user_config(chat_id)
            asyncio.create_task(
                start_direct_execution(
                    chat_id=chat_id,
...
async def main():
    ...
    print(f"✅ البوت يعمل بنجاح في الإصدار {VERSION}!")
