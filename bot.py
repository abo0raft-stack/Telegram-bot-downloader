...
VERSION = "v99.4-DirectDownload-Fix"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🧠 **تحسين تفريغ الذاكرة والحجم الحقيقي (v94):** تفعيل محرك `malloc_trim` قسراً عبر مكتبة `libc` لتفريغ الذاكرة المحجوزة لنظام التشغيل.\n"
            "🌐 **ترجمة المنشورات إلى العربية (v92):** ترجمة نص المنشور الأجنبي تلقائياً وإلحاقه مع الوسائط المحملة.\n\n"
...
# --- إعداد قائمة الأوامر التفاعلية وإطلاق البوت ---
async def set_bot_commands():
    try:
        commands = [
            BotCommand(command="start", description="بدء استخدام البوت وعرض الأوامر"),
            BotCommand(command="settings", description="فتح لوحة الإعدادات والتحكم"),
            BotCommand(command="df", description="فحص استهلاك مساحة التخزين والـ RAM"),
            BotCommand(command="clean", description="تنظيف الملفات المؤقتة وتفريغ الذاكرة"),
            BotCommand(command="restart", description="إعادة تشغيل البوت (للمالك)")
        ]
...
print(f"✅ Bot is running... Version: {VERSION}")
