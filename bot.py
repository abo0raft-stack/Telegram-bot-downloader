...
# استيراد curl_cffi لتجاوز الحظر
...
def get_clean_filename(url):
    ...
    if not filename or not any(filename.lower().endswith(ext) for ext in ['.mp4', '.mkv', '.avi', '.mov', '.mp3', '.flv', '.zip', '.rar']):
        filename = "downloaded_file.mp4"
    return filename

def extract_mediafire_direct_link(url):
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
        }
        resp = async_requests.get(url, headers=headers, impersonate="chrome120", timeout=15)
        if resp.status_code == 200:
            match = re.search(r'href="(https?://download\d+\.mediafire\.com/[^"]+)"', resp.text)
            if match:
                return match.group(1)
            match_alt = re.search(r'aria-label="Download file"\s+href="([^"]+)"', resp.text)
            if match_alt:
                return match_alt.group(1)
    except Exception as e:
        print(f"MediaFire Direct Link Extract Error: {e}")
    return url
...
def smart_download_engine(url, filepath, status_msg, loop, cancel_event):
    if "mediafire.com" in url:
        asyncio.run_coroutine_threadsafe(
            status_msg.edit("🔍 **جاري فك واستخراج رابط MediaFire المباشر...**"), loop
        )
        url = extract_mediafire_direct_link(url)

    parsed_u = urlparse(url)
    netloc = parsed_u.netloc
    dir_name, file_name = os.path.split(filepath)

    user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
    referer = f"{parsed_u.scheme}://{netloc}/"

    # 1. المحاولة الأولى باستخدام aria2c (الأقوى للروابط المباشرة و CDN)
    if shutil.which("aria2c"):
        asyncio.run_coroutine_threadsafe(
            status_msg.edit("⚡ **جاري التنزيل المباشر عبر محرك Aria2...**"), loop
        )
        aria_cmd = [
            'aria2c',
            '-x', '16', '-s', '16', '-k', '1M',
        ...
        try:
            proc = subprocess.Popen(aria_cmd)
    ...
    # 2. المحاولة الثانية: تنزيل مجزأ عبر curl_cffi
    asyncio.run_coroutine_threadsafe(
        status_msg.edit("📥 **جاري التنزيل عبر المحرك الاحتياطي (Stream)...**"), loop
    )
    headers = {
    ...
    try:
        ...
        if "CANCELLED_BY_USER" in str(e): raise e
        print(f"cffi direct download failed: {e}")

    raise Exception("فشل تنزيل الملف من سيرفر الـ CDN. قد يكون الرابط منتهى الصلاحية أو يتطلب تسجيل دخول.")
...
async def start_handler(event):
    welcome_text = (
        ...
        "✨ **تم تحديث النظام ليدعم التحميل المباشر مع محرك Aria2 وتجاوز أخطاء الـ CDN.**"
    )
...
async def text_handler(event):
    ...
    if user_id in USER_STATES and USER_STATES[user_id].get('state') == 'waiting_for_name':
        ...
        if not any(new_name.lower().endswith(ext) for ext in ['.mp4', '.mkv', '.avi', '.mov', '.mp3', '.zip', '.rar']):
            new_name += ".mp4"
...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        await status_msg.edit("📥 **جاري تحليل الرابط والتحميل...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
        
        await loop.run_in_executor(None, smart_download_engine, url, file_path, status_msg, loop, cancel_event)
        ...
        if os.path.exists(file_path):
            ...
            is_video = any(file_path.lower().endswith(ext) for ext in ['.mp4', '.mkv', '.avi', '.mov', '.flv'])

            if is_video and quality in ['1080', '720', '480']:
            ...
            if as_doc or not is_video or quality == 'audio':
                await bot.send_file(
