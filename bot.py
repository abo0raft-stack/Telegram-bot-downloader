...
async def start_execution(chat_id, url, filename_title, as_doc=False):
    ...
    file_path = f"downloads/{msg_id}_{filename_title}"
    ...
    try:
        ...
        await status_msg.edit("⚡ **جاري التنزيل السريع (Turbospeed)...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{msg_id}")])
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
            ...
            raise Exception("فشل تنزيل الملف، الملف غير موجود أو بحجم 0.")
