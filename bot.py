...
# --- التحديث التلقائي للمكتبات لمنع الأعطال ---
...
# --- الإعدادات ---
VERSION = "v62.0-TikTok-Photos-Support"
...
def media_picker_engine(url, task_dir):
    """محرك مخصص لاستخراج ألبومات صور تيك توك وإنستغرام عبر Cobalt API و Gallery-DL"""
    try:
        # 1. محاولة gallery-dl أولاً (إنستغرام)
        ...
        # 2. استخدام Cobalt API (المثال الأحسن لصور تيك توك وإنستغرام)
        ...
        res = requests.post(api_url, json={"url": url}, headers=headers, timeout=20)
        ...
        if res.status_code == 200:
            ...
            for idx, media_url in enumerate(urls_to_dl, start=1):
                ...
                if r.status_code == 200:
                    ...
                    is_vid = "video" in content_type or ".mp4" in media_url
                    ext = "mp4" if is_vid else "jpg"
                    
                    filepath = os.path.join(task_dir, f"media_photo_{idx}.{ext}")
                    with open(filepath, 'wb') as f:
                        for chunk in r.iter_content(chunk_size=1024*1024):
                            if chunk: f.write(chunk)
            return True
    except Exception as e:
        print(f"Media Picker Engine error: {e}")
    return False
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        target_url = clean_url(url) if (is_complex_url(url) and not is_tiktok_url(url)) else url
        
        if target_url and is_complex_url(target_url):
            # 1. التنزيل العادي باستخدام yt-dlp
            await loop.run_in_executor(None, download_with_ytdlp, target_url, task_dir, target_fmt, quality)
            
            # 2. إذا فشل التنزيل أو كان المنشور عبارة عن ألبوم صور (سواء في تيك توك أو إنستغرام)
            downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 500]
            if not downloaded:
                await loop.run_in_executor(None, media_picker_engine, target_url, task_dir)
        ...
        # --- معالجة وتصحيح امتدادات الملفات المكسورة (None) ---
        downloaded_files = []
...
async def start_handler(event):
    await event.respond(
        ...
        "📸 **إنستغرام:** دعم كامل للصور، الألبومات (Carousel)، والفيديوهات.\n"
        "🎵 **تيك توك:** دعم الفيديوهات وألبومات الصور (Photo Mode).\n"
...
def main():
    ...
    print("🤖 البوت يعمل بكفاءة وتم إضافة دعم صور تيك توك بنجاح!")
