...
# استيراد yt_dlp كحل احتياطي قوي جداً للروابط المحجوبة
import yt_dlp
...
async def upload_progress_callback(current, total, status_msg, cancel_event):
    ...
    text = (
        f"outbound 📤 **جاري الرفع إلى تليجرام...**\n"
        f"📊 النسبة: `{percent:.1f}%`\n"
...
def smart_download_engine(url, filepath, status_msg, loop, cancel_event):
    """ محرك تنزيل ذكي هجين يعمل بـ curl_cffi ويتراجع تلقائياً لـ yt-dlp """
    parsed_u = urlparse(url)
    netloc = parsed_u.netloc
    
    # 1. التجربة الأولى: باستخدام curl_cffi مع محاكاة المتصفح الهيدرات الكاملة
    headers = {
    ...
    try:
        ...
        print(f"curl_cffi failed, switching to yt-dlp: {e}")

    # 2. المحاولة الثانية: استخدام yt-dlp للتعامل مع الروابط المحجوبة والمعقدة
    if os.path.exists(filepath):
        try: os.remove(filepath)
        except: pass

    asyncio.run_coroutine_threadsafe(
        status_msg.edit("⚡ **جاري التجاوز عبر محرك yt-dlp الاحتياطي...**"), loop
    )
...
async def start_handler(event):
    welcome_text = (
        ...
        "✨ **تم تحديث النظام بمحرك تنزيل هجين مزدوج لتجاوز كافة حمايات الـ CDN.**"
    )
