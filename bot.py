...
# --- سيرفر إبقاء الخدمة نشطة في Render ---
...
# --- محرك التحميل المباشر مع عداد السرعة ---
...
# --- كلاس إدارة عداد الرفع إلى تليجرام ---
class UploadProgress:
    ...
    async def callback(self, current, total):
        ...
        if now - self.last_update > 3 or current == total:
            ...
            text += f"📊 النسبة: `{percentage:.1f}%` (`{format_bytes(current)}` / `{format_bytes(total)}`)\n"
            text += f"🚀 سرعة الرفع: `{format_bytes(speed)}/s`\n"
...
async def start_handler(event):
    await event.respond(f"🚀 **بوت التحميل والرفع الشامل {VERSION} جاهز!**\nأرسل رابط الملف ليتم معالجته مباشرة مع العداد التفاعلي.")
...
async def handle_url(event):
    ...
    try:
        ...
        if os.path.exists(filename) and os.path.getsize(filename) > 0:
            ...
            await safe_edit(status_msg, f"📤 **جاري بدء عملية الرفع إلى تليجرام...**\n📦 الحجم: `{format_bytes(file_size)}`")
            ...
            # 2. إنشاء كائن العداد للرفع
            uploader = UploadProgress(filename, status_msg, loop)

            # 3. الرفع مع الـ Progress Callback
            await bot.send_file(
                event.chat_id,
                filename,
                ...
                progress_callback=uploader.callback, # ربط العداد هنا
                reply_to=event.id
            )
        ...
        await safe_edit(status_msg, f"❌ حدث خطأ:\n`{str(e)}`")
    finally:
