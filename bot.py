...
# --- الإعدادات ---
VERSION = "v85.1-UniversalSmartVideo-Fix403"
...
def get_dynamic_headers(url):
    """ توليد الترويسات الديناميكية لتجاوز حظر 403 Forbidden من خوادم الـ CDN """
    headers = BROWSER_HEADERS.copy()
    parsed = urlparse(url)
    domain = parsed.netloc.lower()
    
    if "xnxx" in domain or "xnxx-cdn" in domain:
...
def is_complex_url(url):
    ...
    complex_domains = [
        ...
        'threads.net', 'snapchat.com', 'vk.com', 'vimeo.com', 'xnxx.com',
...
# --- التعرف الذكي واستخراج روابط الفيديو المباشرة من الصفحة ---
async def smart_extract_video_url(url):
    """ استخراج رابط الفيديو المباشر في حال كان الرابط صفحة ويب تحتوي فيديو """
    if any(url.lower().endswith(ext) for ext in ['.mp4', '.mkv', '.mov', '.avi', '.flv', '.webm', '.m3u8']):
        return url
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.get(url, timeout=10, ssl=False) as resp:
                if resp.status == 200:
                    ...
                    # 1. البحث باستخدام OpenGraph og:video
                    soup = BeautifulSoup(html, 'html.parser')
                    og_vid = soup.find('meta', property=re.compile(r'og:video(:secure_url)?', re.I))
                    if og_vid and og_vid.get('content'):
                        return og_vid['content']

                    # 2. البحث عن وسوم <video> أو <source>
                    for vid in soup.find_all(['video', 'source']):
                        src = vid.get('src')
                        if src and ('.mp4' in src or '.m3u8' in src or 'video' in src):
                            if src.startswith('//'):
                                return 'https:' + src
                            elif src.startswith('/'):
                                parsed = urlparse(url)
                    ...
                    # 3. البحث في السكريبتات بأستخدام Regex عالي الدقة
...
# --- التنزيل غير المتزامن المباشر المحدث لتجاوز خطأ 403 ---
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None):
    ...
    if not status_msg:
        status_msg = await bot.send_message(chat_id, "⏳ **جاري تحضير الطلب والسحب...**", buttons=cancel_btn)
    else:
        await status_msg.edit("⏳ **جاري تحضير الطلب والسحب...**", buttons=cancel_btn)
    ...
    try:
        ...
        # التعرف الذكي واستخراج روابط مقاطع الفيديو داخل الصفحات
        extracted_url = await smart_extract_video_url(url)
        target_url = clean_url(extracted_url) if is_complex_url(extracted_url) else extracted_url
        is_social = is_complex_url(target_url) if target_url else False
        
        if cancel_event.is_set(): raise Exception("CANCELLED")

        # استخدام عدة مكتبات سحب لمنع الأخطاء
        if target_url and (is_social or not target_url.endswith(('.mp4', '.mkv', '.mov', '.avi', '.flv'))):
            # المحاولة الأولى: yt-dlp
            await loop.run_in_executor(None, download_with_ytdlp, target_url, task_dir, target_fmt, quality)
            if cancel_event.is_set(): raise Exception("CANCELLED")

            # المحاولة الثانية احتياطية عند الفشل: gallery-dl / cobalt
            ...
            if not downloaded:
                if is_instagram_url(target_url):
                    ...
                    await loop.run_in_executor(None, gallery_dl_fallback_engine, task_dir, "reddit")
                else:
            ...
            try:
                ...
                if http_err.status == 403:
                    await loop.run_in_executor(None, download_with_ytdlp, target_url, task_dir, target_fmt, quality)
                    downloaded = [os.path.join(task_dir, f) for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                    if downloaded:
        ...
        if not downloaded_files:
            raise Exception("تعذر سحب أو تحديد الفيديو من الرابط. تأكد من صحة الرابط أو جرب جودة مختلفة.")
...
async def start_handler(event):
    welcome_text = (
        "🤖 **أهلاً بك في بوت التنزيل الذكي والشامل بالفيديوهات!**\n\n"
        "✨ **أبرز ميزات وخدمات البوت المحدثة:**\n\n"
        "🎬 **سحب أوتوماتيكي للتعرف على أية فيديوهات في أي رابط**\n"
        "• إظهار خيارات الجودة من 360p حتى 1080p فور إرسال أي رابط يحتوي فيديو.\n"
        "• استخدام عدة مكتبات متقدمة لتفادي الأخطاء واستخراج الفيديوهات بدقة.\n"
        "• دعم شامل: YouTube, X/Twitter, Instagram, TikTok, Facebook, Reddit, Pinterest, Threads, Snapchat, VK, Vimeo, XNXX وكافة صفحات الويب.\n"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "1️⃣ **معالجة حظر 403 Forbidden:** إضافة الترويسات الديناميكية و Referer لمنع رفض الطلبات من السيرفرات.\n"
            "2️⃣ **التعرف الذكي لروابط الفيديو:** استخراج مباشر وموثوق للفيديوهات في أي موقع أو صفحة ويب.\n"
            "3️⃣ **أزرار اختيار الجودة الشاملة:** إضافة اختيار الخيارات (1080p - 720p - 480p - 360p - MP3 - مستند).\n"
            "4️⃣ **تعدد مكتبات السحب:** دمج yt-dlp, gallery-dl, BeautifulSoup و Cobalt للتحميل بدون خطأ.\n"
            "5️⃣ **الحفاظ على الخصائص المتقدمة:** التقاط الـ 9 صور، التقسيم عند 2GB، والقص التلقائي بنفس الكفاءة.\n\n"
...
# --- معالج كافة الروابط وتوليد أزرار الجودة الذكية ---
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        # أزرار اختيار الجودة المتكاملة من 360p إلى 1080p
        buttons = [
            [Button.inline("🎬 FULL HD (1080p)", data=f"q_1080_{task_key}"), Button.inline("🎥 HD (720p)", data=f"q_720_{task_key}")],
            [Button.inline("📱 SD (480p)", data=f"q_480_{task_key}"), Button.inline("📉 منخفضة (360p)", data=f"q_360_{task_key}")],
            [Button.inline("🎵 صوت فقط (MP3)", data=f"q_mp3_{task_key}"), Button.inline("📄 مستند (Document)", data=f"q_doc_{task_key}")]
        ]
        
        await event.respond("🎬 **تم التعرف على الرابط! اختر جودة الفيديو أو الصيغة المطلوبة للتحميل:**", buttons=buttons)
...
async def quality_callback_handler(event):
    ...
    chat_id = event.chat_id
    as_doc = (quality_choice == 'doc')
    target_fmt = 'mp3' if quality_choice == 'mp3' else 'mp4'
    quality_val = 'best' if quality_choice == '1080' else quality_choice
    
    status_msg = await event.edit("⏳ **جاري الفحص الذكي وبدء سحب الفيديو بالطريقة الأنسب...**", buttons=None)
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة مع الدعم الشامل للجودات والتعرف الذكي على الفيديوهات!")
