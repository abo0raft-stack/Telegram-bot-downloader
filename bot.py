...
def update_and_install_libraries():
    try:
        ...
        print("✅ تم تثبيت وتحديث المكتبات بنجاح (مع دعم curl_cffi).")
...
VERSION = "v97.0-Dailymotion-CurlCffi-Update"
...
# --- تنزيل ورفع Dailymotion مع دعم curl_cffi وتقسيم أكبر من 2 جيجا ---
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    # إعداد yt-dlp لاستخدام curl_cffi لتجاوز حماية منصة Dailymotion
    ydl_opts = {
        'format': fmt,
        ...
        'impersonate': 'chrome'  # استخدام محاكي curl_cffi المتصفح الأصلي
    }
    ...
    try:
        ...
        # فحص تقسيم الفيديوهات ذات الحجم الأكبر من 2 جيجابايت
        video_parts = await split_video_file(file_path, max_size_mb=1950)

        total_parts = len(video_parts)
        for part_idx, part_file in enumerate(video_parts, start=1):
            if cancel_event.is_set():
                raise Exception("CANCELLED")

            start_time = time.time()
            last_upload_update = [0]
            part_str = f" (الجزء {part_idx}/{total_parts})" if total_parts > 1 else ""
            
            await status_msg.edit(f"📤 **جاري تحضير الرفع إلى تيليجرام{part_str}...**", buttons=cancel_btn)
            ...
            async def upload_progress_callback(current, total):
                ...
                if now - last_upload_update[0] >= 2.0 or current == total:
                    ...
                    text = (
                        f"📤 **جاري رفع الفيديو إلى تلجرام{part_str}...**\n\n"
                        f"[{bar}] {percentage:.1f}%\n"
                        f"🚀 **السرعة:** {format_size(speed)}/s\n"
            ...
            caption_out = ""
            if post_caption_text and part_idx == 1:
            ...
            if duration > 0 and user_config.get("snapshots", True) and part_idx == 1:
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "📺 **تحديث منصة Dailymotion ورفع الأحجام الكبيرة (v97):** إضافة مكتبة `curl_cffi` لتجاوز الحماية البرمجية لمنصة Dailymotion مع تفعيل التقسيم التلقائي للملفات أكبر من 2GB والرفع على دفعات.\n"
            ...
            "📝 **دعم استخراج نص المنشور (v91):** جلب النص والوصف الخاص بالمنشورات والفيديوهات من كافة منصات التواصل.\n\n"
...
async def set_bot_commands():
    try:
        ...
        print(f"⚠️ فشل إضافة قائمة الأوامر: {e}")
