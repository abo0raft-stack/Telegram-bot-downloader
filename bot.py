...
VERSION = "v90.0-Clickable-Filename-Copy"
...
# --- v89 التنزيل المباشر المحسّن المعالج لسرقات وتجاوزات السيرفرات الحامية (Direct Headers + Stream) ---
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    ...
    # التنزيل المباشر القياسي الذكي v89 المعالج لمنع الـ Anti-Hotlinking وحظر السيرفرات
    headers = BROWSER_HEADERS.copy()
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.get(url, ssl=False, allow_redirects=True) as response:
            if response.status not in [200, 206]:
                # في حال تطلب السيرفر headers دون Range بالبداية
            ...
            start_time = time.time()
            last_update_time = 0
            chunk_size = 512 * 1024 # زيادة حجم الشريحة 512KB لزيادة استقرار السرعة
