...
# --- الإعدادات ---
VERSION = "v16.0-SeparateScreenshots"
...
def generate_thumbnail(video_path, thumb_path, duration):
    try:
        target_sec = "2" if duration > 5 else str(max(1, int(duration / 2)))
        cmd = [
            'ffmpeg', '-y', '-ss', target_sec, 
...
def generate_9_individual_shots(video_path, msg_id, duration):
    ...
    try:
        ...
        print(f"Individual Shots Error: {e}")
        return [], temp_dir
...
async def start_execution(chat_id, url, filename_title):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            # 4. رفع الفيديو الرئيسي مع إرفاق الصورة المصغرة
            await bot.send_file(
                chat_id, 
                file_path, 
