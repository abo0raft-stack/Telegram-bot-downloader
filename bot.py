...
# --- الإعدادات ---
VERSION = "v46.0-ScreenshotsAlbum"
...
# --- استخراج بيانات الفيديو والصورة المصغرة للمشاهدة المباشرة ---
...
# --- توليد 9 لقطات شاشة وإرسالها كـ ألبوم ---
def generate_grid_screenshots(file_path, duration):
    screenshots = []
    if duration <= 0:
        duration = 60  # افتراضي في حال عدم تمكن النظام من معرفة مدة الفيديو
        
    # حساب الفواصل الزمنية التقريبية لأخذ 9 لقطات
    step = duration / 10.0
    for i in range(1, 10):
        timestamp = step * i
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, fmt='mp4'):
    ...
    try:
        ...
        # 2. تنزيل متوازي وسريع جداً من الروابط
        elif url:
            ...
            if is_m3u8 or not is_direct_mp4:
                await status_msg.edit("📡 **جاري التحميل المتعدد للرابط...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
        ...
        # 3. التحويل السريع
        if fmt in ['mp3', 'mkv'] or quality != 'best':
            await status_msg.edit(f"⚙️ **جاري التحويل الفائق إلى {fmt.upper()}...**")
            final_file = await loop.run_in_executor(None, convert_or_rescale_video, file_path, rescaled_path, fmt if fmt in ['mp3', 'mkv'] else quality)
        else:
            final_file = file_path

        # 4. الرفع الحقيقي مع المشغل والصورة المصغرة
        start_ul_time = time.time()
        def ul_progress(current, total):
            asyncio.run_coroutine_threadsafe(
                progress_callback(current, total, status_msg, "📤 **جاري الرفع إلى تليجرام...**", start_ul_time, cancel_event),
                loop
            )

        if fmt == 'mp3':
            await bot.send_file(chat_id, final_file, voice_note=False, progress_callback=ul_progress)
        else:
            duration, width, height, thumb_path = await loop.run_in_executor(
                None, get_video_metadata_and_thumb, final_file
            )
            ...
            await bot.send_file(
                ...
                thumb=thumb_path if not as_doc else None,
                attributes=video_attributes,
                supports_streaming=True,
                progress_callback=ul_progress
            )

            # توليد وإرسال 9 لقطات شاشة كألبوم بعد الرفع
            await status_msg.edit("🖼️ **جاري التقاط 9 لقطات شاشة وإرسالها كألبوم...**")
            screenshots = await loop.run_in_executor(
                None, generate_grid_screenshots, final_file, duration
            )
            ...
            if screenshots:
                await bot.send_file(
                    chat_id,
                    file=screenshots,
                    caption="📸 **لقطات شاشة توضيحية من داخل الفيديو**"
                )
        ...
        if "CANCELLED_BY_USER" in str(e):
            ...
            print(f"Error: {e}")
            await status_msg.edit(f"❌ **حدث خطأ أثناء المعالجة:** `{e}`")
    finally:
        ...
        for path in [file_path, rescaled_path]:
