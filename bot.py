            ...
            if not sent_as_album:
                for idx, p in enumerate(photos, start=1):
                    try:
                        ...
                        # التعديل الهام: إذا أُرسلت الطلب كمستند، يتم التسمية بصيغة .jpg
                        if as_doc:
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        if is_x_url(clean_u):
            ...
            # فحص ذكي لنوع رابط انستغرام (فيديو أو صور)
            u_lower = clean_u.lower()
            is_ig_video = "/reel/" in u_lower or "/tv/" in u_lower

            if is_ig_video:
                # تنزيل الفيديو المباشر من انستغرام فوراً
                user_config = get_user_config(chat_id)
                asyncio.create_task(
                    start_direct_execution(
                        chat_id=chat_id,
                ...
                # إذا كان منشور/صور/ألبوم (/p/) أو رابط عام، يعرض خيارات التنزيل ومنها "مستند"
                ...
                await event.respond("📸 **تم التعرف على رابط منشور Instagram. اختر طريقة التنزيل:**", buttons=buttons)
