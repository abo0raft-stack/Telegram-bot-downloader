...
# --- استخراج معلومات وصور الفيديو باستخدام MoviePy ---
def process_video_media(video_path, msg_id):
    duration, width, height = 0, 1280, 720
    thumb_path = f"downloads/thumb_{msg_id}.jpg"
    shots_dir = f"downloads/shots_{msg_id}"
    shots_list = []
    ...
    try:
        ...
        # 1. توليد الصورة المصغرة (Thumbnail)
        thumb_time = 2 if duration > 5 else max(1, duration / 2)
        ...
        # 2. توليد 9 لقطات منفصلة موزعة على الفيديو
        os.makedirs(shots_dir, exist_ok=True)
        step = duration / 10 if duration > 0 else 5
        
        for idx in range(1, 10):
            ts = step * idx
            if ts >= duration: break
            shot_file = os.path.join(shots_dir, f"shot_{idx}.jpg")
            clip.save_frame(shot_file, t=ts)
    ...
    return duration, width, height, thumb_path, shots_list, shots_dir
...
async def start_execution(chat_id, url, filename_title):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            # معالجة استخراج الصور والبيانات بواسطة MoviePy
            duration, width, height, thumb_path, shots_list, shots_dir = await loop.run_in_executor(
                None, process_video_media, file_path, status_msg.id
            )

            # 1. إرسال اللقطات المنفصلة كمجموعة ألبوم
            if shots_list:
                await bot.send_file(
                    chat_id, 
                    shots_list, 
            ...
            # 3. رفع الفيديو ومعه الصورة المصغرة المرفقة
            await bot.send_file(
                chat_id, 
                file_path, 
        ...
        for path in [file_path, f"downloads/thumb_{status_msg.id}.jpg"]:
        ...
        if os.path.exists(shots_dir):
            for f in os.listdir(shots_dir):
                try: os.remove(os.path.join(shots_dir, f))
                except: pass
            try: os.rmdir(shots_dir)
            except: pass
