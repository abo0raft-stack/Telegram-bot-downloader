...
VERSION = "v85.0-Add-WhatsApp-Avatar-Support"
...
# --- دالة مخصصة لتحميل الصورة الشخصية (Profile Picture / Avatar) مع دعم WhatsApp ---
async def download_profile_avatar(url, task_dir):
    try:
        if is_whatsapp_url(url):
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=BROWSER_HEADERS, timeout=20) as r:
                    if r.status == 200:
                        html = await r.text()
                        ...
                        if img_url:
                            async with session.get(img_url, headers=BROWSER_HEADERS, timeout=20) as img_res:
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None, is_avatar_task=False):
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if is_avatar_task:
                await status_msg.edit("👤 **جاري استخراج صورة الملف الشخصي (Avatar)...**", buttons=cancel_btn)
