...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, fmt='mp4'):
    ...
    try:
        if media_msg:
            ...
            try:
                # المحاولة الأولى عبر محرك التحميل المباشر المتعدد
                await loop.run_in_executor(None, download_direct_file, url, file_path, cancel_event, 8)
            except Exception as e:
                # التبديل إلى yt-dlp عند وجود خطأ مع تجنب الحظر
                await status_msg.edit("🔄 **جاري التحميل والتجاوز عبر المحرك البديل...**")
