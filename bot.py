...
def update_and_install_libraries():
    try:
        import subprocess
        subprocess.run(["pip", "install", "-U", "yt-dlp", "gallery-dl", "Pillow", "aiohttp", "aiosqlite", "telethon", "requests"], check=True)
...
# تحديد حد أقصى للعمليات الثقيلة المتزامنة لمنع انهيار السيرفر بسبب CPU/RAM
MAX_CONCURRENT_TASKS = asyncio.Semaphore(2)

# --- إدارة قاعدة البيانات (SQLite - Async with aiosqlite) ---
...
VERSION = "v80.2-Private-Edition"
API_ID = int(os.environ.get("API_ID", 0))
API_HASH = os.environ.get("API_HASH", "")
BOT_TOKEN = os.environ.get("BOT_TOKEN", "")
OWNER_ID = int(os.environ.get("OWNER_ID", "5414125521")) # معرف المالكون الحصري للبوت
...
# التحقق السريع من ملكية المستخدم
        ...
        try:
            ...
            if target_url and is_social:
                ...
                if target_fmt == 'mp3':
                    ...
                    mp3_path = base + ".mp3"
                    proc = await asyncio.create_subprocess_exec('ffmpeg', '-y', '-i', filepath, '-vn', '-ab', '192k', mp3_path, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
                    await proc.wait()
...
async def start_handler(event):
    ...
    welcome_text = (
        ...
        "🔐 **الحالة:** تم تفعيل الوضع الخاص (Private Mode) بنجاح.\n\n"
        "✨ **أبرز الميزات المتاحة:**\n"
        ...
        "• **تقسيم الفيديوهات تلقائياً** لقطع +1.9GB.\n"
        "• **قص المقاطع:** أرسل `/trim [بدء] [نهاية] [رابط]`.\n\n"
        "⚙️ لتعديل إعداداتك أرسل: `/settings`"
    )
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "1️⃣ **إضافة منصات جديدة:** دعم شامل للتحميل من Reddit, Pinterest, Threads, Snapchat, VK, Vimeo.\n"
            "2️⃣ **معالجة صور والألبومات:** معالجة وسائط متعددة لسحب الألبومات بسهولة.\n"
            "3️⃣ **تنسيق عرض الوقت:** إظهار الوقت بدقائق وثوانٍ.\n"
            "4️⃣ **أداء Non-Blocking:** تحسينات الاستجابة مع aiosqlite.\n"
            "5️⃣ **تقسيم الفيديوهات:** تقسيم ملفات +1.9GB آلياً.\n"
            "6️⃣ **قص المقطع الزمني:** أمر `/trim` الحصري.\n\n"
