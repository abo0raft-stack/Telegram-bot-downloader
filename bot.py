...
# --- التحديث التلقائي للمكتبات لمنع أعطال إنستغرام ---
...
# --- الإعدادات ---
VERSION = "v60.0-Instagram-Photos-Carousel-Fix"
...
def instagram_carousel_and_photo_engine(url, task_dir):
    ...
    try:
        # المحرك 1: استخدام Gallery-DL المخصص للصور والمعارض
        ...
        # إذا تم التحميل ننهي العمل
        if any(os.path.isfile(os.path.join(task_dir, f)) for f in os.listdir(task_dir)):
            return

        # المحرك 2: استدعاء Cobalt API لاستخراج الصور بشكل مباشر
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        if target_url and is_complex_url(target_url):
            ...
            # 2. إذا كان من إنستغرام ولم يُجلب شيء (خاصة الصور) نفعّل محرك الصور والألبومات الخاص
        ...
        # البحث في كافة المجلدات عن صور وفيديوهات تم تنزيلها
        downloaded_files = []
        ...
        # 1. إرسال الصور (سواء صورة فردية أو ألبوم متكامل)
        if photos:
            if len(photos) == 1:
        ...
        # 2. إرسال الفيديوهات كمشغل متكامل
        for vid in videos:
...
async def start_handler(event):
    await event.respond(
        ...
        "📸 **إنستغرام:** دعم كامل 100% للصور، ألبومات الصور (Carousel)، والفيديوهات (Reels).\n"
...
def main():
    ...
    print("🤖 البوت يعمل بكفاءة عالية وتم التحديث لدعم صور إنستغرام بجميع أنواعها!")
