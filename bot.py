...
async def start_handler(event):
    welcome_text = (
        ...
        "✨ **المميزات:**\n"
        " 🎬 **رفع كـ فيديو ميديا أو ملف مستند** حسب اختيارك.\n"
...
async def url_handler(event):
    ...
    USER_STATES[event.sender_id] = {'url': url, 'custom_name': default_name, 'as_doc': False}
    buttons = [
        [Button.inline("🎬 بدء التنزيل (فيديو)", data="start_dl_stream"), Button.inline("📁 تنزيل كـ (ملف خام)", data="start_dl_doc")],
...
async def callback_handler(event):
    ...
    elif data in ["start_dl_stream", "start_dl_doc"]:
        state_data = USER_STATES.get(user_id)
        if state_data:
            as_doc = (data == "start_dl_doc")
            await event.delete()
            await start_execution(event.chat_id, state_data['url'], state_data['custom_name'], as_doc)
...
async def text_handler(event):
    ...
    if user_id in USER_STATES and USER_STATES[user_id].get('state') == 'waiting_for_name':
        ...
        buttons = [
            [Button.inline("🎬 تنزيل (فيديو)", data="start_dl_stream"), Button.inline("📁 تنزيل (ملف)", data="start_dl_doc")]
        ]
        USER_STATES[user_id] = {'url': url, 'custom_name': new_name}
        await event.respond(f"✅ تم تغيير الاسم إلى: `{new_name}`\nاختر نوع الرفع:", buttons=buttons)
...
async def start_execution(chat_id, url, filename_title, as_doc=False):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            # --- 2. إرسال الفيديو كـ ميديا أو كـ ملف (حسب اختيار المستخدم) ---
            if as_doc:
                await bot.send_file(
                    chat_id, 
                    file_path, 
            ...
            # --- 3. توليد وإرسال اللقطات التسع ---
            await status_msg.edit("📸 **تم رفع الملف! جاري استخراج وإرسال اللقطات التسع...**", buttons=None)
...
async def main():
    clean_download_folder()  # تنظيف الملفات المتبقية إن وجدت عند التشغيل
