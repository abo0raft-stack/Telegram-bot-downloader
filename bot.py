...
def extract_web_info(url):
    ydl_opts = {
        ...
        'impersonate': None,
        'socket_timeout': 60, # رفع مهلة المهام الأولية
...
async def url_handler(event):
    ...
    if len(urls) == 1:
        url = urls[0]
        checking_msg = await event.respond("🌐 **جاري تحليل رابط الفيديو...**")
...
async def start_execution(chat_id, url, filename_title, as_doc=False):
    status_msg = await bot.send_message(chat_id, "🔍 **جاري الفحص واستخراج البث...**")
    msg_id = status_msg.id
    ...
    try:
        ...
        await status_msg.edit("⚡ **جاري الاتصال بالسيرفر وإكمال التنزيل...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{msg_id}")])
        ...
        # الخيارات المعززة لتفادي مشاكل انقضاء المهلة (Read Timeout)
        ydl_opts = {
            ...
            # --- تعديلات حاسمة لحل خطأ Timeout ---
            'socket_timeout': 120,          # زيادة مهلة الاتصال إلى 120 ثانية
            'extractor_retries': 10,        # عدد محاولات استخراج البيانات عند التعثر
            'retries': 20,                  # عدد محاولات التنزيل عند انقطاع الشبكة
            'fragment_retries': 20,
        ...
        if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
            ...
            raise Exception("فشل التنزيل، تعذر استخراج مقطع شغال من الصفحة.")
