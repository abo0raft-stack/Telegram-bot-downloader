...
def setup_all_cookies():
    ...
    yt_b64 = os.environ.get("YOUTUBE_COOKIES_BASE64")
    if yt_b64:
        try:
...
def format_time(seconds):
    if not seconds or seconds <= 0: return "0 ثانية"
    seconds = int(seconds)
    mins = seconds // 60
    secs = seconds % 60
...
# --- دالة التنزيل والرفع المخصصة لـ YouTube مع معالجة حماية الصيغ المتغيرة ---
async def download_youtube_video(event, url, quality_choice, status_msg):
    ...
    if not status_msg:
        status_msg = await event.respond("⏳ **جاري بدء عملية التحميل من YouTube...**", buttons=cancel_btn)
    else:
        await status_msg.edit("⏳ **جاري بدء عملية التحميل من YouTube...**", buttons=cancel_btn)
    ...
    def progress_hook(d):
        ...
        if d['status'] == 'downloading':
            ...
            if now - last_update_time[0] >= 2.0:
                ...
                text = (
                    f"📥 **جاري تنزيل الفيديو من YouTube...**\n\n"
                    f"[{bar}] {percentage:.1f}%\n"
    ...
    # تعديل الفلترة المرنة لمنع خطأ Format Not Available
    if quality_choice == 'mp3':
        ...
        if quality_choice != '1080':
            fmt = f"bestvideo[height<={quality_choice}]+bestaudio/best[height<={quality_choice}]/best"
        else:
    ...
    try:
        ...
        if quality_choice == 'mp3':
            base_p = os.path.splitext(file_path)[0] + ".mp3"
            if os.path.exists(base_p): file_path = base_p
        else:
        ...
        if quality_choice == 'mp3':
            await bot.send_file(
                chat_id,
                file_path,
                caption="🎵 **تم استخراج الصوت بنجاح من YouTube!**",
            ...
            await bot.send_file(
                chat_id,
                file_path,
                caption="🎥 **تم التنزيل بنجاح من YouTube!**",
                thumb=thumb_path,
                attributes=attr,
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    fmt = 'bestaudio/best' if quality_choice == 'mp3' else (f"bestvideo[height<={quality_choice}]+bestaudio/best" if quality_choice != '1080' else 'bestvideo+bestaudio/best')
...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best'):
    ...
    if is_youtube_url(url) and os.path.exists(YOUTUBE_COOKIES_FILE):
        ydl_opts['cookiefile'] = YOUTUBE_COOKIES_FILE
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🔴 **إصلاح مشكلة requested format is not available:** إتاحة جميع الصيغ المفتوحة (WebM / VP9) مع تحويلها تلقائياً بالخلفية بدلاً من فرض امتداد مخصص قد يفشل التنزيل عند غيابه.\n"
            "🔴 **إصلاح حظر يوتيوب الشامل:** معالجة مشكلة `The page needs to be reloaded` بالربط الديناميكي مع Player Clients المختلفة (android/ios/web).\n"
            "🔴 **إضافة كوكيز يوتيوب:** تفعيل متغيّر `YOUTUBE_COOKIES_BASE64` لتجاوز القيود والحظر.\n"
            "🔴 **مؤشرات تقدم YouTube الحية:** إظهار شريط التقدم النسبة المئوية وحجم الملف التراكمي والدقائق/الثواني المتبقية للتنزيل والرفع بشكل مباشر.\n"
...
# --- معالج خيارات الجودة المحدث لـ YouTube و Dailymotion والمنصات الأخرى ---
...
async def quality_callback_handler(event):
    ...
    if task_type == "youtube" or is_youtube_url(url):
        status_msg = await event.edit("⏳ **تم استلام طلبك، جاري بدء التنزيل من YouTube...**", buttons=None)
        asyncio.create_task(
    ...
    elif task_type == "dailymotion" or is_dailymotion_url(url):
