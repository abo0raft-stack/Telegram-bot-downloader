...
# --- الإعدادات ---
VERSION = "v75.0-Railway-Enhanced-Full-Edition"
...
# --- إضافة النص والوقت في وسط الصورة بخط أبيض شفاف ---
def add_transparent_text_center(image_path, text):
    try:
        with Image.open(image_path).convert("RGBA") as base:
            ...
            w, h = base.size
            font_size = max(24, int(min(w, h) * 0.12))
            ...
            # أبيض شفاف (RGB: 255,255,255 - الشفافية Alpha: 130)
...
def extract_9_frames(video_path, duration):
    frames = []
    if duration <= 0:
        return frames
    ...
    for idx, ts in enumerate(timestamps, start=1):
        ...
        if os.path.exists(out_name) and os.path.getsize(out_name) > 0:
            add_transparent_text_center(out_name, time_str)
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        if target_url and is_complex_url(target_url):
            ...
            if as_doc:
                ...
                if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                    ...
                    # استخراج وإرسال 9 لقطات إجبارياً بعد الرفع
                    if duration > 0:
                        await status_msg.edit("📸 **جاري التقاط 9 صور من الفيديو وتجهيز الألبوم...**")
                        frames = extract_9_frames(filepath, duration)
                        if frames:
        ...
        for vid in videos:
            ...
            # إرسال ألبوم اللقطات التسع للفيديو إجبارياً
            if duration > 0:
                await status_msg.edit("📸 **جاري توليد ألبوم اللقطات الـ 9 للفيديو...**")
                frames = extract_9_frames(vid, duration)
                if frames:
...
async def start_handler(event):
    changelog_text = (
        ...
        "1️⃣ **إلغاء العمليات:** إمكانية إيقاف وإلغاء أي عملية تنزيل أو رفع فوراً عبر زر `❌ إلغاء العملية`.\n"
        "2️⃣ **لقطات الفيديو:** إرسال ألبوم مكون من 9 لقطات تلقائياً ومقسّمة على الفيديو بعد الرفع.\n"
        "3️⃣ **توقيت اللقطات:** كتابة توقيت كل لقطة بخط أبيض شفاف كبير وسط الصورة.\n"
        "4️⃣ **شريط التقدم المطور:** دعم شريط التقدم للرفع والتنزيل مع إظهار (السرعة، الوقت المنقضي، الوقت المتبقي، وحجم الملف).\n\n"
        "⚡ **المنصات المدعومة:**\n"
        "📸 **إنستغرام:** الصور، الألبومات، الفيديوهات و Reeling.\n"
        "🎵 **تيك توك:** فيديوهات بدون علامة مائية وسلاسل الصور.\n"
        "𝕏 **منصة X (تويتر):** جودات متعددة وصوت MP3.\n"
        "🌐 **الروابط المباشرة:** تنزيل مباشر سريع بجميع الصيغ (MP4 / MP3 / Document)."
    )
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة وجاهز تماماً لـ Railway!")
