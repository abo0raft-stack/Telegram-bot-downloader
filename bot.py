...
# --- الإعدادات ---
VERSION = "v27.0-RealDownscaleLight"
...
# --- دالة الضغط الخفيف وإعادة تغيير الجودة الحقيقي عبر FFmpeg ---
def compress_and_rescale_video(input_path, output_path, target_quality):
    ...
    if target_quality not in scale_map:
        return input_path

    vf_scale = scale_map[target_quality]
    
    # خيارات خفيفة جداً لمنع الضغط العالي على المعالج ورام السيرفر (preset ultra fast)
    cmd = [
        'ffmpeg', '-y',
        '-i', input_path,
...
async def start_handler(event):
    welcome_text = (
        ...
        " 🎯 **تغيير وضغط الجودة الحقيقي (1080p / 720p / 480p / MP3)**.\n"
...
async def callback_handler(event):
    ...
    if data == "ask_rename":
        ...
        quality_buttons = [
            ...
            [Button.inline("📺 1080p (حقيقي)", data="set_q_1080"), Button.inline("📺 720p (حقيقي)", data="set_q_720")],
            [Button.inline("📱 480p (حقيقي)", data="set_q_480"), Button.inline("🎵 صوت فقط MP3", data="set_q_audio")]
        ]
        await event.edit("⚙️ **اختر الجودة المطلوبة للتحميل والضغط:**", buttons=quality_buttons)
    ...
    try:
        ...
        if is_direct_url:
            ...
            try:
                ...
                if "CANCELLED_BY_USER" in str(direct_err):
                    raise direct_err
                
                await status_msg.edit("⚠️ **تجاوز حظر 403 عبر محرك yt-dlp...**")
        ...
        if os.path.exists(file_path):
            ...
            # --- معالجة تغير الجودة وإعادة الضغط الحقيقية في حال التحميل المباشر ---
            if quality in ['1080', '720', '480']:
                await status_msg.edit(f"⚙️ **جاري تغيير الجودة وضغط الفيديو حقيقياً إلى {quality}p...**", buttons=None)
            ...
            caption_text = (
                ...
                f"🎯 **الجودة:** `{quality}`\n"
                f"💾 **الحجم النهائي:** `{file_size_mb:.1f} MB`\n\n"
