...
# --- الإعدادات ---
VERSION = "v70.0-Final-X-Platform-Fix"
...
def x_media_engine(url, task_dir):
    """محرك مخصص وشامل لمنصة X للصور والفيديوهات"""
    try:
        ...
        if res.status_code == 200:
            ...
            for idx, media_url in enumerate(urls_to_dl, start=1):
                ...
                if r.status_code == 200:
                    ...
                    filepath = os.path.join(task_dir, f"x_media_{idx}.{ext}")
                    with open(filepath, 'wb') as f:
                        for chunk in r.iter_content(chunk_size=1024*1024):
                            if chunk: f.write(chunk)
    except Exception as e:
        print(f"X Cobalt API error: {e}")
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        if target_url and is_complex_url(target_url):
            ...
            # محرك الميديا الخاص بـ X عند الشك أو عدم اكتمال التنزيل عبر yt-dlp
            if is_x_url(target_url):
                downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                if not downloaded:
                    await loop.run_in_executor(None, x_media_engine, target_url, task_dir)
        ...
        # 1. إرسال الصور بنظام الرفع المباشر
        if photos:
            sent_as_album = False
...
async def start_handler(event):
    await event.respond(
        ...
        "𝕏 **منصة X:** إمكانية اختيار جودة الفيديو والصور بشكل كامل ومباشر.\n\n"
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة وتم دعم تنزيل صور منصة X بنجاح!")
