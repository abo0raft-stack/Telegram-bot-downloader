...
# --- محرك انستغرام المُحدث كلياً لحل مشكلة عدم تنزيل الصور v85 ---
async def instagram_carousel_and_photo_engine(url, task_dir):
    # 1. التجربة عبر gallery-dl الممتاز للتغلب على التقييد
    try:
        cmd = [
            "gallery-dl",
            "--directory", task_dir,
    ...
    # 3. الاستعانة بمحرك GraphQL والمحركات السريعة (Cobalt + IG Direct CDN Scraper)
    try:
        api_url = "https://api.cobalt.tools/api/json"
        headers = {
            "Accept": "application/json",
    ...
    # 4. محرك مخصص للاستخراج المباشر بدقة للصور الثابتة والألبومات
    try:
...
# --- معالج خيارات الجودة المحدث لـ Dailymotion والمنصات الأخرى ---
