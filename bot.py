...
def fix_mp4_container(input_path, output_path):
    """إعادة تجميع الفيديو بدون إعادة ترميز لضمان توافق الحاوية مع تليجرام"""
    try:
        cmd = ['ffmpeg', '-y', '-i', input_path, '-c', 'copy', '-bsf:a', 'aac_adtstoasc', output_path]
        process = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=120)
        if process.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
            return output_path
    except Exception as e:
        print(f"Fix Container Error: {e}")
    return input_path
...
def generate_thumbnail_fallback(video_path, thumb_path, duration):
    try:
        ...
        process = subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=30)
...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            await status_msg.edit("🛠️ **جاري إصلاح حاوية الفيديو للرفع...**")
            
            # إصلاح صيغة MPEG-TS إلى MP4 قياسي
            final_video_path = await loop.run_in_executor(None, fix_mp4_container, file_path, fixed_path)
            if final_video_path != file_path and os.path.exists(final_video_path):
                os.remove(file_path)
