...
VERSION = "v86.0-Enhanced-Avatar-Engine"
...
# --- محرك v86 المحسّن والشامل لجلب الصورة الشخصية (Profile Picture / Avatar) ---
async def download_profile_avatar(url, task_dir):
    try:
        # 1. المعالجة المباشرة لواتساب عبر جلب ميتاداتا Meta HTML والتأكد من جلب الصورة الأصلية الحقيقية
        if is_whatsapp_url(url):
        ...
        # 2. الاستعانة بمحرك gallery-dl لاستخراج الأفاتار للمنصات المدعومة (Instagram, X, إلخ)
        cmd = [
            "gallery-dl",
            "--directory", task_dir,
        ...
        # 3. استخدام yt-dlp لاستخراج الصورة المصغرة (Thumbnail / Avatar) كبديل ثانوي
        ...
        # 4. محرك Cobalt API كحل أخير ومساند للأفاتار
        ...
        print(f"v86 Profile Avatar Engine Error: {e}")
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None, is_avatar_task=False):
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if is_avatar_task:
                await status_msg.edit("👤 **جاري استخراج صورة الملف الشخصي عبر المحرك الجديد (v86)...**", buttons=cancel_btn)
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **تحديث محرك الأفاتار (v86):** تم إضافة واستبدال محرك جلب الصورة الشخصية بمحرك هجين متعدد الطبقات يعالج روابط الواتساب، إنستغرام، إكس، وغيرها بثبات عالي.\n"
            "🟢 **دعم تنزيل صور واتساب (v85):** إضافة إمكانية تنزيل الصورة الشخصية لحسابات واتساب مباشرة من خلال إرسال رابط مثل `https://wa.me/PHONE_NUMBER`.\n"
            ...
            "👤 **تنزيل الصور الشخصية (Avatars):** دعم إرسال روابط الملفات الشخصية (Instagram, TikTok, X, YouTube, Pinterest, WhatsApp) لتنزيل صورة البروفايل بأعلى جودة.\n"
