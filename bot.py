...
# استيراد curl_cffi لتجاوز حظر CDN
...
def smart_download_engine(url, filepath, status_msg, loop, cancel_event):
    ...
    headers = {
        ...
        "Origin": f"{parsed_u.scheme}://{netloc}",
        "Connection": "keep-alive"
    }

    # المحاولة الأولى: تنزيل مباشر محسّن عبر curl_cffi
    try:
    ...
    # المحاولة الثانية: استخدام yt-dlp مع تفعيل الوضع العام لتجنب Unsupported URL
    if os.path.exists(filepath):
        try: os.remove(filepath)
        except: pass

    asyncio.run_coroutine_threadsafe(
        status_msg.edit("⚡ **جاري التحميل عبر المحرك الاحتياطي...**"), loop
    )
    ...
    ydl_opts = {
        ...
        'default_search': 'auto',
        'force_generic_extractor': True, # حل مشكلة Unsupported URL لروابط الميديا والـ CDN
...
async def start_handler(event):
    welcome_text = (
        ...
        "✨ **تم إصلاح معالجة الروابط المباشرة وتجاوز خطأ Unsupported URL بنجاح.**"
    )
