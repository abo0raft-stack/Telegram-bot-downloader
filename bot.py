...
# --- الإعدادات ---
VERSION = "v26.0-UltimateFeaturePacked"
...
async def text_handler(event):
    ...
    if user_id in USER_STATES and USER_STATES[user_id].get('state') == 'waiting_for_name':
        ...
        if not new_name.endswith(('.mp4', '.mkv', '.avi', '.mov', '.mp3')): 
            new_name += ".mp4"
...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        ...
        if is_direct_url:
            ...
            # تحديد الجودة بناءً على خيار المستخدم
            format_opt = 'best'
        ...
        # معالجة تغيير الامتداد في حال تحميل صوت MP3
        ...
        if os.path.exists(file_path):
            ...
            # --- 2. إرسال الصوت أو الفيديو أو الملف مع معالجة حظر تلجرام ---
            retry = True
            while retry:
                try:
            ...
            # --- 3. توليد وإرسال اللقطات التسع (في حال كان ملف فيديو) ---
            if quality != 'audio':
