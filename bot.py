...
def update_libraries():
    try:
        import subprocess
        subprocess.run(["pip", "install", "-U", "yt-dlp", "gallery-dl", "Pillow", "aiohttp", "aiosqlite"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        print("✅ تم تحديث مكتبات التنزيل ومعالجة الصور وقواعد البيانات بنجاح.")
...
# --- استخراج معلومات الفيديو عبر asyncio.subprocess ---
...
# --- التقسيم التلقائي للفيديوهات الكبيرة مع asyncio.subprocess ---
...
# --- قص جزء محدد من الفيديو عبر asyncio.subprocess ---
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None):
    ...
    try:
        ...
        # --- تنزيل ألبومات الصور بطريقة مستقرة وبدون FloodWait عبر send_file مباشرة مع album=True ---
        if photos:
            if len(photos) > 1 and not as_doc:
                try:
                    # تقسيم الصور إلى مجموعات ألبومات أقصاها 10 صور لكل ألبوم كحد أقصى لتيليجرام
...
async def start_handler(event):
    welcome_text = (
        ...
        "• **حفظ التفضيلات الدائم:** حفظ خياراتك عبر SQLite اللاتزامنية.\n\n"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "4️⃣ **تحسين الأداء Non-Blocking:** التنزيل المباشر مع aiosqlite و asyncio.subprocess لتسريع الاستجابة.\n"
