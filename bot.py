...
# هيدرات متصفح فائقة الدقة لتبدو مثل TDM / Chrome لتجاوز 403 Forbidden
...
def download_file_direct_fallback(url, filepath, status_msg, loop, cancel_event):
    ...
    # تجاوز فحص الشهادات الأمنية لبعض سيرفرات CDN
    import ssl
...
def download_media_smart(url, filepath, status_msg, loop, cancel_event):
    parsed_u = urlparse(url)
    origin_referer = f"{parsed_u.scheme}://{parsed_u.netloc}/"
    ...
    ydl_opts = {
        ...
        'default_search': 'auto',
        'force_generic_extractor': True,  # إجبار yt-dlp على تحميل الروابط المباشرة بدون بحث عن Extractor
    ...
    try:
        ...
        if cancel_event.is_set(): raise Exception("CANCELLED_BY_USER")
        # إذا فشل yt-dlp ينزل الملف بالمستعرض المباشر التلقائي
        download_file_direct_fallback(url, filepath, status_msg, loop, cancel_event)
...
async def start_handler(event):
    welcome_text = (
        ...
        "✨ **تم إصلاح التعرف على الروابط المباشرة لتنزيل الميديا بدون أخطاء!**"
    )
...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        await status_msg.edit("📥 **جاري تحميل الملف...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
        
        await loop.run_in_executor(None, download_media_smart, url, file_path, status_msg, loop, cancel_event)
        ...
        if temp_shots_dir and os.path.exists(temp_shots_dir):
            shutil.rmtree(temp_shots_dir, ignore_ignore=True)
