...
# --- الإعدادات ---
VERSION = "v71.0-DirectDownload-CancelSupport"
...
async def handle_message(event):
    ...
    if text.startswith('/start'):
        await event.respond("👋 **أهلاً بك! أرسل لي أي رابط مباشر أو من منصات التواصل الاجتماعي لبدء التحميل.**")
        return

    # التمييز بين روابط التواصل والروابط المباشرة
    if text.startswith('http://') or text.startswith('https://'):
        if is_complex_url(text):
            # رابط منصة تواصل اجتماع - تنزيل تلقائي مباشرة
