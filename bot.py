...
async def extract_9_frames(video_path, duration, chat_id=None):
    frames = []
    if duration <= 0 or not os.path.exists(video_path): return frames
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"لم يتم العثور على ملف Dailymotion المحمل: {file_path}")
...
async def tiktok_multi_engine(url, task_dir):
    ...
    if os.path.exists(task_dir) and any(os.path.isfile(os.path.join(task_dir, f)) for f in os.listdir(task_dir)):
        return
...
# --- استئناف وتنفيذ التنزيل المباشر الذكي (محدث ومحسّن لحجب الـ CDN ومعالجة المسارات) ---
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    ...
    # Fallback إلى yt-dlp في حال حظر سيرفر CDN للتنزيل المباشر أو التحميل بحجم وهمي
    if not success or (os.path.exists(filepath) and os.path.getsize(filepath) < 10240):
        try:
            if os.path.exists(filepath):
                try: os.remove(filepath)
                except: pass
                
            await status_msg.edit("⚡ **جاري تجاوز حماية CDN عبر محرك yt-dlp...**")
            ...
            print(f"Fallback yt-dlp error: {ex}")
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if target_url and is_social:
                ...
                downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0] if os.path.exists(task_dir) else []
                if not downloaded:
                ...
                clean_fn = unquote(filename)
                filepath = os.path.join(task_dir, clean_fn)
                ...
                # إصلاح مشكلة البحث عن الملف عند عدم تطابق اسمه المسجل مع التنزيل المباشر
                if not os.path.exists(filepath):
                    files_in_dir = [os.path.join(task_dir, f) for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                    if files_in_dir:
                        filepath = files_in_dir[0]

                if not os.path.exists(filepath):
                    raise FileNotFoundError(f"لم يتم العثور على الملف بعد التنزيل المباشر: {filepath}")
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    if not os.path.exists(vid_file): continue
            ...
            for vid in videos:
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    if not os.path.exists(part_vid): continue
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🛡️ **تجاوز حماية CDN:** تم بناء مسار احتياطي لتخطي حظر السيرفرات عند محاولة تحميل روابط الميديا المباشرة.\n"
