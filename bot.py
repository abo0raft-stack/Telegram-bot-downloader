...
# --- محرك التنزيل لـ yt-dlp المحدث والمحسّن مع حل مشكلة ydl_opts ---
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    ...
                    if as_doc:
                        ...
                        if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                            ...
                            attr = [DocumentAttributeVideo(duration=duration, w=width, h=height, supports_streaming=True)]
                            await bot.send_file(chat_id, file_to_send, caption=caption_text, thumb=thumb_path, attributes=attr)
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🛠️ **حل مشكلة Upload File Part (v99.5):** بناء دالة الرفع المباشر `upload_file_smart` بحزم متوازنة لمنع فشل الرفع عند الأجزاء المقتطعة العالية (مثل part 1656).\n"
            "✨ **دعم نطاق brqz.online (v99.4):** إضافة الدعم المباشر والشامل للتنزيل من نطاق brqz.online مع الحفاظ على كامل الميزات الهيكلية واستئناف التنزيل.\n"
            "🚀 **حل مشكلة الحظر (v99.3):** إضافة نظام التخطي الذكي وإعادة المحاولة التلقائية (Exponential Backoff) لحل مشكلة '429 Too Many Requests'.\n"
            "⚡ **تحديث اختصارات الأوامر (v99.2):** دعم تنفيذ `/start` بـ `/` و `/settings` بـ `//`.\n"
            "🔧 **إصلاح خيارات التنزيل (v99.1):** إعادة هيكلة `ydl_opts` لمنع أخطاء التوافق مع ffmpeg.\n"
            "🎵 **تحديث شامل لمحرك TikTok (v99):** بناء محرك هجين متعدد المصادر للفيديوهات والصور مع الترجمة.\n\n"
...
async def handle_user_input_and_urls(event):
    ...
    for u in urls:
        ...
        if is_profile_url(clean_u):
            ...
            await event.respond("📸 **تم التعرف على رابط Instagram. اختر خيار التحميل:**", buttons=buttons)

        else:
            filename = get_clean_filename(clean_u)
            status_msg = await event.respond("⏳ **جاري التنزيل المباشر من الرابط...**")
            user_config = await get_user_config(chat_id)
            asyncio.create_task(
                start_direct_execution(
                    chat_id=chat_id,
...
@bot.on(events.CallbackQuery(pattern=r"^(q_1080_|q_720_|q_480_|q_mp3_|prof_img_|prof_doc_|tt_photo_|tt_doc_)(.*)"))
async def process_task_callback(event):
    ...
    task_key = event.pattern_match.group(2)
    ...
    if action.startswith("prof_"):
        as_doc = True if action == "prof_doc_" else False
        status_msg = await bot.send_message(chat_id, "⏳ **جاري معالجة طلب الصورة الشخصية...**")
        asyncio.create_task(
            start_direct_execution(
                chat_id=chat_id,
                url=url,
        ...
        as_doc = True if action == "tt_doc_" else False
        status_msg = await bot.send_message(chat_id, "⏳ **جاري تنزيل وسائط منشور TikTok...**")
        asyncio.create_task(
            start_direct_execution(
                chat_id=chat_id,
                url=url,
        ...
        status_msg = await bot.send_message(chat_id, "⏳ **جاري معالجة طلب Dailymotion...**")
        asyncio.create_task(
        ...
        status_msg = await bot.send_message(chat_id, "⏳ **جاري تحضير وبدء تنزيل المحتوى...**")
        asyncio.create_task(
            start_direct_execution(
                chat_id=chat_id,
                url=url,
