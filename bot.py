...
# --- الإعدادات ---
VERSION = "v67.0-TikTok-MultiEngine-Fix"
...
def tiktok_multi_engine(url, task_dir):
    """محرك متعدد الطرق لاستخراج صور وسلاسل صور تيك توك"""
    # 1. المحاولة عبر gallery-dl
    try:
    ...
    # 2. المحاولة عبر Cobalt API الاحتياطية
    try:
...
def sanitize_and_fix_image(file_path):
    """إعادة تشفير الصور وتحويل WebP إلى JPG معيارية لتجنب خطأ UploadMediaRequest"""
    try:
        with Image.open(file_path) as img:
            base_name, _ = os.path.splitext(file_path)
            new_path = f"{base_name}_clean.jpg"
            img = img.convert("RGB")
        ...
        print(f"Image sanitize error on {file_path}: {e}")
        return file_path
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        # --- معالجة الامتدادات وإصلاح الصور لتوافق التليجرام ---
        downloaded_files = []
        for root, _, files in os.walk(task_dir):
            for file in files:
                fpath = os.path.join(root, file)
                if os.path.isfile(fpath) and os.path.getsize(fpath) > 500 and not file.endswith('_thumb.jpg') and not file.endswith('_clean.jpg'):
                    ...
                    # معالجة كافة الصور وإعادة إنتاجها بصيغة JPEG متوافقة تماماً
                    if current_ext in ['.jpg', '.jpeg', '.png', '.webp']:
                        fpath = await loop.run_in_executor(None, sanitize_and_fix_image, fpath)
...
def main():
    ...
    print("🤖 البوت يعمل بكفاءة وتم حل مشكلة صور تيك توك وتوافق الرفع على تليجرام!")
