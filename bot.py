...
VERSION = "v84.0-UltraUpload-FastTelethon"
...
# --- محرك الرفع الموازي السريع FastTelethon ---
async def fast_upload_file(client, file_path, progress_callback=None):
    file_size = os.path.getsize(file_path)
    if file_size == 0:
        raise Exception("الملف فارغ، تعذر الرفع.")

    part_size = 512 * 1024 if file_size > 100 * 1024 * 1024 else 256 * 1024
    part_count = math.ceil(file_size / part_size)
    is_big = file_size > 10 * 1024 * 1024
    file_id = int.from_bytes(os.urandom(8), 'big', signed=True)

    pool_size = 4 if file_size < 500 * 1024 * 1024 else 8
    sender_clients = [client]

    # إنشاء اتصالات إضافية مضاعفة للمساعدة في الرفع المتوازي
    try:
        for _ in range(pool_size - 1):
            s_client = TelegramClient(None, API_ID, API_HASH)
            await s_client.connect()
            s_client.session.auth_key = client.session.auth_key
            s_client.session.server_address = client.session.server_address
            s_client.session.port = client.session.port
            sender_clients.append(s_client)
    except Exception:
        sender_clients = [client]
    ...
    async def upload_part(part_index, sender):
        ...
        async with lock:
            ...
            if progress_callback:
                ...
                res = progress_callback(current_bytes, file_size)
    ...
    async def worker(sender):
        while not queue.empty():
            try:
                ...
                break
            await upload_part(part_idx, sender)
            queue.task_done()

    workers = [asyncio.create_task(worker(sender_clients[i % len(sender_clients)])) for i in range(len(sender_clients))]
    await asyncio.gather(*workers)

    # تنظيف الاتصالات الثانوية
    for sc in sender_clients[1:]:
        try: await sc.disconnect()
        except: pass

    filename = os.path.basename(file_path)
    if is_big:
        return InputFileBig(id=file_id, parts=part_count, name=filename)
    else:
        return InputFile(id=file_id, parts=part_count, name=filename, md5_checksum='')
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    try:
        ...
        await status_msg.edit("📤 **اكتمل التنزيل! جاري الرفع السريع إلى تيليجرام...**", buttons=cancel_btn)
        ...
        async def upload_progress_callback(current, total):
            ...
            if now - last_upload_update[0] >= 2.0 or current == total:
                ...
                text = (
                    f"⚡ **جاري الرفع السريع لـ Dailymotion إلى تلجرام...**\n\n"
                    f"[{bar}] {percentage:.1f}%\n"
                    f"🚀 **السرعة الخارقة:** {format_size(speed)}/s\n"
        ...
        # الرفع الموازي الخارق لـ Dailymotion
        uploaded_file = await fast_upload_file(bot, file_path, progress_callback=upload_progress_callback)
...
# --- محرك التنزيل السريع والمتعدد الخيوط للروابط المباشرة (Multi-Connection Fast Downloader) ---
        ...
        async def update_progress():
            while downloaded_bytes[0] < total_size or total_size == 0:
                ...
                if now - last_update_time[0] >= 1.5:
                    ...
                    text = (
                        f"🚀 **جاري التنزيل المباشر بأقصى سرعة...**\n"
                        f"[{bar}] {percent:.1f}%\n"
                        f"📦 الحجم: `{format_size(current_downloaded)}` / `{format_size(total_size)}`\n"
                        f"⚡ السرعة: `{format_size(speed)}/s`\n"
        ...
        try:
            ...
            if target_url and is_social:
                ...
                async def upload_progress_callback(current, total):
                    ...
                    now = time.time()
                    if now - last_upload_update > 1.5 or current == total:
                        ...
                        text = (
                            f"⚡ **جاري الرفع الخارق إلى تيليجرام (متعدد الأجزاء)...**\n"
                            f"[{bar}] {percent:.1f}%\n"
                            f"📦 الحجم: `{format_size(current)}` / `{format_size(total)}`\n"
                            f"🚀 السرعة الفائقة: `{format_size(speed)}/s`\n"
                            f"⏱️ الوقت المنقضي: `{format_time(elapsed)}`\n"
                ...
                await status_msg.edit("📤 **جاري تحضير محرك الرفع السريع...**", buttons=cancel_btn)
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    ...
                    # استخدام محرك FastTelethon لرفع الملفات الكبيرة والمتوسطة بسرعة فائقة
                    uploaded_file = await fast_upload_file(bot, vid_file, progress_callback=upload_progress_callback)
                    
                    if as_doc:
                        await bot.send_file(chat_id, uploaded_file, force_document=True, caption=part_caption)
                    else:
                        ext = os.path.splitext(vid_file)[1].lower()
                        if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                            duration, width, height, thumb_path = await get_video_metadata_and_thumb(vid_file)
                            attr = [DocumentAttributeVideo(duration=duration, w=width, h=height, supports_streaming=True)]
                            await bot.send_file(chat_id, uploaded_file, caption=part_caption, thumb=thumb_path, attributes=attr, supports_streaming=True)
                            ...
                            await bot.send_file(chat_id, uploaded_file, caption=part_caption)
                        else:
                            await bot.send_file(chat_id, uploaded_file, caption=part_caption, force_document=True)
            ...
            await status_msg.edit(f"📤 **جاري رفع المحتوى السريع ({len(downloaded_files)} عنصر)...**", buttons=cancel_btn)
                ...
                if len(photos) > 1 and not as_doc:
                    try:
                        ...
                        for idx, p in enumerate(photos, start=1):
                            uploaded_img = await fast_upload_file(bot, p)
                            await bot.send_file(chat_id, uploaded_img, caption=f"📸 **صورة ({idx}/{len(photos)}):**")
                else:
                    for idx, p in enumerate(photos, start=1):
                        try:
                            ...
                            uploaded_img = await fast_upload_file(bot, p)
                            if as_doc:
                                await bot.send_file(chat_id, uploaded_img, caption=f"📄 **مستند صورة ({idx}/{len(photos)}):**", force_document=True)
                            else:
                                cap = f"📸 **صورة ({idx}/{len(photos)}):**" if len(photos) > 1 else "📸 **تم تنزيل الصورة بنجاح!**"
                                await bot.send_file(chat_id, uploaded_img, caption=cap, force_document=False)
                        except Exception:
                            uploaded_img = await fast_upload_file(bot, p)
                            await bot.send_file(chat_id, uploaded_img, force_document=True)
            ...
            for vid in videos:
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    ...
                    uploaded_v = await fast_upload_file(bot, part_vid)
            ...
            for aud in audio_files:
                if cancel_event.is_set(): raise Exception("CANCELLED")
                uploaded_a = await fast_upload_file(bot, aud)
                await bot.send_file(chat_id, uploaded_a, caption=f"🎵 **تم استخراج الملف الصوتي بأعلى خامة (320kbps):**\n📁 `{os.path.basename(aud)}`")

            for oth in other_files:
                if cancel_event.is_set(): raise Exception("CANCELLED")
                uploaded_o = await fast_upload_file(bot, oth)
                await bot.send_file(chat_id, uploaded_o, caption=f"📄 `{os.path.basename(oth)}`", force_document=True)
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🚀 **تسريع الرفع إلى تيليجرام (FastTelethon):** دمج محرك الرفع الموازي عبر تقسيم الملف وفتحه على 8 خيوط لسرعة تصل إلى 30+ MB/s.\n"
            "⚡ **محرّك التحميل المباشر الخارق:** تسريع روابط التنزيل المباشر عبر اتصالات متعددة الخيوط (Multi-Part Async Requests).\n"
