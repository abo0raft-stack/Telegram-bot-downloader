...
async def progress_callback(current, total, status_msg, action_text, start_time, cancel_event):
    ...
    # تحديث الرسالة كل ثانيتين لتجنب حظر التليجرام (FloodWait)
    if msg_id in LAST_UPDATE_TIME and (now - LAST_UPDATE_TIME[msg_id]) < 2.0 and current != total:
        return
        ...
        for chunk in response.iter_content(chunk_size=1024*1024):
            ...
            if chunk:
                ...
                asyncio.run_coroutine_threadsafe(
                    progress_callback(downloaded, total_size, status_msg, "📥 **جاري التحميل المباشر...**", start_time, cancel_event),
                    loop
                )
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, fmt='mp4'):
    ...
    try:
        # --- 1. التحميل من تليجرام مع شريط التقدم ---
        if media_msg:
            start_dl_time = time.time()
            
            def dl_progress(current, total):
                asyncio.run_coroutine_threadsafe(
                    progress_callback(current, total, status_msg, "📥 **جاري تنزيل الملف من تليجرام...**", start_dl_time, cancel_event),
                    loop
                )

            await media_msg.download_media(file=file_path, progress_callback=dl_progress)
            
        # --- 2. التحميل من الرابط الخارجي ---
        elif url:
            ...
            if is_m3u8 or not is_direct_mp4:
                await status_msg.edit("📡 **جاري معالجة التحميل...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
        ...
        # --- 3. تحويل أو تعديل الصيغة ---
        if fmt in ['mp3', 'mkv'] or quality != 'best':
            await status_msg.edit(f"⚙️ **جاري تحويل الملف إلى صيغة {fmt.upper()}...**")
        ...
        # --- 4. الرفع إلى تليجرام مع شريط التقدم ---
        ...
        if fmt == 'mp3':
            ...
            await bot.send_file(chat_id, final_file, force_document=as_doc, progress_callback=ul_progress)
