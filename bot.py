...
# --- محرك التنزيل السلس المانع للتجمد ---
        ...
        with open(filename, 'wb') as f:
            for chunk in r.iter_content(chunk_size=1024 * 1024): # 1MB Chunk
                if chunk:
                    ...
                    # التحديث كل 4 ثوانٍ لمنع التجمد
...
# --- إدارة الرفع السلسة بفاصل زمني آمن ---
class AntiFreezeUploader:
    ...
    def callback(self, current, total):
        now = time.time()
        # التحديث كل 4.5 ثانية آمن جداً ويمنع التجمد والتعليق
        if now - self.last_update > 4.5 or current == total:
            ...
            text = f"📤 **جاري الرفع لـ Telegram ({VERSION})...**\n\n"
...
async def start_handler(event):
    await event.respond(f"🚀 **بوت التنزيل والرفع المباشر {VERSION} يعمل بكفاءة وسلاسة!**\nأرسل رابط الملف للبدء.")

@bot.on(events.NewMessage(pattern=r"^https?://"))
async def handle_url(event):
    url = event.text.strip()
    status_msg = await event.respond("⏳ **جاري الاتصال بالسيرفر لبدء التحميل...**")
    ...
    try:
        ...
        if os.path.exists(filename) and os.path.getsize(filename) > 0:
            ...
            # 2. إنشاء العداد المانع للتجمد
            uploader = AntiFreezeUploader(filename, status_msg, loop)

            # 3. الرفع مع التحديث السلس
            await bot.send_file(
                event.chat_id,
                filename,
