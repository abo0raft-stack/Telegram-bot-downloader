...
# --- الإعدادات ---
VERSION = "v56.0-Instagram-Media-Info-Fix"
...
def clean_url(url):
    """إزالة جميع المعاملات الإضافية من رابط إنستغرام لضمان تحليله بشكل صحيح"""
    parsed = urlparse(url)
...
def get_video_metadata_and_thumb(file_path):
    """جلب أبعاد الفيديو والمدة والصورة المصغرة بدقة 100% باستخدام ffprobe و ffmpeg"""
    duration, width, height = 0, 1280, 720
    thumb_path = f"{file_path}_thumb.jpg"
    
    try:
        # 1. استخراج التفاصيل عبر ffprobe
        cmd = [
            'ffprobe', '-v', 'error',
        ...
        if 'streams' in data and len(data['streams']) > 0:
            for stream in data['streams']:
                if 'width' in stream and 'height' in stream:
                    ...
                    # تعديل الأبعاد إذا كان الفيديو مقلوباً/رأسياً بناءً على تدوير الفيديو (Rotation tag)
        ...
        # 2. توليد صورة مصغرة ممتازة في الثانية الأولى من الفيديو
        subprocess.run([
...
def instagram_api_fallback(url, task_dir):
    """محرك بديل يفك حظر إنستغرام للصور والفيديوهات والألبومات"""
    try:
        ...
        print(f"Cobalt API Fallback Failed: {e}")
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4'):
    ...
    try:
        ...
        # 2. إرسال الفيديوهات بأبعاد وصورة مصغرة صحيحة
        for vid in videos:
            ...
            # تنظيف ملف الصورة المصغرة بعد الإرسال
...
def main():
    ...
    print("🤖 البوت يعمل بكفاءة مع استخراج الأبعاد والصورة المصغرة للفيديوهات!")
