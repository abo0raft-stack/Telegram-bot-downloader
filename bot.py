...
# --- الإعدادات ---
VERSION = "v48.2-TimestampScreenshots"
...
def add_timestamp_to_image(image_path, time_str, text_color="white"):
    """
    إضافة التوقيت أعلى يمين الشاشة بحجم خط واضح ورؤية ممتازة بالعين
    text_color: "white" ليكون النص أبيض بخلفية سوداء، أو "black" ليكون النص أسود بخلفية بيضاء
    """
    try:
        with Image.open(image_path) as img:
            img = img.convert("RGBA")
            
            # حساب حجم الخط ديناميكياً ليناسب دقة الصورة ويكون واضحاً (حوالي 7% من ارتفاع الصورة)
            font_size = max(24, int(img.height * 0.07))
            ...
            # إنشاء طبقة مرسومة جديدة للتعامل مع الشفافية والألوان
            ...
            # تحديد الموقع في **أعلى يمين الشاشة** مع هوامش مريحة
            ...
            # اختيار الألوان حسب المطلوب
            if text_color == "black":
                fill_color = (0, 0, 0, 255)         # نص أسود
                box_color = (255, 255, 255, 220)    # خلفية بيضاء وواضحة
            else:
                fill_color = (255, 255, 255, 255)   # نص أبيض
                box_color = (0, 0, 0, 200)          # خلفية سوداء داكنة

            # رسم مربع الخلفية المتباين خلف النص لضمان وضوحه
            draw.rectangle(rect_box, fill=box_color)

            # رسم نص التوقيت
            draw.text((x, y), time_str, fill=fill_color, font=font)

            # دمج الطبقة وحفظ الصورة
...
def generate_grid_screenshots(file_path, duration):
    ...
    for i in range(1, 10):
        ...
        if os.path.exists(ss_path) and os.path.getsize(ss_path) > 0:
            # إضافة النص باللون الأبيض (يمكنك تغييره إلى "black" إن أردت نصاً أسود بخلفية بيضاء)
            add_timestamp_to_image(ss_path, time_str, text_color="white")
...
async def start_handler(event):
    welcome_text = (
        ...
        "3️⃣ أرسل الأمر /settings للتحكم بخصائص وميزات البوت."
    )
...
async def settings_handler(event):
    ...
    await event.respond("⚙️ **لوحة تحكم وإعدادات البوت:**\nانقر على أي ميزة لتفعيلها أو إيقافها:", buttons=buttons)
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, fmt='mp4'):
    ...
    try:
        ...
        if user_settings.get('send_screenshots') and fmt not in ['mp3', 'audio_mp3']:
            screenshots = generate_grid_screenshots(final_path, duration)
        ...
        if screenshots:
            await bot.send_file(chat_id, screenshots, caption="📸 **لقطات شاشة مع التوقيت المالي:**")
