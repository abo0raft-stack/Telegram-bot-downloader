...
VERSION = "v94.0-Memory-and-Server-Optimization"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🧠 **تحسين تفريغ الذاكرة والحجم الحقيقي (v94):** تفعيل محرك `malloc_trim` قسراً عبر مكتبة `libc` لتفريغ الذاكرة المحجوزة لنظام التشغيل، مع تخصيص أمر `/df` لعرض أحجام المجلدات المؤقتة واستهلاك الذاكرة الحقيقي للبوت بدقة.\n"
            "🛠️ **إدارة السيرفر والتنظيف الحقيقي (v93):** تحديث أمر `/df` لعرض نسبة استهلاك التخزين الحالية بدقة، مع تطوير أمر `/clean` ليقوم بحذف فعلي لكافة الملفات المؤقتة وبقايا التنزيل في السيرفر وتفريغ الذاكرة.\n"
            "🌐 **ترجمة المنشورات إلى العربية (v92):** ترجمة نص المنشور الأجنبي تلقائياً وإلحاقه مع الوسائط المحملة من جميع منصات التواصل الاجتماعي.\n"
