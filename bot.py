...
# هيدرات متصفح فائقة الدقة لتبدو مثل TDM / Chrome
...
def download_with_ytdlp_robust(url, filepath, status_msg, loop, cancel_event):
    ...
    def progress_hook(d):
        ...
        if d['status'] == 'downloading':
            ...
            text = (
                f"📥 **جاري التحميل بالمحرك المتقدم...**\n"
                f"📊 النسبة: `{p}`\n"
                f"💾 الحجم: `{curr_mb:.1f}MB / {total_mb:.1f}MB`\n"
                f"🚀 السرعة: `{s}`\n"
                f"⏳ المتبقي: `{eta}`"
...
async def start_handler(event):
    welcome_text = (
        ...
        "✨ **تم تحديث المحرك لتجاوز قيود سيرفرات CDN المعقدة!**"
    )
...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        await status_msg.edit("📥 **بدء جلب الفيديو من السيرفر...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{status_msg.id}")])
        
        # الاعتماد الكامل على المحرك المحسّن
        await loop.run_in_executor(None, download_with_ytdlp_robust, url, file_path, status_msg, loop, cancel_event)
        ...
        if os.path.exists(file_path):
            ...
            if file_size < 100 * 1024: 
                raise Exception("السيرفر رفض إرسال الفئة المطلوبة أو إجابة الصفحة محجوبة.")

            if quality in ['1080', '720', '480']:
                await status_msg.edit(f"⚙️ **جاري تعديل الجودة والضغط إلى {quality}p...**")
