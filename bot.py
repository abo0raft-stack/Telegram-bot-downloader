...
async def start_handler(event):
    welcome_text = (
        ...
        "• دعم التحميل الشامل: X/Twitter, Instagram, TikTok, Facebook, Reddit, Pinterest, Threads, Snapchat, VK, Vimeo, XNXX.\n"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "1️⃣ **إضافة منصات جديدة:** دعم شامل للتنزيل من Reddit, Pinterest, Threads, Snapchat, VK, Vimeo, و XNXX.\n"
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        if is_x_url(clean_u) or is_xnxx_url(clean_u):
            platform_name = "XNXX" if is_xnxx_url(clean_u) else "X"
            ...
            await event.respond(f"🎬 **اختر جودة الفيديو المطلوبة لمنصة {platform_name}:**", buttons=buttons)
