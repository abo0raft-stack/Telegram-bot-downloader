...
VERSION = "v88.0-Facebook-Fix-Quality-Selection"
...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best'):
    ...
    # إجبار التنزيل كفيديو ومنع التنسيقات غير المرغوبة (مثل الصور المتحركة GIF)
...
# --- v87/v88 التنزيل المباشر الذكي مع دعم الاستئناف والبث عبر HTTP Range & yt-dlp ---
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    path_lower = urlparse(url).path.lower()
    
    # التعامل مع مقاطع البث والروابط المبتكرة (.m3u8, .mpd) عبر yt-dlp للتجزئة السلسة
    ...
    # التنزيل المباشر القياسي مع دعم HTTP Range Requests للتحميل المقطوع
    headers = BROWSER_HEADERS.copy()
    downloaded = 0
...
async def start_handler(event):
    if not await is_allowed(event):
        ...
        return
    welcome_text = (
        "🤖 **أهلاً بك في بوت التنزيل المتقدم (v88)!**\n\n"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "📘 **إصلاح فيديوهات Facebook (v88):** حل مشكلة وصول فيديوهات الفيسبوك كـ GIF أو صورة متحركة عبر تحويل وتصفية صيغ الفيديو بترميز MP4.\n"
            "🎛️ **خيارات جودة Facebook (v88):** إضافة أزرار انسيابية لاختيار جودة التنزيل عند إرسال روابط الفيسبوك (1080p / 720p / 480p / MP3).\n"
