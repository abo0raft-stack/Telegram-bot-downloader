...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    ydl_opts = {
        'format': fmt, 'outtmpl': os.path.join(task_dir, '%(title).30s_%(id)s.%(ext)s'),
        'progress_hooks': [progress_hook], 'quiet': True, 'no_warnings': True,
        'headers': headers, 'nocheckcertificate': True, 'geo_bypass': True, 'merge_output_format': 'mp4'
    }
    ...
    try:
        ...
        await bot.send_file(chat_id, file_path, caption=caption_out, thumb=thumb_path, attributes=attr, supports_streaming=True, progress_callback=upload_progress_callback)
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **تثبيت أداء Async وثغرات التنزيل (v96.1):** حل مشاكل التثبيت ومنع أخطاء Event Loop المتداخلة أوجهاً أثناء تنزيل فيديوهات HLS و Dailymotion.\n"
            "✨ **تنظيف شرح فيديوهات التواصل (v96):** إلغاء كتابة اسم الملف ورأس الجملة وإظهار نص المنشور المجلوب والترجمة العربية مباشرة أسفل الفيديو.\n"
            "🧠 **تحسين تفريغ الذاكرة والحجم الحقيقي (v94):** تفعيل محرك `malloc_trim` قسراً عبر مكتبة `libc` لتفريغ الذاكرة المحجوزة.\n\n"
