...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        target_url = clean_url(url) if url and is_complex_url(url) else url
...
# --- معالجة الأحداث والأوامر (الأجزاء المضافة للربط) ---

@bot.on(events.NewMessage(pattern=r"^/start"))
async def start_handler(event):
    welcome_text = (
        "👋 **أهلاً بك في بوت التحميل المباشر!**\n\n"
        "أرسل أي رابط فيديو أو صور (TikTok, Instagram, YouTube, X, إلخ) أو رابط مباشر لبدء التنزيل فوراً."
    )
...
async def message_handler(event):
    ...
    if url_match:
        ...
        status_msg = await event.respond("⏳ **جاري بدء عملية التحميل...**")
        asyncio.create_task(start_direct_execution(
        ...
        status_msg = await event.respond("⏳ **جاري معالجة الملف...**")
        asyncio.create_task(start_direct_execution(
            chat_id=event.chat_id,
            url=None,
            filename=filename,
