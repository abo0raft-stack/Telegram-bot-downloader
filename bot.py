...
VERSION = "v97.0-TikTok-Engine-Fix"
...
# --- محرك TikTok المطور والمصلح (v97) ---
async def tiktok_multi_engine(url, task_dir):
    """محرك هجين ومتكامل لتنزيل صور وفيديوهات تيكتوك مع الوصف"""
    # 1. محاولة التنزيل من خلال gallery-dl أولاً
    ...
    # 2. المحرك الاحتياطي المطور: TikWM API
    try:
        async with aiohttp.ClientSession() as session:
            headers = {'User-Agent': BROWSER_HEADERS['User-Agent']}
            async with session.post("https://www.tikwm.com/api/", data={"url": url, "hd": 1}, headers=headers, timeout=20) as res:
                if res.status == 200:
                    res_json = await res.json()
                    data = res_json.get("data", {})
                    
                    # حفظ شرح/عنوان المنشور
                    ...
                    # التثبت إذا كانت مجموعة صور (Photos/Slideshow)
    ...
    # 3. المحرك الاحتياطي الأخير: Cobalt API
    try:
        api_url = "https://api.cobalt.tools/api/json"
        headers = {
            "Accept": "application/json",
