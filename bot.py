...
def update_and_install_libraries():
    try:
        import subprocess
        subprocess.run(["pip", "install", "-U", "yt-dlp", "gallery-dl", "Pillow", "aiohttp", "aiosqlite", "telethon", "requests", "psutil", "beautifulsoup4", "googletrans==4.0.0-rc1"], check=True)
...
from telethon.tl.types import DocumentAttributeVideo, BotCommand
...
async def translate_to_arabic(text: str) -> str:
    """ترجمة النص إلى العربية إذا كان بلغة أجنبية"""
    if not text or not text.strip():
        return ""
    try:
        loop = asyncio.get_event_loop()
        detection = await loop.run_in_executor(None, lambda: translator.detect(text))
        if detection and detection.lang != 'ar':
            translation = await loop.run_in_executor(None, lambda: translator.translate(text, dest='ar'))
...
VERSION = "v96.4-MergeFix-Optimized"
...
def clean_download_folder():
    ...
    return cleaned_bytes, files_deleted
...
async def fix_video_metadata(input_path):
    """إعادة ترتيب الفهرس (moov atom) وإصلاح الميتا داتا دون إعادة الترميز الكلي للسرعة"""
    if not os.path.exists(input_path):
        return input_path
    base, ext = os.path.splitext(input_path)
    fixed_path = f"{base}_fixed.mp4"
...
async def convert_to_mp4(input_path):
    """تحويل الفيديو ومزامنة ترميز الصوت والصورة مع تسكين الفهرس (moov atom) في بداية الملف"""
    base, ext = os.path.splitext(input_path)
    ...
    return await fix_video_metadata(input_path)
...
async def split_video_file(filepath, max_size_mb=1950):
    """تقسيم الفيديو إذا كان الحجم أكبر من 2 جيجابايت مع الحفاظ على الفهرس والـ faststart لكل جزء"""
    ...
    for i in range(parts_count):
        ...
        cmd = [
            ...
            '-i', filepath, '-c', 'copy', '-movflags', '+faststart', out_part
        ]
        proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
        await proc.wait()
...
async def trim_video_clip(filepath, start_time, end_time):
    ...
    cmd = [
        ...
        '-i', filepath, '-c', 'copy', '-movflags', '+faststart', out_trimmed
    ]
    proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
    await proc.wait()
...
async def download_profile_avatar(url, task_dir):
    try:
                ...
                async with session.get(url, headers=custom_headers, timeout=20) as r:
                    if r.status == 200:
                        ...
                        if og_img and og_img.get("content"):
                            img_url = unquote(og_img["content"]).replace('&amp;', '&')
                            async with session.get(img_url, headers=custom_headers, timeout=20) as img_res:
                                if img_res.status == 200:
                                    ...
                                    with open(filepath, 'wb') as f: f.write(await img_res.read())
                                    if os.path.exists(filepath) and os.path.getsize(filepath) > 1000: return True

        cmd = [
            "gallery-dl", "--directory", task_dir, "--filename", "profile_avatar.{ext}",
            "--option", "extractor.instagram.include=avatar", "--option", "extractor.twitter.include=avatar",
            "--user-agent", BROWSER_HEADERS['User-Agent']
        ]
        if is_instagram_url(url) and os.path.exists(INSTAGRAM_COOKIES_FILE): cmd.extend(["--cookies", INSTAGRAM_COOKIES_FILE])
        elif is_x_url(url) and os.path.exists(X_COOKIES_FILE): cmd.extend(["--cookies", X_COOKIES_FILE])
        ...
        ydl_opts = {
            ...
            'skip_download': True, 'writethumbnails': True,
            'quiet': True, 'no_warnings': True, 'headers': BROWSER_HEADERS
        }
        if is_instagram_url(url) and os.path.exists(INSTAGRAM_COOKIES_FILE): ydl_opts['cookiefile'] = INSTAGRAM_COOKIES_FILE
        elif is_x_url(url) and os.path.exists(X_COOKIES_FILE): ydl_opts['cookiefile'] = X_COOKIES_FILE

        await loop.run_in_executor(None, lambda: yt_dlp.YoutubeDL(ydl_opts).extract_info(url, download=True))
        if any(os.path.isfile(os.path.join(task_dir, f)) and os.path.getsize(os.path.join(task_dir, f)) > 1000 for f in os.listdir(task_dir)): return True

    except Exception as e: print(f"Profile Avatar Engine Error: {e}")
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    """تنزيل وتحميل ديليموشن مع دعم التقسيم لملفات > 2GB وتسكين الفهرس (moov atom) ومنع تعليق 99.3%"""
    chat_id = event.chat_id
    ...
    if not status_msg: status_msg = await event.respond("⏳ **جاري بدء عملية التحميل من Dailymotion...**", buttons=cancel_btn)
    else: await status_msg.edit("⏳ **جاري بدء عملية التحميل من Dailymotion...**", buttons=cancel_btn)
    ...
    def progress_hook(d):
        ...
        if d['status'] == 'downloading':
            ...
            try:
                asyncio.run_coroutine_threadsafe(status_msg.edit("⚙️ **جاري تجميع ودمج الصوت والصورة بنجاح...**", buttons=cancel_btn), loop)
            except Exception: pass
    ...
    if quality_choice == 'mp3':
        ...
        fmt = f"bestvideo[height<={quality_choice}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<={quality_choice}]+bestaudio/best[height<={quality_choice}]/best"
    ...
    try:
        ...
        def run_dl():
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if info: post_caption_text = info.get('description') or info.get('title') or ""
        ...
        await status_msg.edit("✂️ **جاري فحص حجم الفيديو والتحقق من الحاجة للتقسيم...**", buttons=cancel_btn)
        split_files = await split_video_file(file_path, max_size_mb=1950)
        ...
        for idx, part_file in enumerate(split_files, start=1):
            ...
            part_file = await fix_video_metadata(part_file)

            start_time = time.time()
            last_upload_update = [0]
            part_suffix = f" (Part {idx}/{len(split_files)})" if len(split_files) > 1 else ""

            async def upload_progress_callback(current, total):
                if cancel_event.is_set(): raise Exception("CANCELLED")
                now = time.time()
                if now - last_upload_update[0] >= 3.0 or current == total:
                    ...
                    text = (
                        f"📤 **جاري رفع الفيديو إلى تلجرام{part_suffix}...**\n\n"
                        f"[{bar}] {percentage:.1f}%\n"
                        f"🚀 **السرعة:** {format_size(speed)}/s\n"
            ...
            final_caption = f"{caption_out}\n\n🎬 `{os.path.basename(part_file)}`{part_suffix}" if caption_out else f"🎬 `{os.path.basename(part_file)}`{part_suffix}"

            await bot.send_file(chat_id, part_file, caption=final_caption, thumb=thumb_path, attributes=attr, supports_streaming=True, progress_callback=upload_progress_callback)
            ...
            if duration > 0 and user_config.get("snapshots", True) and idx == 1:
                ...
                frames = await extract_9_frames(part_file, duration, chat_id=chat_id)
                if frames:
        ...
        else: await status_msg.edit(f"❌ **حدث خطأ أثناء العملية:**\n`{str(e)}`", buttons=None)
    finally:
...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best'):
    ...
    ydl_opts = {
        ...
        'headers': BROWSER_HEADERS, 'nocheckcertificate': True, 'ignoreerrors': True,
        'writethumbnails': True, 'allow_playlist_files': True, 'geo_bypass': True,
        'age_limit': 99, 'concurrent_fragment_downloads': 10,
        'postprocessor_args': {'ffmpeg': ['-movflags', '+faststart']}
    }
    if is_x_url(url) and os.path.exists(X_COOKIES_FILE): ydl_opts['cookiefile'] = X_COOKIES_FILE
    elif is_instagram_url(url) and os.path.exists(INSTAGRAM_COOKIES_FILE): ydl_opts['cookiefile'] = INSTAGRAM_COOKIES_FILE

    if is_facebook_url(url): ydl_opts['postprocessors'] = [{'key': 'FFmpegVideoConvertor', 'preferedformat': 'mp4'}]
    ...
    if fmt in ['mp3', 'audio_mp3']:
        ...
        if is_facebook_url(url):
            if quality != 'best': ydl_opts['format'] = f"(bestvideo[height<={quality}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<={quality}]+bestaudio/best[height<={quality}])/best[ext=mp4]/best"
            else: ydl_opts['format'] = "(bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best)/best[ext=mp4]/best"
        else:
            if quality != 'best': ydl_opts['format'] = f"bestvideo[height<={quality}][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<={quality}]+bestaudio/best[height<={quality}]/best"
            else: ydl_opts['format'] = 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio/best'
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if info:
                ...
                else: caption_text = info.get('description') or info.get('title') or ""
                
                if caption_text:
                    desc_file = os.path.join(task_dir, "post_description.txt")
                    with open(desc_file, "w", encoding="utf-8") as f: f.write(caption_text.strip())
    except Exception as e: print(f"yt-dlp error: {e}")

async def gallery_dl_fallback_engine(url, task_dir, prefix="media"):
    try:
        cmd = ["gallery-dl", "--directory", task_dir, "--filename", f"{prefix}_{{id}}_{{num}}.{{ext}}", "--user-agent", BROWSER_HEADERS['User-Agent'], url]
        proc = await asyncio.create_subprocess_exec(*cmd, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
        await asyncio.wait_for(proc.wait(), timeout=45)
    except Exception as e: print(f"gallery-dl error ({prefix}): {e}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(api_url, json={"url": url}, headers=headers, timeout=15) as res:
                if res.status == 200:
                    ...
                    urls_to_dl = []
                    if status in ["stream", "redirect"]: urls_to_dl.append(data.get("url"))
                    elif status == "picker":
                        for item in data.get("picker", []): urls_to_dl.append(item.get("url"))
                    for idx, media_url in enumerate(urls_to_dl, start=1):
                        ...
                        async with session.get(media_url, headers=BROWSER_HEADERS, timeout=30) as r:
                            if r.status == 200:
                                ...
                                with open(filepath, 'wb') as f: f.write(await r.read())
    except Exception as e: print(f"TikTok Cobalt API error: {e}")

async def instagram_carousel_and_photo_engine(url, task_dir):
    try:
        cmd = ["gallery-dl", "--directory", task_dir, "--filename", "ig_{id}_{num}.{ext}", "--option", "extractor.instagram.post-quality=max", "--option", "extractor.instagram.include=posts", "--user-agent", BROWSER_HEADERS['User-Agent']]
        ...
        loop = asyncio.get_event_loop()
        ydl_opts = {'outtmpl': os.path.join(task_dir, 'ig_hd_%(id)s_%(autonumber)s.%(ext)s'), 'quiet': True, 'no_warnings': True, 'headers': BROWSER_HEADERS, 'writethumbnails': True, 'format': 'best', 'merge_output_format': 'mp4', 'postprocessor_args': {'ffmpeg': ['-movflags', '+faststart']}}
        ...
        def run_ig_ydl():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if info:
                    ...
                    if desc:
                        with open(os.path.join(task_dir, "post_description.txt"), "w", encoding="utf-8") as f: f.write(desc.strip())

        await loop.run_in_executor(None, run_ig_ydl)
        if any(os.path.isfile(os.path.join(task_dir, f)) for f in os.listdir(task_dir)): return

    except Exception as e: print(f"Instagram engine error: {e}")
...
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    ...
    if any(path_lower.endswith(ext) for ext in ['.m3u8', '.mpd']):
        ...
        ydl_opts = {
            'outtmpl': filepath, 'quiet': True, 'no_warnings': True,
            'headers': BROWSER_HEADERS, 'nocheckcertificate': True,
            'progress_hooks': [progress_hook], 'concurrent_fragment_downloads': 10,
            'postprocessor_args': {'ffmpeg': ['-movflags', '+faststart']}
        }
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.get(url, ssl=False, allow_redirects=True) as response:
            ...
            if not is_resumed:
                ...
                if content_range and '/' in content_range: total_size = int(content_range.split('/')[-1])
                else: total_size = downloaded + int(response.headers.get('content-length', 0))
            ...
            with open(filepath, mode) as f:
                async for chunk in response.content.iter_chunked(chunk_size):
                    ...
                    if chunk:
                        ...
                        if current_time - last_update_time > 3.0 or downloaded == total_size:
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if is_avatar_task:
                ...
                if not success: raise Exception("لم نتمكن من جلب الصورة الشخصية لهذا الحساب.")
                ...
                if not downloaded:
                    if is_instagram_url(target_url): await instagram_carousel_and_photo_engine(target_url, task_dir)
                    elif is_tiktok_url(target_url): await tiktok_multi_engine(target_url, task_dir)
                    elif is_dailymotion_url(target_url): await gallery_dl_fallback_engine(target_url, task_dir, "dm")
                    elif is_pinterest_url(target_url): await gallery_dl_fallback_engine(target_url, task_dir, "pin")
                    elif is_reddit_url(target_url): await gallery_dl_fallback_engine(target_url, task_dir, "reddit")
                ...
                async def upload_progress_callback(current, total):
                    ...
                    now = time.time()
                    if now - last_upload_update > 3.0 or current == total:
                ...
                await status_msg.edit("📤 **جاري تجهيز الرفع ومعالجة الفهرس المترابط...**", buttons=cancel_btn)
                if target_fmt == 'mp3':
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    vid_file = await fix_video_metadata(vid_file)
                    ...
                    if as_doc: await bot.send_file(chat_id, vid_file, force_document=True, caption=caption_text, progress_callback=upload_progress_callback)
                    else:
                        ext = os.path.splitext(vid_file)[1].lower()
                        if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                            duration, width, height, thumb_path = await get_video_metadata_and_thumb(vid_file)
                            attr = [DocumentAttributeVideo(duration=duration, w=width, h=height, supports_streaming=True)]
                            await bot.send_file(chat_id, vid_file, caption=caption_text, thumb=thumb_path, attributes=attr, supports_streaming=True, progress_callback=upload_progress_callback)
                        ...
                        elif ext in ['.mp3', '.wav', '.m4a', '.aac']: await bot.send_file(chat_id, vid_file, caption=caption_text, progress_callback=upload_progress_callback)
                        else: await bot.send_file(chat_id, vid_file, caption=caption_text, force_document=True, progress_callback=upload_progress_callback)
            ...
            if os.path.exists(desc_file_path):
                try:
                    with open(desc_file_path, "r", encoding="utf-8") as df:
                        ...
                        if post_desc_text: translated_desc_text = await translate_to_arabic(post_desc_text)
                except Exception: pass
            ...
            for fpath in sorted(downloaded_files):
                ...
                if ext in image_extensions: photos.append(fpath)
                elif ext in video_extensions: videos.append(fpath)
                elif ext in audio_extensions: audio_files.append(fpath)
            ...
            if post_desc_text:
                ...
                if len(clean_desc) > 800: clean_desc = clean_desc[:800] + "..."
                post_caption_formatted = f"📝 **نص المنشور:**\n{clean_desc}"
                
                if translated_desc_text:
                    clean_trans = translated_desc_text.strip()
                    if len(clean_trans) > 800: clean_trans = clean_trans[:800] + "..."
                ...
                if len(photos) > 1 and not as_doc:
                    try:
                        for chunk_idx in range(0, len(photos), 10):
                            ...
                            if chunk_idx == 0 and post_caption_formatted: album_cap += f"\n\n{post_caption_formatted}"
                            await bot.send_file(chat_id, chunk_photos, caption=album_cap, album=True)
                    except Exception as album_err:
                        print(f"Album send failed: {album_err}")
                        for idx, p in enumerate(photos, start=1):
                            pname = os.path.basename(p)
                            cap = f"📸 **صورة ({idx}/{len(photos)}):**\n📁 `{pname}`"
                            if idx == 1 and post_caption_formatted: cap += f"\n\n{post_caption_formatted}"
                    ...
                    for idx, p in enumerate(photos, start=1):
                        ...
                        except Exception: await bot.send_file(chat_id, p, force_document=True)

            for vid_idx, vid in enumerate(videos, start=1):
                if cancel_event.is_set(): raise Exception("CANCELLED")
                vid = await convert_to_mp4(vid)
                if trim_times: vid = await trim_video_clip(vid, trim_times[0], trim_times[1])

                split_vids = await split_video_file(vid)

                for idx, part_vid in enumerate(split_vids, start=1):
                    part_vid = await fix_video_metadata(part_vid)
                    ...
                    if is_social:
                        video_caption = post_caption_formatted if (vid_idx == 1 and idx == 1) else ""
                    else:
                        video_caption = f"🎥 **تم تنزيل الفيديو بنجاح!**{part_caption}\n📁 `{vname}`"
                        if vid_idx == 1 and idx == 1 and post_caption_formatted:
                            video_caption += f"\n\n{post_caption_formatted}"

                    await bot.send_file(chat_id, part_vid, caption=video_caption, force_document=as_doc, thumb=thumb_path, attributes=attr, supports_streaming=True)
            ...
            for aud in audio_files:
                ...
                if post_caption_formatted: aud_cap += f"\n\n{post_caption_formatted}"
...
async def clean_handler(event):
    ...
    freed_bytes, files_deleted = clean_download_folder()
    mb_freed = freed_bytes / (1024 * 1024)
    await event.respond(
        f"🧹 **تم التنظيف بنجاح!**\n\n"
        f"• الملفات المحذوفة: `{files_deleted}`\n"
        f"• المساحة المحررة: `{mb_freed:.2f} MB`\n"
        f"• تم إجبار الذاكرة (RAM) على التفريغ."
    )

@bot.on(events.NewMessage(pattern=r"^/df$"))
async def disk_status_handler(event):
    if not await is_allowed(event): return
    downloads_size = get_dir_size('./downloads') / (1024 * 1024)
    tmp_size = get_dir_size('./tmp') / (1024 * 1024)
    process = psutil.Process(os.getpid())
    ram_usage_mb = process.memory_info().rss / (1024 * 1024)

    await event.respond(
        f"📊 **حالة تخزين البوت الحالية:**\n\n"
        f"📁 مجلد Downloads: `{downloads_size:.2f} MB`\n"
        f"📁 مجلد Tmp: `{tmp_size:.2f} MB`\n"
        f"🧠 استهلاك الذاكرة الحقيقي (RAM): `{ram_usage_mb:.2f} MB`"
    )
...
async def settings_callback_handler(event):
    ...
    try: await event.edit(msg, buttons=new_buttons)
...
async def start_handler(event):
    ...
    welcome_text = (
        ...
        "• `/df` - فحص أحجام مجلدات downloads و tmp واستهلاك RAM البوت.\n"
        "• `/clean` - التنظيف الشامل للمجلدات وحذف الملفات وتفريغ الذاكرة قسراً.\n"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **معالجة تعليق 99.3%:** تحسين عملية دمج الفيديوهات بتقليل الضغط على واجهة تلجرام وتسريع FFmpeg.\n"
            "✨ **معالجة الميتا داتا و Faststart الكاملة:** إصلاح ظهور طول مقاطع الفيديو وشريط الوقت بنسبة 100% بنقل الفهرس (moov atom) لأول الملف.\n"
            "✨ **دعم الملفات الكبيرة (> 2GB):** تقسيم تلقائي للمقاطع المرتفعة الحجم لرفعها على تلجرام بنجاح دون أخطاء.\n"
            "✨ **تكامل Dailymotion:** استقرار كامل لرفع التنزيلات الطويلة وعالية الجودة من ديليموشن مع خيار التقسيم والترجمة.\n\n"
...
async def handle_user_input_and_urls(event):
    ...
    if event.text and event.text.startswith("/trim"): return
    ...
    for u in urls:
        if u.startswith('/video/'): u = f"https://www.dailymotion.com{u}"
...
async def set_bot_commands():
    try:
        commands = [
            ...
            BotCommand(command="settings", description="⚙️ الإعدادات وإدارة البوت"),
            BotCommand(command="clean", description="🧹 تنظيف المجلدات والذاكرة"),
            BotCommand(command="restart", description="🔄 إعادة تشغيل البوت"),
            BotCommand(command="trim", description="✂️ قص مقطع فيديو")
        ]
        await bot(SetBotCommandsRequest(scope=None, lang_code='', commands=commands))
        print("✅ تم ضبط قائمة الأوامر التفاعلية الرسمية للبوت بنجاح.")
    except Exception as e: print(f"⚠️ فشل إضافة قائمة الأوامر: {e}")
