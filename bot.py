...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best'):
    ...
    if fmt in ['mp3', 'audio_mp3']:
        ...
        ydl_opts['postprocessors'] = [{
            ...
            'preferredcodec': 'mp3',
            'preferredquality': '320', # أقصى خام جودة صوات متاح
        }]
    elif quality != 'best':
            ...
            for root, _, files in os.walk(task_dir):
                for file in files:
                    ...
                    if os.path.isfile(fpath) and os.path.getsize(fpath) > 500 and not file.endswith('_thumb.jpg') and not file.endswith('_clean.jpg') and not file.endswith('_final.jpg') and not file.startswith('frame_'):
                        ...
                        if "none" in file.lower() or current_ext not in ['.jpg', '.jpeg', '.png', '.webp', '.mp4', '.mkv', '.mov', '.avi', '.mp3', '.m4a']:
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🔴 **دعم يوتيوب الذكي:** تنزيل فيديوهات وShorts/Reels يوتيوب بجودة مختارة أو صوت MP3 عالية الخامة.\n"
            ...
            "3️⃣ **تنسيق الوقت:** إظهار الوقت بدقائق وثوانٍ.\n"
            "4️⃣ **أداء Non-Blocking:** تحسينات الاستجابة مع aiosqlite.\n"
            "5️⃣ **تقسيم الفيديوهات:** تجزئة ملفات +1.9GB تلقائياً.\n"
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        if is_youtube_url(clean_u):
            ...
            await save_task(task_key, u, "youtube") # نمرر الرابط الأصلي للتحكم بالخيارات
            buttons = [
