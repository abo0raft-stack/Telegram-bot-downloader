...
VERSION = "v96.2-Dailymotion-LargeFiles-Fix"
...
async def split_video_file(filepath, max_size_mb=1950):
    """تقسيم الفيديو إذا كان الحجم أكبر من 2 جيجابايت"""
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    """تنزيل وتحميل ديليموشن مع دعم التقسيم لملفات > 2GB"""
    chat_id = event.chat_id
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if is_avatar_task:
                ...
                else: filepath = await convert_to_mp4(filepath)
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **تثبيت أداء Async وثغرات التنزيل (v96.1):** حل مشاكل التثبيت ومنع أخطاء Event Loop المتداخلة.\n\n"
