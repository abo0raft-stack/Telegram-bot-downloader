...
# --- الإعدادات ---
VERSION = "v72.0-FixStartCommand-CancelSupport"
...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best', cancel_event=None):
    def progress_hook(d):
...
# --- دالة التنزيل المباشر مع شريط التقدم والسرعة وحجم الملف وإلغاء ---
def download_direct_file_with_progress(url, filepath, cancel_event, loop, status_msg, cancel_btn):
    ...
    with open(filepath, 'wb') as f:
        for chunk in res.iter_content(chunk_size=1024*512):
            ...
            if chunk:
                ...
                # تحديث التلجرام كل 2.5 ثانية تجنباً لـ FloodWait
...
# --- التنفيذ المباشر المحدث وشريط الرفع ودعم الإلغاء ---
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    cancel_btn = [[Button.inline("✖️ إلغاء العملية", data=f"cancel_{task_id}")]]
    
    if not status_msg:
        status_msg = await bot.send_message(chat_id, "⏳ **جاري تحضير المحتوى...**", buttons=cancel_btn)
    else:
        await status_msg.edit("⏳ **جاري تحضير المحتوى...**", buttons=cancel_btn)
    ...
    try:
        ...
        if target_url and is_complex_url(target_url):
            await loop.run_in_executor(None, download_with_ytdlp, target_url, task_dir, target_fmt, quality, cancel_event)
            ...
            await loop.run_in_executor(None, download_direct_file_with_progress, target_url, filepath, cancel_event, loop, status_msg, cancel_btn)
        elif media_msg:
            filepath = os.path.join(task_dir, filename)
        ...
        async def upload_progress_callback(current, total):
            ...
            if now - last_upload_update[0] > 2.5:
                ...
                bar = create_progress_bar(percent)
                msg_text = (
                    f"📤 **جاري رفع الملف إلى التلجرام...**\n\n"
                    f"[{bar}] {percent:.1f}%\n"
                    f"⏱️ **وقت الرفع:** {human_readable_time(elapsed)}\n"
                    f"💾 **الحجم الكلي:** {human_readable_size(total)}\n"
                    f"📊 **تم رفع:** {human_readable_size(current)} | **متبقي:** {human_readable_size(rem_size)}\n"
                    f"⚡ **سرعة الرفع:** {human_readable_size(speed)}/s"
                )
                try:
        ...
        if photos:
            ...
            if len(photos) > 1 and not as_doc:
                try:
                    ...
                    for p in photos[:10]:
                        if cancel_event.is_set(): raise Exception("CANCELLED")
                        uploaded_file = await bot.upload_file(p, progress_callback=upload_progress_callback)
                    ...
                    if "CANCELLED" in str(album_err): raise album_err
            ...
            if not sent_as_album:
                for idx, p in enumerate(photos, start=1):
                    if cancel_event.is_set(): raise Exception("CANCELLED")
                    try:
                        uploaded_single = await bot.upload_file(p, progress_callback=upload_progress_callback)
                        ...
                        if "CANCELLED" in str(ex): raise ex
                        await bot.send_file(chat_id, p, force_document=True, progress_callback=upload_progress_callback)

        # 2. إرسال الفيديوهات
        for vid in videos:
            if cancel_event.is_set(): raise Exception("CANCELLED")
            if as_doc:
                await bot.send_file(chat_id, vid, force_document=True, caption="📁 **تم التحميل كمستند بنجاح!**", progress_callback=upload_progress_callback)
            else:
        ...
        # 3. إرسال ملفات الصوت والمستندات الأخري
...
# --- استقبال أمر البدء /start بشكل مستقل ومباشر ---
@bot.on(events.NewMessage(pattern=r'^/start'))
async def start_command_handler(event):
    await event.respond("👋 **أهلاً بك! أرسل لي أي رابط مباشر أو رابط من منصات التواصل الاجتماعي لبدء التحميل.**")

# --- استقبال الرسائل والتفاعل مع الروابط ---
...
async def handle_message(event):
    ...
    # إهمال الأمر start حتى لا يتعارض مع المعالج الخص به
    if text.startswith('/start'):
        return

    # التمييز بين روابط التواصل والروابط المباشرة
    if text.startswith('http://') or text.startswith('https://'):
        if is_complex_url(text):
            # رابط منصة تواصل اجتماعي - تنزيل تلقائي مباشرة
            filename = get_clean_filename(text)
            await start_direct_execution(chat_id, text, filename)
        else:
            # رابط تحميل مباشر غير تابع للتواصل - إظهار قائمة الخيارات
            filename = get_clean_filename(text)
            task_key = f"dl_{int(time.time()*1000)}"
            PENDING_TASKS[task_key] = {'url': text, 'filename': filename}

            buttons = [
                [Button.inline("🎥 فيديو (MP4)", data=f"fmt_mp4_{task_key}"), Button.inline("🎵 صوت (MP3)", data=f"fmt_mp3_{task_key}")],
                [Button.inline("📁 مستند (Document)", data=f"fmt_doc_{task_key}")]
            ]
            
            await event.respond(
                f"🔗 **تم اكتشاف رابط تحميل مباشر:**\n`{text}`\n\nاختر الصيغة المطلوبة للتحميل:",
                buttons=buttons
            )
...
async def callback_handler(event):
    ...
    if data.startswith("cancel_"):
        ...
        if cancel_event:
            cancel_event.set()
            await event.answer("⏳ جاري إلغاء العملية...")
        else:
    ...
    if data.startswith("fmt_"):
        parts = data.split("_")
        fmt_type = parts[1]  # mp4, mp3, doc
        task_key = f"{parts[2]}_{parts[3]}"
        
        task_info = PENDING_TASKS.get(task_key)
        if not task_info:
            await event.answer("⚠️ انتهت صلاحية هذا الطلب، يرجى إعادة إرسال الرابط.", alert=True)
            return

        await event.answer("🚀 جار بدء التنزيل...")
        status_msg = await event.edit("⏳ **جاري تحضير الاتصال بالمباشر...**", buttons=None)

        url = task_info['url']
        filename = task_info['filename']

        if fmt_type == "mp4":
            await start_direct_execution(event.chat_id, url, filename, as_doc=False, target_fmt='mp4', status_msg=status_msg)
        elif fmt_type == "mp3":
            base_name = os.path.splitext(filename)[0]
            await start_direct_execution(event.chat_id, url, f"{base_name}.mp3", as_doc=False, target_fmt='mp3', status_msg=status_msg)
        elif fmt_type == "doc":
            await start_direct_execution(event.chat_id, url, filename, as_doc=True, target_fmt='doc', status_msg=status_msg)
