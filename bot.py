...
def convert_or_rescale_video(input_path, output_path, target_format):
    ...
    if target_format == 'audio_mp3':
        ...
        cmd.extend(['-c:v', 'libx264', '-crf', '28', '-preset', 'ultrafast', '-c:a', 'aac', '-b:a', '128k', output_path])
...
async def process_queue(chat_id):
    ...
    async with QUEUE_LOCKS[chat_id]:
        while chat_id in DOWNLOAD_QUEUES and len(DOWNLOAD_QUEUES[chat_id]) > 0:
            ...
            await start_execution(chat_id, url, filename, as_doc, quality, media_msg)
...
async def start_handler(event):
    welcome_text = (
        ...
        "2️⃣ أرسل فيديو محلي لتغيير صيغته أو ضغط جودته أو تحويله إلى MP3!"
    )
    await event.respond(welcome_text)

# --- معالجة استقبال الفيديوهات المباشرة لتحويل الصيغ ---
...
async def video_file_handler(event):
    ...
    buttons = [
        [Button.inline("🎵 تحويل إلى MP3 (صوت)", data="convert_audio_mp3")],
        [Button.inline("🎬 تحويل إلى MP4 (1080p)", data="convert_1080"), Button.inline("🎬 (720p)", data="convert_720")],
        [Button.inline("📱 تحويل إلى MP4 (480p)", data="convert_480"), Button.inline("📁 رفع كملف خام", data="convert_doc")],
    ]
    await event.respond(f"📹 **تم استقبال الفيديو!**\n📁 **الاسم:** `{orig_name}`\n\nاختر صيغة/جودة التحويل المطلوبة:", buttons=buttons)
...
async def callback_handler(event):
    ...
    # معالجة أزرار تحويل الفيديو المباشر
    elif data.startswith("convert_"):
        state_data = USER_STATES.get(user_id)
        if state_data and 'media_msg' in state_data:
            conv_type = data.replace("convert_", "")
            as_doc = False
            quality = 'best'
            custom_name = state_data['custom_name']
            
            if conv_type == "audio_mp3":
                quality = "audio_mp3"
                custom_name = os.path.splitext(custom_name)[0] + ".mp3"
            elif conv_type in ["1080", "720", "480"]:
                quality = conv_type
            elif conv_type == "doc":
                as_doc = True
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None):
    ...
    try:
        ...
        # 2. إذا كان رابط خارجي
            ...
            if is_m3u8 or not is_direct_mp4:
                ...
                elif quality in ['audio', 'audio_mp3']: format_opt = 'bestaudio/best'
        ...
        # --- معالجة التحويل و الضغط بالـ FFmpeg ---
        if os.path.exists(file_path):
            file_size = os.path.getsize(file_path)
            if file_size < 10 * 1024: 
                raise Exception("الملف المحمل غير صالح أو صغير جداً.")

            if quality in ['1080', '720', '480', 'audio_mp3']:
                await status_msg.edit(f"⚙️ **جاري معالجة وتغيير الصيغة ({quality})...**")
                processed_file = await loop.run_in_executor(None, convert_or_rescale_video, file_path, rescaled_path, quality)
                if processed_file != file_path and os.path.exists(processed_file):
                    os.remove(file_path)
                    file_path = processed_file

            is_audio = quality in ['audio', 'audio_mp3'] or file_path.endswith('.mp3')
