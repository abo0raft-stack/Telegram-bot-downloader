...
# تخزين إعدادات المستخدمين في الذاكرة (حجم الخط الافتراضي: كبير جداً)
USER_SETTINGS = {}
PENDING_TASKS = {}
DIRECT_TASKS = {}
...
def add_transparent_text_center(image_path, text, font_ratio=0.35):
    try:
        with Image.open(image_path).convert("RGBA") as base:
            ...
            w, h = base.size
            # استخدام النسبة الجديدة المكبرة لضمان الوضوح التام
            ...
            # أبيض شفاف بوضوح عالي (Alpha = 160)
...
def extract_9_frames(video_path, duration, chat_id=None):
    ...
    # جلب تفضيل حجم الخط الخاص بالمستخدم (الافتراضي كبير ضخم)
    font_choice = USER_SETTINGS.get(chat_id, {}).get("font_size", "large") if chat_id else "large"
...
# --- واجهة إعدادات حجم الخط ---
async def send_settings_menu(event):
    chat_id = event.chat_id
    current = USER_SETTINGS.get(chat_id, {}).get("font_size", "large")
    ...
    buttons = [
        [
            Button.inline(f"{'✅ ' if current=='small' else ''}كبير", data="setfont_small"),
            Button.inline(f"{'✅ ' if current=='medium' else ''}كبير جداً", data="setfont_medium")
        ],
        [
            Button.inline(f"{'✅ ' if current=='large' else ''}ضخم (موصى به)", data="setfont_large"),
            Button.inline(f"{'✅ ' if current=='xlarge' else ''}عملاق", data="setfont_xlarge")
        ]
    ]
    
    msg = f"⚙️ **إعدادات حجم الخط على صور الفيديو:**\n\nالحجم الحالي المقترن بحسابك: **{labels.get(current, 'ضخم')}**\n\nاختر الحجم الذي يناسبك:"
...
@bot.on(events.CallbackQuery(pattern=r"^setfont_"))
async def font_settings_callback(event):
    chat_id = event.chat_id
    font_choice = event.data.decode("utf-8").split("_")[1]
    
    if chat_id not in USER_SETTINGS:
        USER_SETTINGS[chat_id] = {}
        
    USER_SETTINGS[chat_id]["font_size"] = font_choice
    await event.answer("✅ تم تحديث حجم الخط بنجاح!", alert=True)
    await send_settings_menu(event)
...
async def start_handler(event):
    changelog_text = (
        ...
        "1️⃣ **التحكم بحجم الخط:** إضافة إعدادات تخصيص حجم الخط على الصور عبر الأمر `/settings` (تم تكبير القيم لتكون واضحة بالعين).\n"
        "2️⃣ **إلغاء العمليات:** إكانية إيقاف وإلغاء أي عملية تنزيل أو رفع فوراً عبر زر `❌ إلغاء العملية`.\n"
        ...
        "⚙️ لتعديل حجم الخط على الصور، أرسل الأمر: `/settings`"
    )
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة وجاهز مع خيارات إعدادات الخط المكبرة!")
