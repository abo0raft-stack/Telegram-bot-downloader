...
# --- دالة جديدة: توليد 9 لقطات من الفيديو ---
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        # 2. إرسال الفيديوهات + استخراج وإرسال 9 لقطات (ألبوم)
        for vid in videos:
            ...
            # --- توليد وإرسال 9 لقطات شاشة ألبوم من الفيديو ---
            try:
                shots = await loop.run_in_executor(None, extract_9_screenshots, vid, duration)
                if shots:
                    shot_handles = []
                    for shot in shots:
                        uploaded_shot = await bot.upload_file(shot)
                        shot_handles.append(uploaded_shot)
                    await bot.send_file(chat_id, shot_handles, caption="🖼️ **لقطات شاشة ممثلة للفيديو (9 صور):**")
            except Exception as shot_err:
                print(f"Error sending video screenshots: {shot_err}")

        # 3. إرسال ملفات الصوت والمستندات الأخرى
...
async def callback_handler(event):
    ...
    if data.startswith("cancel_"):
        ...
        if task:
            ...
            await event.answer("⚠️ لا توجد عملية نشطة للإلغاء.", alert=True)
        return
