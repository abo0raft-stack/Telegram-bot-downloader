...
VERSION = "v85.0-Fast-Avatar-Engine"
...
# --- دالة فائقة السرعة لتحميل الصورة الشخصية (Fast Avatar Engine v85) ---
async def download_profile_avatar(url, task_dir):
    filepath = os.path.join(task_dir, "profile_avatar.jpg")
    
    # 1. الملاذ المباشر الفائق السرعة عبر Requests + Meta Tags (Fast Direct Scraping)
    try:
        async with aiohttp.ClientSession(headers=BROWSER_HEADERS) as session:
            async with session.get(url, timeout=10) as resp:
                if resp.status == 200:
                    html = await resp.text()
                    # البحث عن وسم og:image
                    img_match = re.search(r'<meta\s+property=["\']og:image["\']\s+content=["\']([^"\'\s]+)["\']', html, re.I)
                    if not img_match:
                        img_match = re.search(r'<meta\s+content=["\']([^"\'\s]+)["\']\s+property=["\']og:image["\']', html, re.I)
                    
                    if img_match:
                        avatar_url = img_match.group(1).replace("&amp;", "&")
                        async with session.get(avatar_url, timeout=10) as img_resp:
        ...
        print(f"Fast Meta Avatar Fetch Error: {e}")

    # 2. الخيار السريع عبر Cobalt API
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(api_url, json={"url": url}, headers=headers, timeout=10) as res:
                if res.status == 200:
                    data = await res.json()
                    img_url = data.get("url") or (data.get("picker", [{}])[0].get("url") if data.get("picker") else None)
                    if img_url:
                        async with session.get(img_url, headers=BROWSER_HEADERS, timeout=15) as r:
                            if r.status == 200:
                                with open(filepath, 'wb') as f:
        ...
        print(f"Cobalt Avatar Error: {e}")

    # 3. الخيار الاحتياطي بواسطة gallery-dl
    try:
        cmd = [
            "gallery-dl",
            "--directory", task_dir,
        ...
        print(f"gallery-dl Avatar Error: {e}")

    # 4. الخيار الاحتياطي بواسطة yt-dlp
    try:
        ...
        print(f"yt-dlp Avatar Error: {e}")

    return any(os.path.isfile(os.path.join(task_dir, f)) for f in os.listdir(task_dir))
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None, is_avatar_task=False):
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if is_avatar_task:
                await status_msg.edit("👤 **جاري استخراج صورة الملف الشخصي بسرعة عالية...**", buttons=cancel_btn)
                    ...
                    for idx, p in enumerate(photos, start=1):
                        try:
                            ...
                            if as_doc:
                                ...
                                cap = "👤 **تم تنزيل الصورة الشخصية بنجاح!**" if is_avatar_task else (f"📸 **صورة ({idx}/{len(photos)}):**" if len(photos) > 1 else "📸 **تم تنزيل الصورة بنجاح!**")
...
async def send_settings_menu(event):
    chat_id = event.chat_id
    msg = "⚙️ **لوحة التحكم والإعدادات الخاصّة بالبوت:**\n\nقم بالضغط على الأزرار أدناه لتعديل خيارات التحميل والتقاط الصور أو إدارة الأعضاء."
...
@bot.on(events.CallbackQuery(pattern=r"^(toggle_snaps|toggle_social_snaps|change_qual|change_font|add_user|list_users|close_settings|rmuser_.*)$"))
...
async def start_handler(event):
    ...
    welcome_text = (
        ...
        "⚙️ لتعديل الإعدادات أو إدارة الأعضاء أرسل: `/settings`"
    )
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "⚡ **تسريع استخراج الصور الشخصية (Fast Avatar):** تم دمج محرك كشط مباشر مع Cobalt API لتنزيل صور البروفايل خلال ثوانٍ معدودة وبصورة فورية.\n"
            "👥 **دعم السماح لإضافة عضو عبر اليوزر:** إمكانية إضافة أو إزالة أعضاء بواسطة اسم المستخدم (Username) عبر خيارات لوحة الإعدادات ومنحهم صلاحية الاستخدام الكاملة.\n"
            "📺 **إصلاح Dailymotion الشامل:** معالجة مسار التوجيه واستعادة النطاق الكامل وإسناد الـ Referer المطلوبة.\n"
            "🔒 **الحماية والخصوصية:** دعم الصلاحيات عبر OWNER_ID والأعضاء المضافين بأسماء المستخدمين.\n"
...
async def handle_user_input_and_urls(event):
    ...
    # الاستجابة لإضافة يوزر عضو جديد إذا كان ينتظر الإدخال
...
def main():
    ...
    print(f"🤖 البوت يعمل ومحمي. المالك: {OWNER_ID}")
