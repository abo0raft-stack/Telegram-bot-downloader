...
VERSION = "v81.0-Dailymotion-Supported"
...
def is_dailymotion_url(url):
    ...
    return any(x in domain for x in ['dailymotion.com', 'dai.ly'])

# --- تحديث دالة تنظيف الرابط لاستثناء ديليموشن ---
def clean_url(url):
...
# --- تحديث دالة التنزيل عبر yt-dlp بخيارات Dailymotion الممتازة ---
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "📺 **دعم منصة Dailymotion:** إضافة خيارات التنزيل المتعدد للجودة والتحويل التلقائي لصيغة MP4.\n"
...
async def url_handler(event):
    ...
    for u in urls:
        # --- استثناء Dailymotion من فصل المعاملات split('?') ---
