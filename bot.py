...
async def get_video_metadata_and_thumb(file_path):
    ...
    try:
        ...
        # فحص جودة وصحة ملف الغلاف لتجنب خطأ SendMediaRequest
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    try:
        ...
        # التحقق الآمن لملف الغلاف قبل الرفع
...
# --- محرك TikTok الشامل الهجين والمطور ---
async def tiktok_multi_engine(url, task_dir):
    """محرك متكامل وهجين لتنزيل صور وفيديوهات تيكتوك مع الوصف وبدائل متعددة لضمان الاستخراج"""
    
    # 1. التجربة باستخدام gallery-dl
    ...
    # 2. التجربة عبر TikWM API (تنسيق الصور والفيديو HD)
    try:
        async with aiohttp.ClientSession() as session:
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None, is_avatar_task=False):
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            # استخراج نص المنشور وترجمته إذا لزم الأمر
            post_desc_text = ""
            ...
            # إعداد نص الوصف المرفق والترجمة
            ...
            for vid_idx, vid in enumerate(videos, start=1):
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    ...
                    # ضبط الخواص والأبعاد بشكل دقيق لإرسال مقطع فيديو
                    ...
                    # تحسين وقاية إرسال الصور المصغرة المرفقة بالوسائط
                    safe_thumb = thumb_path if (thumb_path and os.path.exists(thumb_path) and os.path.getsize(thumb_path) > 200) else None
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "✨ **دعم نطاق brqz.online (v99.4):** إضافة الدعم المباشر والتنزيل بدون قيود.\n"
            "🚀 **حل مشكلة الحظر (v99.3):** إضافة نظام التخطي الذكي وإعادة المحاولة لتجاوز حظر '429 Too Many Requests'.\n\n"
...
# --- معالجة الضغط على أزرار اختيار الجودة والملفات الشخصية ---
@bot.on(events.CallbackQuery(pattern=r"^(q_|prof_|fb_)"))
...
async def handle_user_input_and_urls(event):
    ...
    for u in urls:
        ...
        if is_profile_url(clean_u):
            ...
            buttons = [
                [
                    Button.inline("🎬 جودة عالية (1080p)", data=f"fb_1080_{task_key}"),
                    Button.inline("📹 جودة متوسطة (720p)", data=f"fb_720_{task_key}")
                ],
                [
                    Button.inline("📱 جودة منخفضة (480p)", data=f"fb_480_{task_key}"),
                    Button.inline("🎵 صوت فقط (MP3)", data=f"fb_mp3_{task_key}")
                ]
            ]
            await event.respond("📘 **تم التعرف على رابط فيسبوك. اختر الجودة المطلوبة للتحميل:**", buttons=buttons)
            ...
            buttons = [
                [
                    ...
                    Button.inline("📹 جودة متوسطة (720p)", data=f"q_720_{task_key}")
                ],
                [
                    Button.inline("📱 جودة منخفضة (480p)", data=f"q_480_{task_key}"),
                    Button.inline("🎵 صوت فقط (MP3)", data=f"q_mp3_{task_key}")
                ]
            ]
            await event.respond("📺 **تم التعرف على رابط Dailymotion. اختر الجودة المطلوبة:**", buttons=buttons)

        elif is_complex_url(clean_u):
            task_key = f"soc_{chat_id}_{int(time.time()*1000)}"
            await save_task(task_key, clean_u, "social")
            buttons = [
                [
                    Button.inline("🎬 جودة عالية (1080p)", data=f"q_1080_{task_key}"),
                    Button.inline("📹 جودة متوسطة (720p)", data=f"q_720_{task_key}")
            ...
            await event.respond("🌐 **تم التعرف على رابط من منصات التواصل الاجتماعي. حدد الجودة للبدء:**", buttons=buttons)
...
async def main():
    await init_db()
    print(f"🚀 تم تشغيل البوت بنجاح - الإصدار ({VERSION})...")
