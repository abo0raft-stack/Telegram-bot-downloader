...
# --- الإعدادات ---
VERSION = "v85.0-UniversalQuality-Edition"
...
def get_dynamic_headers(url):
    """ الترويسات الديناميكية لتجاوّز حظر 403 Forbidden """
    parsed = urlparse(url)
    ...
    if "xnxx" in domain or "xnxx-cdn" in domain or "cdn" in domain:
...
def is_complex_url(url):
    ...
    complex_domains = [
        ...
        'threads.net', 'snapchat.com', 'vk.com', 'vimeo.com', 'youtube.com', 'youtu.be'
    ]
...
# --- المحركات الذكية لسحب الفيديو بكافة الجودات ---
...
def cobalt_fallback_engine(url, task_dir, quality='720'):
    try:
        api_url = "https://api.cobalt.tools/api/json"
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
            "User-Agent": get_dynamic_headers(url)['User-Agent']
        }
        payload = {"url": url, "videoQuality": str(quality)}
        ...
        if res.status_code == 200:
            ...
            for idx, media_url in enumerate(urls_to_dl, start=1):
                ...
                r = requests.get(media_url, stream=True, headers=get_dynamic_headers(media_url), timeout=30)
                if r.status_code == 200:
                    is_vid = ".mp4" in media_url or "video" in r.headers.get("content-type", "")
                    ext = "mp4" if is_vid else "jpg"
                    filepath = os.path.join(task_dir, f"cobalt_media_{idx}.{ext}")
...
# --- التنزيل غير المتزامن المباشر والتنقل الذكي المحسن ---
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='720', media_msg=None, target_fmt='mp4', status_msg=None, trim_times=None):
    ...
    if not status_msg:
        status_msg = await bot.send_message(chat_id, "⏳ **جاري تحضير الطلب وسحب الفيديو...**", buttons=cancel_btn)
    else:
        await status_msg.edit("⏳ **جاري تحضير الطلب وسحب الفيديو...**", buttons=cancel_btn)
    ...
    try:
        ...
        # 1. التنزيل المباشر كخيار أول في حال إرسال ملف مباشر
        direct_success = False
        if not is_complex_url(target_url):
            try:
                filepath = os.path.join(task_dir, filename)
                await download_direct_async(bot, chat_id, target_url, filepath, status_msg, cancel_event, task_id)
                direct_success = True
            except Exception as d_err:
                print(f"Direct Async download failed, falling back to multi-engine: {d_err}")

        # 2. في حال فشل التنزيل المباشر أو كان الرابط منصة/موقع تفاعلي: التبديل لمحركات الذكاء الشاملة
        if not direct_success:
            await status_msg.edit("⚡ **جاري استخراج الفيديو عبر المحرك الذكي...**", buttons=cancel_btn)
            ...
            if not downloaded:
                await loop.run_in_executor(None, cobalt_fallback_engine, target_url, task_dir, quality)
                downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                
            if not downloaded:
                await loop.run_in_executor(None, gallery_dl_fallback_engine, target_url, task_dir, "fallback")
        ...
        if not downloaded_files:
            raise Exception("تعذر الوصول إلى الفيديو أو المحتوى. تأكد من صحة الرابط وسماحية الوصول.")
        ...
        # رفع الفيديوهات وتقسيمها إن كانت ضخمة
        for vid in videos:
            if cancel_event.is_set(): raise Exception("CANCELLED")
            ...
            for idx, part_vid in enumerate(split_vids, start=1):
                ...
                await bot.send_file(chat_id, part_vid, caption=f"🎥 **تم تنزيل الفيديو بنجاح ({quality}p)!**{part_caption}\n📁 `{os.path.basename(part_vid)}`", force_document=as_doc, thumb=thumb_path, attributes=attr)
                
                should_take_snaps = user_config.get("snapshots", True)
...
def build_settings_buttons(chat_id):
    ...
    social_snap_status = "✅ مفعّلة" if config["social_snapshots"] else "❌ معطلة"
    ...
    buttons = [
        ...
        [
            Button.inline(f"🎥 الجودة الافتراضية: {config['quality']}p", data="change_qual"),
...
async def settings_callback_handler(event):
    ...
    if data == "toggle_snaps":
        ...
        current_idx = qualities.index(config["quality"]) if config["quality"] in qualities else 2
...
async def start_handler(event):
    welcome_text = (
        "🤖 **أهلاً بك في بوت التنزيل الشامل ونسخة الجودات المعتمدة!**\n\n"
        "✨ **أبرز ميزات وخدمات البوت:**\n\n"
        "🎬 **تنزيل أسرع وأقوى بالفيديوهات والروابط المباشرة**\n"
        "• إرسال أي رابط فيديو للاختيار المباشر بين الجودات: (360p, 480p, 720p, 1080p).\n"
        "• دعم سحب الفيديوهات من جميع المنصات والمواقع المباشرة وحل أخطاء 403.\n"
        "• **تقسيم الفيديوهات الضخمة تلقائياً** لمنع تجاوز حدود تيليجرام.\n"
        "• **قص مقطع محدد:** استخدم الأمر `/trim [بدء] [نهاية] [رابط]`.\n"
        "• **توليد ألبوم 9 لقطات مصورة** مع استعراض الأوقات بالثواني والدقائق.\n\n"
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "1️⃣ **التحميل المباشر والتلقائي ذكي الجودات:** إضافة خيارات (360p, 480p, 720p, 1080p) لجميع الفيديوهات والروابط[span_4](start_span)[span_4](end_span).\n"
            "2️⃣ **تجاوز حظر 403 Forbidden:** إضافة نظام الترويسات الديناميكي المتكيف مع خوادم الـ CDN العالمية.\n"
            "3️⃣ **المحركات الشاملة Fallback Engine:** الدمج التلقائي بين yt-dlp و Cobalt API و gallery-dl لتنزيل الفيديو بدون أخطاء.\n"
            "4️⃣ **تقسيم الفيديوهات تلقائياً (Video Splitter):** تجزئة أوتوماتيكية للفيديوهات +1.9GB[span_5](start_span)[span_5](end_span).\n"
            "5️⃣ **قص الفيديو (Video Trimmer):** قص مقطع زمني محدد باستخدام أمر `/trim`[span_6](start_span)[span_6](end_span).\n\n"
...
# --- معالج استقبال الروابط بجميع الجودات ---
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        task_key = f"vid_{chat_id}_{int(time.time()*1000)}"
        save_task(task_key, clean_u, "universal")
        
        buttons = [
            [Button.inline("🎬 1080p (عالية)", data=f"q_1080_{task_key}"), Button.inline("🎥 720p (متوسطة)", data=f"q_720_{task_key}")],
            [Button.inline("📱 480p (عادية)", data=f"q_480_{task_key}"), Button.inline("📺 360p (منخفضة)", data=f"q_360_{task_key}")],
            [Button.inline("🎵 صوت فقط (MP3)", data=f"q_mp3_{task_key}"), Button.inline("📄 مستند (Document)", data=f"q_doc_{task_key}")]
        ]
        await event.respond("🎬 **تم التعرف على الرابط! اختر جودة أو صيغة التحميل المطلوبة:**", buttons=buttons)
...
async def quality_callback_handler(event):
    ...
    if choice in ['1080', '720', '480', '360']:
        quality_val = choice
    else:
        quality_val = '720'
        
    status_msg = await event.edit("⏳ **جاري بدء عملية الاستخراج والتنزيل...**", buttons=None)
...
def main():
    ...
    print("🤖 البوت يعمل بكفاءة مع أزرار التحكم بالجودة والنظام المباشر الشامل!")
