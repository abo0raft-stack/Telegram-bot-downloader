...
VERSION = "v91.0-Media-Size-Formats-Info"
...
def format_size(size_bytes):
    if not size_bytes or size_bytes == 0: return "غير معروف"
    units = ['B', 'KB', 'MB', 'GB']
    i = 0
...
# --- دالة استخراج وتوقع أحجام التنسيقات المتوفرة ---
async def fetch_media_formats_info(url):
    ...
    qualities = {'1080': None, '720': None, '480': None, 'mp3': None}
    formats = info.get('formats', [])
    
    # البحث عن أفضل أحجام التنسيقات لكل جودة
    for f in formats:
        h = f.get('height')
        ...
        if f.get('vcodec') != 'none':
            if h:
                if h >= 1080 and not qualities['1080']: qualities['1080'] = filesize
                elif 720 <= h < 1080 and not qualities['720']: qualities['720'] = filesize
                elif h < 720 and not qualities['480']: qualities['480'] = filesize
        elif f.get('acodec') != 'none' and not qualities['mp3']:
            qualities['mp3'] = filesize
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    try:
        ...
        await bot.send_file(
            chat_id,
            file_path,
            caption=f"🎥 **تم التنزيل بنجاح من Dailymotion!**\n📁 `{fname}`",
            thumb=thumb_path,
            attributes=attr,
                ...
                if len(photos) > 1 and not as_doc:
                    ...
                    for idx, p in enumerate(photos, start=1):
                        try:
                            ...
                            pname = os.path.basename(p)
                            if as_doc:
                                await bot.send_file(chat_id, p, caption=f"📄 **مستند صورة ({idx}/{len(photos)}):**\n📁 `{pname}`", force_document=True)
                            else:
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "📊 **لوحة إظهار حجم الملفات والتنسيقات (v91):** معالجة وجلب أحجام وسعة التنزيل المقابلة للجودات قبل التحميل، مع تحسين لوحة أزرار اختيار الجودة للمنصات وDailymotion.\n"
            "📋 **نسخ اسم الملف تلقائياً (v90):** دعم الضغط على اسم الملف المُنَزَّل لنسخ اسمه مباشرة إلى الحافظة بسهولة وسرعة.\n"
            "🌐 **حل روابط السيرفرات المباشرة (v89):** معالجة فشل التحميل لروابط CDN والم مواقع المباشرة عن طريق تضمين الهيدرز الخاصة بالمتصفح وخاصية البث الشريحي.\n"
            "📘 **إصلاح فيديوهات Facebook:** حل مشكلة وصول فيديوهات الفيسبوك كـ GIF أو صورة متحركة عبر تحويل وتصفية صيغ الفيديو بترميز MP4.\n"
            "🎛️ **خيارات جودة Facebook:** إضافة أزرار انسيابية لاختيار جودة التنزيل عند إرسال روابط الفيسبوك.\n"
            "⚡ **دعم التنزيل المباشر المحسّن:** دعم روابط MP4, M3U8, MPD, MKV والبث المباشر وتجزئتها بسلاسة عالية.\n"
            "🔄 **محرك الاستئناف المباشر الذكي:** إمكانية استئناف التحميل عند انقطاع الاتصال دون البدء من صفر MB.\n"
            "🖼️ **الاحتفاظ بألبومات Instagram & TikTok:** إمكانية تحميل المنشورات كألبوم صور/مستندات دون أي تغيير.\n\n"
...
async def handle_user_input_and_urls(event):
    ...
    for u in urls:
        ...
        if is_profile_url(clean_u):
        ...
        elif is_complex_url(clean_u) or is_dailymotion_url(clean_u):
            # فحص وتوقع الحجم والجودات المتوفرة (v91)
            wait_msg = await event.respond("🔍 **جاري فحص الأحجام والجودات المتوفرة...**")
            sizes = await fetch_media_formats_info(clean_u)
            
            s_1080 = format_size(sizes.get('1080'))
            s_720 = format_size(sizes.get('720'))
            s_480 = format_size(sizes.get('480'))
            s_mp3 = format_size(sizes.get('mp3'))

            size_panel = (
                "📊 **معلومات الجودات والأحجام المتوفرة للملف:**\n\n"
                f"• 🎬 **1080p (عالية):** `{s_1080}`\n"
                f"• 🎥 **720p (متوسطة):** `{s_720}`\n"
                f"• 📱 **480p (منخفضة):** `{s_480}`\n"
                f"• 🎵 **صوت MP3:** `{s_mp3}`\n\n"
            ...
            if is_facebook_url(clean_u):
                ...
                await wait_msg.edit(f"📘 **تم التعرف على رابط Facebook.**\n\n{size_panel}", buttons=buttons)
                ...
                await wait_msg.edit(f"🔴 **تم التعرف على رابط YouTube.**\n\n{size_panel}", buttons=buttons)
                ...
                await wait_msg.edit(f"📺 **تم التعرف على رابط Dailymotion.**\n\n{size_panel}", buttons=buttons)
                ...
                await wait_msg.edit(f"📸 **تم التعرف على منشور TikTok.**\n\n{size_panel}", buttons=buttons)
                ...
                if is_ig_video:
                    ...
                    await wait_msg.edit(f"🎬 **منصة Instagram.**\n\n{size_panel}", buttons=buttons)
                else:
                    buttons = [
                        [
                    ...
                    await wait_msg.edit(f"📸 **منشور Instagram.**\n\n{size_panel}", buttons=buttons)
