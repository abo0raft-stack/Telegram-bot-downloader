...
# --- الإعدادات ---
VERSION = "v69.0-Final-TikTok-UploadMedia-Fix"
...
def tiktok_multi_engine(url, task_dir):
    """محرك استخراج صور تيك توك متعدد المصادر"""
    try:
...
def deep_sanitize_image(file_path):
    """إعادة تصحيح بيئة ألوان وأبعاد الصور قسرياً لمنع خطأ UploadMediaRequest"""
    try:
        base_dir = os.path.dirname(file_path)
        base_name = os.path.splitext(os.path.basename(file_path))[0]
        cleaned_path = os.path.join(base_dir, f"{base_name}_clean.jpg")

        # 1. المعالجة بواسطة Pillow أولاً لإلغاء أي أنماط ألوان معقدة (Exif/ICC Profile)
        with Image.open(file_path) as img:
            rgb_img = img.convert('RGB')
            # حماية الأبعاد لضمان أنها أرقام زوجية مقبولة في السيرفر
            w, h = rgb_img.size
            if w % 2 != 0: w -= 1
            if h % 2 != 0: h -= 1
            if w > 0 and h > 0:
        ...
        # 2. التمرير عبر FFmpeg كطبقة حماية ثانية لإزالة Metadata بالكامل
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    ...
    try:
        ...
        # --- تنقية وتجهيز الصور بنظام المعالجة الشاملة ---
        downloaded_files = []
        for root, _, files in os.walk(task_dir):
            for file in files:
                ...
                if os.path.isfile(fpath) and os.path.getsize(fpath) > 500 and not file.endswith('_thumb.jpg') and not file.endswith('_clean.jpg') and not file.endswith('_final.jpg'):
                    ...
                    # إجبار أي صورة على التمرير عبر التجهيز والتنظيف المعمق
        ...
        # 1. إرسال الصور بنظام الرفع المباشر المتعدد النطاقات (Robust Multi-Fallback)
        if photos:
            sent_as_album = False
            if len(photos) > 1 and not as_doc:
                try:
                    # البديل 1: رفع الصور المجهزة أولاً لمنع أخطاء المعالجة الذاتية
                    uploaded_handles = []
                    for p in photos[:10]:
                    ...
                    print(f"Album upload failed, activating fallback: {album_err}")
            ...
            if not sent_as_album:
                for idx, p in enumerate(photos, start=1):
                    try:
                        ...
                        print(f"Single photo upload failed, sending as doc: {single_err}")
...
async def start_handler(event):
    await event.respond(
        ...
        "🎵 **تيك توك:** الفيديوهات وسلاسل الصور (Photo Mode).\n"
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة وتم معالجة رفع صور تيك توك بنجاح!")
