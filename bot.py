...
# --- الإعدادات ---
VERSION = "v48.8-FastPrep-16xFast"
...
def download_chunk(url, headers, start, end, chunk_file, cancel_event):
    ...
    res = requests.get(url, headers=req_headers, stream=True, timeout=15, verify=False)
...
def download_direct_file(url, filepath, cancel_event, num_threads=16):
    parsed_url = urlparse(url)
    headers = BROWSER_HEADERS.copy()
    headers['Referer'] = f"{parsed_url.scheme}://{parsed_url.netloc}/"
    
    # مهلة قصيرة لا تتجاوز 5 ثوان لتجنب التأخير والتعليق
    try:
        head_res = requests.head(url, headers=headers, allow_redirects=True, timeout=5, verify=False)
        if head_res.status_code == 429:
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, fmt='mp4'):
    status_msg = await bot.send_message(chat_id, "🔍 **جاري التحضير وربط السيرفر...**")
    ...
    try:
        if media_msg:
            ...
            await status_msg.edit("📥 **بدء معالجة رابط التحميل...**")
            
            if is_direct_mp4:
                try:
                    await loop.run_in_executor(None, download_direct_file, url, file_path, cancel_event, 16)
                except Exception as e:
                    if "429" in str(e) or "TOO_MANY_REQUESTS" in str(e):
                        await status_msg.edit("🔄 **تم اكتشاف تقييد من السيرفر، جاري التبديل لمحرّك التحميل التكيفي...**")
                        await loop.run_in_executor(None, download_with_ytdlp, url, file_path, fmt, quality)
                    else:
                        raise e
            else:
                await loop.run_in_executor(None, download_with_ytdlp, url, file_path, fmt, quality)
        ...
        await status_msg.edit("⚙️ **جاري فحص وتعديل الفيديو...**")
        final_path = await loop.run_in_executor(None, convert_or_rescale_video, file_path, rescaled_path, fmt)
        
        duration, width, height, thumb_path = await loop.run_in_executor(None, get_video_metadata_and_thumb, final_path)

        if user_settings.get('send_screenshots') and fmt not in ['mp3', 'audio_mp3']:
            font_size_mode = user_settings.get('font_size', 'large')
            screenshots = await loop.run_in_executor(None, generate_grid_screenshots, final_path, duration, font_size_mode)
