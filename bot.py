...
def update_and_install_libraries():
    try:
        ...
        # تثبيت مكتبة cryptg لتسريع عمليات التشفير والرفع (ترفع السرعة من 3MB/s إلى 40MB/s)
        subprocess.run(["pip", "install", "-U", "cryptg"], check=False)
...
async def download_dailymotion_video(event, url, quality_choice, status_msg):
    ...
    if not status_msg:
        status_msg = await event.respond("⏳ **جاري بدء العملية...**", buttons=cancel_btn)
    else:
        await status_msg.edit("⏳ **جاري بدء العملية...**", buttons=cancel_btn)
    ...
    def progress_hook(d):
        ...
        if d['status'] == 'downloading':
            ...
            if now - last_update_time[0] >= 2.0:
                ...
                text = (f"📥 **جاري التنزيل من Dailymotion...**\n\n[{bar}] {percentage:.1f}%\n"
                        f"🚀 **السرعة:** {format_size(speed)}/s\n📦 **الحجم:** {format_size(downloaded)} / {format_size(total)}\n"
    ...
    try:
        ...
        await status_msg.edit("📤 **اكتمل التنزيل! جاري الرفع إلى سيرفرات تيليجرام...**", buttons=cancel_btn)
        ...
        async def upload_progress_callback(current, total):
            ...
            if now - last_upload_update[0] >= 2.0 or current == total:
                ...
                text = (f"📤 **جاري رفع الملف إلى تيليجرام...**\n\n[{bar}] {percentage:.1f}%\n"
                        f"🚀 **السرعة:** {format_size(speed)}/s\n📦 **الحجم:** {format_size(current)} / {format_size(total)}\n"
                        f"⏱️ **المتبقي:** {format_time(eta)}")
                try: await status_msg.edit(text, buttons=cancel_btn)
                except: pass

        uploaded_vid = await bot.upload_file(
            file_path,
            part_size_kb=512,
            progress_callback=upload_progress_callback
        )
        
        await status_msg.edit("🔄 **جاري الإرسال النهائي للمحادثة...**", buttons=cancel_btn)
        ...
        else: await status_msg.edit(f"❌ **حدث خطأ:**\n`{str(e)}`", buttons=None)
    finally:
...
async def instagram_carousel_and_photo_engine(url, task_dir):
    try:
        ...
        loop = asyncio.get_event_loop()
        ydl_opts = {'outtmpl': os.path.join(task_dir, 'ig_hd_%(id)s_%(autonumber)s.%(ext)s'), 'quiet': True, 'no_warnings': True, 'headers': BROWSER_HEADERS, 'writethumbnails': True, 'format': 'best', 'merge_output_format': 'mp4'}
...
async def download_direct_async(client, chat_id, url, filepath, status_msg, cancel_event, task_id):
    headers = BROWSER_HEADERS.copy()
    conn = aiohttp.TCPConnector(limit=0, ttl_dns_cache=300, force_close=False, enable_cleanup_closed=True)
    async with aiohttp.ClientSession(headers=headers, connector=conn) as session:
        async with session.head(url, ssl=False, allow_redirects=True) as head_res:
            total_size = int(head_res.headers.get('content-length', 0))
            accept_ranges = head_res.headers.get('accept-ranges', '').lower() == 'bytes'
        if total_size == 0:
            async with session.get(url, ssl=False) as get_res:
                total_size = int(get_res.headers.get('content-length', 0))
                accept_ranges = get_res.headers.get('accept-ranges', '').lower() == 'bytes' or accept_ranges
        num_chunks = 8 if (accept_ranges and total_size > 5 * 1024 * 1024) else 1
        start_time = time.time()
        last_update_time = [0]
        downloaded_bytes = [0]
        cancel_btn = [Button.inline("❌ إلغاء العملية", data=f"cancel_{task_id}")]
        ...
        async def update_progress():
            while downloaded_bytes[0] < total_size or total_size == 0:
                ...
                if now - last_update_time[0] >= 1.5:
                    ...
                    rem_time = (total_size - current_downloaded) / speed if speed > 0 and total_size > 0 else 0
                    text = (f"🚀 **جاري التنزيل المباشر بأقصى سرعة...**\n[{bar}] {percent:.1f}%\n📦 الحجم: `{format_size(current_downloaded)}` / `{format_size(total_size)}`\n"
                            f"⚡ السرعة: `{format_size(speed)}/s`\n⏱️ المنقضي: `{format_time(elapsed)}` | المتبقي: `{format_time(rem_time)}`")
                    try: await status_msg.edit(text, buttons=cancel_btn)
                    except: pass
        ...
        try:
            if num_chunks > 1:
                ...
                part_files = [f"{filepath}.part{i}" for i in range(num_chunks)]
                async def download_part(part_id, start_b, end_b):
                    p_headers = headers.copy()
                    p_headers['Range'] = f"bytes={start_b}-{end_b}"
                    async with session.get(url, headers=p_headers, ssl=False) as res:
                        with open(part_files[part_id], 'wb') as f:
                            async for chunk in res.content.iter_chunked(2 * 1024 * 1024):
                                if cancel_event.is_set(): raise Exception("CANCELLED")
                                if chunk:
                                    f.write(chunk)
                ...
                for i in range(num_chunks):
                    ...
                    end_b = (start_b + chunk_size - 1) if i < num_chunks - 1 else total_size - 1
                    tasks.append(download_part(i, start_b, end_b))
                await asyncio.gather(*tasks)
                with open(filepath, 'wb') as main_f:
                    for part_f in part_files:
                        if os.path.exists(part_f):
                            with open(part_f, 'rb') as pf: shutil.copyfileobj(pf, main_f)
                            try: os.remove(part_f)
                            except: pass
            else:
                async with session.get(url, ssl=False) as response:
                    if response.status not in [200, 206]: response.raise_for_status()
                    chunk_buffer = 2 * 1024 * 1024
                    with open(filepath, 'wb') as f:
                        async for chunk in response.content.iter_chunked(chunk_buffer):
                            if cancel_event.is_set(): raise Exception("CANCELLED")
                            if chunk:
                                f.write(chunk)
                                downloaded_bytes[0] += len(chunk)
        finally: progress_task.cancel()
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    ...
                    async def upload_progress_callback(current, total):
                        ...
                        if now - last_upload_update[0] > 2.0 or current == total:
                            ...
                            text = (f"📤 **جاري رفع الملف إلى سيرفرات تيليجرام...**\n[{bar}] {percent:.1f}%\n"
                                    f"📦 الحجم: `{format_size(current)}` / `{format_size(total)}`\n"
                                    f"⚡ السرعة: `{format_size(speed)}/s`\n⏱️ المتبقي: `{format_time(rem_time)}`")
                            try: await status_msg.edit(text, buttons=cancel_btn)
                            except: pass

                    if as_doc:
                        uploaded_doc = await bot.upload_file(vid_file, part_size_kb=512, progress_callback=upload_progress_callback)
                        await bot.send_file(chat_id, uploaded_doc, force_document=True, caption=part_caption)
                    else:
                        ...
                        if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                            ...
                            uploaded_vid = await bot.upload_file(vid_file, part_size_kb=512, progress_callback=upload_progress_callback)
                            await status_msg.edit("🔄 **جاري الإرسال النهائي للمحادثة...**", buttons=cancel_btn)
                            await bot.send_file(chat_id, uploaded_vid, caption=part_caption, thumb=thumb_path, attributes=attr, supports_streaming=True)
                            ...
                            uploaded_aud = await bot.upload_file(vid_file, part_size_kb=512, progress_callback=upload_progress_callback)
                            await bot.send_file(chat_id, uploaded_aud, caption=part_caption)
                        else:
                            uploaded_oth = await bot.upload_file(vid_file, part_size_kb=512, progress_callback=upload_progress_callback)
                            await bot.send_file(chat_id, uploaded_oth, caption=part_caption, force_document=True)
            ...
            for root, _, files in os.walk(task_dir):
                for file in files:
                    ...
                    if os.path.isfile(fpath) and os.path.getsize(fpath) > 500 and not file.endswith('_thumb.jpg') and not file.endswith('_clean.jpg') and not file.endswith('_final.jpg') and not file.startswith('frame_'):
                        ...
                        if current_ext in ['.jpg', '.jpeg', '.png', '.webp']: fpath = await deep_sanitize_image(fpath)
            ...
            for fpath in sorted(downloaded_files):
                ...
                elif ext in ('.mp3', '.m4a', '.wav', '.flac', '.aac', '.opus'): audio_files.append(fpath)
            ...
            if photos:
                if len(photos) > 1 and not as_doc:
                    for chunk_idx in range(0, len(photos), 10):
                        ...
                        await bot.send_file(chat_id, chunk_photos, caption=f"📸 **ألبوم الصور ({chunk_idx+1}-{chunk_idx+len(chunk_photos)}/{len(photos)}):**", album=True)
                else:
                    for idx, p in enumerate(photos, start=1):
                        base_p = os.path.splitext(p)[0] + ".jpg"
                        if p != base_p: os.rename(p, base_p)
                        if as_doc: await bot.send_file(chat_id, base_p, caption=f"📄 **مستند صورة ({idx}/{len(photos)}):**", force_document=True)
                        else: await bot.send_file(chat_id, base_p, caption=f"📸 **صورة ({idx}/{len(photos)}):**" if len(photos) > 1 else "📸 **تم تنزيل الصورة بنجاح!**")
                ...
                for idx, part_vid in enumerate(split_vids, start=1):
                    ...
                    async def upload_progress_callback(current, total):
                        ...
                        if now - last_upload_update[0] > 2.0 or current == total:
                            ...
                            bar = "█" * filled + "░" * (10 - filled)
                            rem_time = (total - current) / speed if speed > 0 and total > 0 else 0
                            text = (f"📤 **جاري رفع الفيديو إلى تيليجرام...**\n[{bar}] {percent:.1f}%\n"
                                    f"📦 الحجم: `{format_size(current)}` / `{format_size(total)}`\n"
                                    f"⚡ السرعة: `{format_size(speed)}/s`\n⏱️ المتبقي: `{format_time(rem_time)}`")
                            try: await status_msg.edit(text, buttons=cancel_btn)
                            except: pass

                    uploaded_vid = await bot.upload_file(part_vid, part_size_kb=512, progress_callback=upload_progress_callback)
                    await status_msg.edit("🔄 **جاري الإرسال النهائي للمحادثة...**", buttons=cancel_btn)

                    await bot.send_file(
                        chat_id, uploaded_vid, caption=f"🎥 **تم تنزيل الفيديو بنجاح!**{part_caption}\n📁 `{os.path.basename(part_vid)}`",
                        force_document=as_doc, thumb=thumb_path, attributes=attr, supports_streaming=True
                    )
            ...
            for aud in audio_files:
                if cancel_event.is_set(): raise Exception("CANCELLED")
                uploaded_aud = await bot.upload_file(aud, part_size_kb=512)
                await bot.send_file(chat_id, uploaded_aud, caption=f"🎵 **تم استخراج الملف الصوتي بأعلى خامة:**\n📁 `{os.path.basename(aud)}`")

            for oth in other_files:
                if cancel_event.is_set(): raise Exception("CANCELLED")
                uploaded_oth = await bot.upload_file(oth, part_size_kb=512)
                await bot.send_file(chat_id, uploaded_oth, caption=f"📄 `{os.path.basename(oth)}`", force_document=True)
...
async def disk_status_handler(event):
    ...
    msg = (f"📊 **حالة السيرفر والتخزين الحالي:**\n\n💾 **مساحة القرص:**\n• المستهلك: `{format_size(disk.used)}`\n• المتبقي: `{format_size(disk.free)}`\n\n"
           f"🧠 **الذاكرة العشوائية (RAM):**\n• المستهلك: `{format_size(mem.used)}` ({mem.percent}%)\n• المتاح: `{format_size(mem.available)}`")
...
async def cancel_callback_handler(event):
    ...
    else: await event.answer("⚠️ العملية غير موجودة أو انتهت بالفعل.", alert=True)
...
async def settings_handler(event):
    if not is_owner(event.chat_id): return
    buttons = await build_settings_buttons(event.chat_id)
    await event.respond("⚙️ **لوحة التحكم والإعدادات الخاصّة بالبوت:**", buttons=buttons)
...
async def settings_callback_handler(event):
    ...
    if data == "toggle_snaps": config["snapshots"] = not config["snapshots"]
    elif data == "toggle_social_snaps": config["social_snapshots"] = not config["social_snapshots"]
    elif data == "change_qual":
        qualities = ["480", "720", "1080"]
        config["quality"] = qualities[(qualities.index(config["quality"]) + 1) % len(qualities) if config["quality"] in qualities else 1]
    elif data == "change_font":
        fonts = ["small", "medium", "large", "xlarge"]
        config["font_size"] = fonts[(fonts.index(config["font_size"]) + 1) % len(fonts) if config["font_size"] in fonts else 2]
    ...
    await event.edit("⚙️ **لوحة التحكم والإعدادات الخاصّة بالبوت:**", buttons=new_buttons)
...
async def trim_handler(event):
    ...
    asyncio.create_task(start_direct_execution(chat_id=event.chat_id, url=url, filename=get_clean_filename(url), as_doc=False, quality=user_config["quality"], target_fmt='mp4', status_msg=status_msg, trim_times=(start_t, end_t)))

@bot.on(events.NewMessage(pattern=r"^/start$"))
async def start_handler(event):
    if not is_owner(event.chat_id): return
    welcome_text = ("🤖 **أهلاً بك في بوت التنزيل الخاص بك!**\n\n🛠️ **أوامر إدارة السيرفر السريعة:**\n• `/df` - فحص مساحة القرص والـ RAM.\n"
...
async def url_handler(event):
    ...
    urls = re.findall(r"https?://\S+|/video/\S+", event.text)
    if not urls: return
    chat_id = event.chat_id
    
    for u in urls:
        if u.startswith('/video/'): u = f"https://www.dailymotion.com{u}"
        clean_u = clean_url(u.split('?')[0] if (is_instagram_url(u) or is_youtube_url(u)) else u)
        
        if is_youtube_url(clean_u):
            task_key = f"yt_{chat_id}_{int(time.time()*1000)}"
            await save_task(task_key, clean_u, "youtube")
            buttons = [[Button.inline("🎬 1080p", data=f"q_1080_{task_key}"), Button.inline("🎥 720p", data=f"q_720_{task_key}")], [Button.inline("📱 480p", data=f"q_480_{task_key}"), Button.inline("🎵 MP3", data=f"q_mp3_{task_key}")]]
            ...
            buttons = [[Button.inline("📸 ألبوم صور", data=f"tt_photo_{task_key}"), Button.inline("📄 مستند (JPG)", data=f"tt_doc_{task_key}")], [Button.inline("🎥 فيديو 720p", data=f"q_720_{task_key}"), Button.inline("🎬 1080p", data=f"q_1080_{task_key}")]]
            await event.respond("📸 **تم التعرف على رابط TikTok. اختر طريقة التنزيل:**", buttons=buttons)

        elif is_x_url(clean_u):
            task_key = f"x_{chat_id}_{int(time.time()*1000)}"
            await save_task(task_key, clean_u, "x")
            buttons = [[Button.inline("🎬 1080p", data=f"q_1080_{task_key}"), Button.inline("🎥 720p", data=f"q_720_{task_key}")], [Button.inline("📱 480p", data=f"q_480_{task_key}"), Button.inline("🎵 MP3", data=f"q_mp3_{task_key}")]]
            await event.respond("🎬 **اختر جودة الفيديو لمنصة X:**", buttons=buttons)

        elif is_instagram_url(clean_u):
            task_key = f"ig_{chat_id}_{int(time.time()*1000)}"
            await save_task(task_key, clean_u, "instagram")
            if "/reel/" in clean_u.lower() or "/tv/" in clean_u.lower():
                buttons = [[Button.inline("🎬 1080p", data=f"q_1080_{task_key}"), Button.inline("🎥 720p", data=f"q_720_{task_key}")], [Button.inline("📱 480p", data=f"q_480_{task_key}"), Button.inline("🎵 MP3", data=f"q_mp3_{task_key}")]]
                await event.respond("🎬 **اختر جودة الفيديو لمنصة Instagram:**", buttons=buttons)
            else:
                buttons = [[Button.inline("📸 ألبوم صور", data=f"ig_photo_{task_key}"), Button.inline("📄 مستند", data=f"ig_doc_{task_key}")], [Button.inline("🎥 فيديو 720p", data=f"q_720_{task_key}"), Button.inline("🎬 1080p", data=f"q_1080_{task_key}")]]
                await event.respond("📸 **تم التعرف على منشور Instagram. اختر التنزيل:**", buttons=buttons)
            ...
            await event.respond("📌 **اختر صيغة التحميل للرابط المباشر:**", buttons=buttons)
            
        else:
            user_config = await get_user_config(chat_id)
            asyncio.create_task(start_direct_execution(chat_id=chat_id, url=clean_u, filename=get_clean_filename(clean_u), as_doc=False, quality=user_config["quality"], target_fmt='mp4'))

@bot.on(events.CallbackQuery(pattern=r"^(tt_|ig_|dir_)"))
async def platform_callback_handler(event):
    if not is_owner(event.chat_id): return
    data = event.data.decode("utf-8").split("_")
    ...
    user_config = await get_user_config(event.chat_id)
    status_msg = await event.edit("⏳ **جاري التنزيل...**", buttons=None)
    asyncio.create_task(start_direct_execution(chat_id=event.chat_id, url=url, filename=get_clean_filename(url), as_doc=as_doc, quality=user_config["quality"], target_fmt=target_fmt, status_msg=status_msg))
...
async def quality_callback_handler(event):
    ...
    if task_type == "dailymotion" or is_dailymotion_url(url):
        status_msg = await event.edit("⏳ **جاري التنزيل من Dailymotion...**", buttons=None)
        asyncio.create_task(download_dailymotion_video(event=event, url=url, quality_choice=quality_choice, status_msg=status_msg))
    else:
        status_msg = await event.edit("⏳ **جاري التنزيل...**", buttons=None)
        target_fmt, quality_val = ('mp3', 'best') if quality_choice == 'mp3' else ('mp4', 'best' if quality_choice == '1080' else quality_choice)
        asyncio.create_task(start_direct_execution(chat_id=event.chat_id, url=url, filename=get_clean_filename(url), as_doc=False, quality=quality_val, target_fmt=target_fmt, status_msg=status_msg))

def main():
    bot.start(bot_token=BOT_TOKEN)
    print(f"🤖 البوت جاهز للاستخدام ومحمي بالمعرف: {OWNER_ID}")
