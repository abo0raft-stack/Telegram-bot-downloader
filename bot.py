...
async def convert_to_mp4(input_path):
    """دالة تحويل أي فيديو غير MP4 إلى MP4 H.264 لتوافق التيليجرام الكامل"""
...
async def get_video_metadata_and_thumb(file_path):
    ...
    # التحقق مما إذا كان هناك بوستر أصلي تم تنزيله بواسطة yt-dlp
...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best'):
    ...
    if fmt in ['mp3', 'audio_mp3']:
        ...
        # إجبار الدمج في قالب MP4 لضمان التشغيل داخل تيليجرام
        ...
        try:
            ...
            if target_url and is_social:
                ...
                if target_fmt == 'mp3':
                    ...
                    # تحويل تلقائي إلى MP4 إن لم يكن كذلك
            ...
            for vid in videos:
                ...
                # التحويل الإجباري لـ MP4 لدعم التشغيل في مشغل تيليجرام
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🔴 **إصلاح مشكلة التشغيل (MP4):** تحويل ومعالجة روابط يوتيوب لتصل كفيديوهات MP4 قابلة للمشاهدة المباشرة.\n"
            ...
            "🔄 **استئناف التنزيلات:** دعم خاصية Range لاستكمال التحميل المنقطع.\n"
            "1️⃣ **إضافة منصات جديدة:** دعم شامل للتحميل من Reddit, Pinterest, Threads, Snapchat, VK, Vimeo.\n"
            "2️⃣ **معالجة الصور والألبومات:** سحب الوسائط بمرونة.\n"
            "3️⃣ **تقسيم الفيديوهات:** تجزئة ملفات +1.9GB تلقائياً.\n"
            "4️⃣ **قص المقطع الزمني:** أمر /trim الحصري.\n\n"
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        if is_youtube_url(clean_u):
            ...
            await save_task(task_key, u, "youtube")
            buttons = [
            ...
            await event.respond("🔴 **تم التعرف على رابط YouTube (فيديو/Shorts). اختر خيار التحميل:**", buttons=buttons)
