...
# --- الإعدادات ---
VERSION = "v70.0-Direct-Download-Progress-Bar"
...
# --- دالة التنزيل المباشر مع شريط التقدم والمعلومات المطلوبة ---
...
async def download_direct_with_progress(client, chat_id, url, filepath, status_msg, cancel_event):
    ...
    with open(filepath, 'wb') as f:
        for chunk in res.iter_content(chunk_size=chunk_size):
            ...
            if chunk:
                ...
                if current_time - last_update_time > 1.5 or downloaded == total_size:
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4', status_msg=None):
    if not status_msg:
        status_msg = await bot.send_message(chat_id, "⏳ **جاري تحضير الطلب...**")
    else:
        await status_msg.edit("⏳ **جاري تحضير الطلب...**")
    ...
    try:
        ...
        if target_url and is_complex_url(target_url):
            ...
            # --- تعديل الرفع مع تتبع شريط التقدم للرفع والتنزيل ---
            ...
            await download_direct_with_progress(bot, chat_id, target_url, filepath, status_msg, cancel_event)
            
            await status_msg.edit("📤 **جاري تجهيز الرفع...**")
...
async def start_handler(event):
    await event.respond(
        ...
        "📸 **إنستغرام:** دعم كامل للصور، ألبومات الصور، والفيديوهات.\n"
        "🎵 **تيك توك:** الفيديوهات وسلاسل الصور.\n"
        "𝕏 **منصة X:** اختيار جودة الفيديو والصور.\n"
        "🌐 **الروابط المباشرة:** تظهر قائمة اختيار الجودة (Mp4, mp3, مستند) مع شريط تقدم مفصل للسرعة والحجم والوقت.\n"
    )
...
async def url_handler(event):
    ...
    for u in urls:
        ...
        if is_x_url(clean_u):
            ...
            # رابط مباشر غير تابع لمنصات التواصل الاجتماعي
...
def main():
    ...
    print("🤖 البوت يعمل بأعلى كفاءة مع شريط التقدم للروابط المباشرة!")
