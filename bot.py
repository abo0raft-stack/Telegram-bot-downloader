...
def extract_web_info(url):
    ydl_opts = {
        ...
        'impersonate': None, # تعطيل التخفي لمنع خطأ Dailymotion
...
async def start_handler(event):
    await event.respond(f"🚀 **بوت التحميل الشامل ودعم Dailymotion ({VERSION})**\nأرسل رابط الصفحة أو الفيديو للبدء.")
...
async def start_execution(chat_id, url, filename_title, as_doc=False):
    ...
    try:
        ...
        await status_msg.edit("⚡ **جاري التنزيل من المشغل...**", buttons=[Button.inline("❌ إلغاء", data=f"cancel_{msg_id}")])
        ...
        # خيارات توافق متقدمة لحل مشاكل Dailymotion والمشغلات الأخرى
        ydl_opts = {
            ...
            'noplaylist': True,
            'nocheckcertificate': True,
            'impersonate': None,                   # تعطيل خيار impersonate المسبب للخطأ
        ...
        # استخدام aria2c فقط إن كان ثابتاً وموجوداً للنظام
