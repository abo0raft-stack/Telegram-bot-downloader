...
def update_and_install_libraries():
    try:
        import subprocess
        subprocess.run(["pip", "install", "-U", "yt-dlp", "gallery-dl", "Pillow", "aiohttp", "aiosqlite", "telethon", "requests", "psutil", "curl-cffi"], check=True)
...
VERSION = "v83.0-Cloudflare-Bypass"
...
# --- دالة تجاوز حماية Cloudflare والروابط المحمية المحدثة ---
def bypass_cloudflare_download(url: str, download: bool = False, outtmpl=None, progress_hook=None):
    """
    دالة متقدمة لتخطي حماية Cloudflare والروابط المحمية باستخدام yt-dlp و curl_cffi.
    
    :param url: رابط الفيديو المحمي
    :param download: هل تريد تحميل الفيديو مباشرة أم استخراج الروابط فقط (False للاستخراج)
    :return: معلومات الفيديو أو رابط التحميل المباشر
    """
    ydl_opts = {
        'extractor_args': {
            'generic': {
                'impersonate': 'chrome'  # محاكاة بصمة متصفح كروم لتخطي حماية Cloudflare
            }
        },
        'format': 'best',
        'quiet': True,
    ...
    try:
        print("🔄 جاري تجاوز حماية Cloudflare وجلب البيانات...")
        ...
        print(f"❌ فشل تجاوز الحماية أو حدث خطأ أثناء التحميل: {str(e)}")
        return None
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if target_url and is_social:
                ...
                def dl_progress_hook(d):
                    ...
                    if d['status'] == 'downloading':
                        ...
                        if now - last_update_time[0] >= 2.0:
                            ...
                            text = (
                                f"📥 **جاري التنزيل مع تجاوز حماية Cloudflare...**\n\n"
                                f"[{bar}] {percentage:.1f}%\n"
                                f"🚀 **السرعة:** {format_size(speed)}/s\n"
                                f"📦 **الحجم:** {format_size(downloaded)} / {format_size(total)}\n"
                                f"⏱️ **المتبقي:** {format_time(eta)}"
                ...
                out_tmpl = os.path.join(task_dir, '%(title).50s_%(id)s.%(ext)s')
                
                def run_bypass():
                    return bypass_cloudflare_download(target_url, download=True, outtmpl=out_tmpl, progress_hook=dl_progress_hook)

                await loop.run_in_executor(None, run_bypass)

                downloaded_files = [os.path.join(task_dir, f) for f in os.listdir(task_dir) if os.path.isfile(os.path.join(task_dir, f)) and os.path.getsize(os.path.join(task_dir, f)) > 0]
                
                if not downloaded_files:
                    try:
                        await download_direct_async(bot, chat_id, target_url, filepath, status_msg, cancel_event, task_id)
                        downloaded_files = [filepath] if os.path.exists(filepath) and os.path.getsize(filepath) > 0 else []
                    except Exception as direct_err:
                        print(f"Direct download fallback error: {direct_err}")

                if cancel_event.is_set(): raise Exception("CANCELLED")

                downloaded_files = [os.path.join(task_dir, f) for f in os.listdir(task_dir) if os.path.isfile(os.path.join(task_dir, f)) and os.path.getsize(os.path.join(task_dir, f)) > 0 and not f.endswith('_thumb.jpg')]
                
                if not downloaded_files:
                    raise Exception("تعذر الوصول إلى المحتوى أو تجاوز الحماية.")
                ...
                for idx, vid_file in enumerate(video_files, start=1):
                    ...
                    if as_doc:
                        ...
                        if ext in ['.mp4', '.mkv', '.avi', '.mov', '.webm']:
                            ...
                            attr = [DocumentAttributeVideo(duration=duration, w=width, h=height, supports_streaming=True)]
                            await bot.send_file(chat_id, vid_file, caption=part_caption, thumb=thumb_path, attributes=attr)
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "🛡️ **تخطي حماية Cloudflare:** تحديث دالة التحميل المباشر لتشمل `bypass_cloudflare_download` مع محاكاة المتصفح (Chrome Impersonation) لتجاوز أخطاء 403 وحماية Anti-bot.\n"
            "📺 **إصلاح Dailymotion:** معالجة مسار التوجيه واستعادة النطاق الكامل وإسناد الـ Referer المطلوبة.\n"
            "🔴 **إصلاح تشغيل MP4:** معالجة وتحويل الفيديو لتصل قابلة للمشاهدة المباشرة.\n"
...
async def direct_callback_handler(event):
    ...
    status_msg = await event.edit("⏳ **جاري بدء التنزيل المباشر مع تجاوز الحماية...**", buttons=None)
