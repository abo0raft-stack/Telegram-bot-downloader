...
# --- الإعدادات ---
VERSION = "v48.6-AsyncFixed-16xFast"
...
# --- محرك التحميل السريع 16 خيط (yt-dlp) ---
def download_with_ytdlp(url, filepath, fmt='mp4', quality='best'):
    ydl_opts = {
        ...
        'concurrent_fragment_downloads': 16, # رفع الطبقات إلى 16
        'retries': 10,
        'fragment_retries': 10,
        'external_downloader_args': {'aria2c': ['-x16', '-s16', '-k1M']} # إجبار aria2c على استخدام 16 مسار إذا وجد
    }
...
# --- محرك التنزيل المباشر بـ 16 خيط (Direct Multithreaded Downloader) ---
def download_chunk(url, headers, start, end, chunk_file, cancel_event):
    ...
    res = requests.get(url, headers=req_headers, stream=True, timeout=30, verify=False)
...
def download_direct_file(url, filepath, cancel_event, num_threads=16):
    ...
    head_res = requests.head(url, headers=headers, allow_redirects=True, timeout=15, verify=False)
    ...
    # إذا كان السيرفر لا يدعم التقسيم أو الحجم غير معروف، قم بالتنزيل التسلسلي العادي
