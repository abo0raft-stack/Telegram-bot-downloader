...
# هيدرات متصفح حقيقي لتجاوز الحظر
BROWSER_HEADERS = {
...
def download_direct_file(url, filepath, status_msg, loop, cancel_event):
    ...
    session.verify = False # التجاوز في حالة وجود مشكلة بتدقيق الشهادات
...
async def start_execution(chat_id, url, filename_title, as_doc=False, quality='best'):
    ...
    try:
        ...
        # كود التحميل المحسن لتخطي حجب السيرفرات
        try:
            ...
            # محاولة احتياطية عبر yt-dlp في حال فشل Requests
            await status_msg.edit("⚠️ **محاولة جلب الملف عبر المحرك الثانوي...**")
            ydl_opts = {
                'format': 'best',
                'outtmpl': file_path,
                'quiet': True,
        ...
        # التحقق الحقيقي من الحجم (أكبر من 50 كيلو بايت لضمان أنه ليس صفحة خطأ HTML)
        if os.path.exists(file_path):
            file_size = os.path.getsize(file_path)
            if file_size < 50 * 1024: 
                raise Exception("السيرفر رفض إعطاء الفيديو وقام بإرجاع صفحة محجوبة.")

            if quality in ['1080', '720', '480']:
                await status_msg.edit(f"⚙️ **جاري تحويل الضغط إلى {quality}p...**")
