...
# --- الإعدادات ---
VERSION = "v20.0-OriginalThumbFixed"
...
# --- توليد صورة مصغرة عبر FFmpeg (في حال عدم وجود صورة أصلية) ---
def generate_thumbnail_fallback(video_path, thumb_path, duration):
    try:
        ...
        print(f"Thumbnail Fallback Exception: {e}")
    return None
...
async def start_execution(chat_id, url, filename_title):
    ...
    try:
        ...
        if is_direct_url:
            ...
            # إضافة writethumbnail لجلب الغلاف الأصلي
            ydl_opts = {
                'format': 'best',
                'quiet': True,
                'outtmpl': file_path,
        ...
        if os.path.exists(file_path):
            ...
            # --- 1. معالجة الصورة المصغرة الأصلية المرفقة مع التنزيل ---
            final_thumb = None
            ...
            # إذا لم يتم العثور على صورة مصغرة أصلية (مثلاً رابط مباشر)، نولد صورة عبر FFmpeg
