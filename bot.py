...
from telethon.tl.types import DocumentAttributeVideo, MessageMediaDocument, MessageMediaDocument
from telethon.errors import FloodWaitError
from hachoir.metadata import extractMetadata
from hachoir.parser import createParser

# --- الإعدادات ---
VERSION = "v49.0-ForwardFix"
...
def fix_mp4_faststart(input_path, output_path):
    cmd = [
        ...
        if len(lines) >= 3:
            ...
            for l in lines:
                try:
                    val = float(l)
                    if val > 1000: pass
                    elif val > 0 and duration == 0: duration = int(val)
                except: pass
    except Exception as e:
...
def convert_or_rescale_video(input_path, output_path, target_format, quality='best'):
    ...
    if target_format in ['audio_mp3', 'mp3']:
        ...
        cmd.extend(['-c:v', 'libx264', '-crf', crf_val, '-preset', 'ultrafast', '-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', output_path])
...
async def start_handler(event):
    welcome_text = (
        ...
        "2️⃣ أرسل أو حوّل أي فيديو لتغيير صيغته (MP4, MKV, MP3) وتحديد الجودة!"
    )
    await event.respond(welcome_text)

# --- معالج الوسائط الشامل لتفادي تجاهل الرسائل المحولة (Forwarded) ---
...
async def video_file_handler(event):
    ...
    # التخدي عن الرابط في حال وجود نص
    if msg.text and msg.text.startswith('http'):
        return

    is_video = False
    orig_name = "video_download.mp4"

    # التقاط الوسيط سواء كان Video, Document, Animation أو Forwarded
    if msg.video or msg.video_note or msg.gif:
        ...
        if mime.startswith('video/') or mime.startswith('application/') or mime.startswith('audio/'):
            is_video = True
            if hasattr(msg, 'file') and msg.file and msg.file.name:
                orig_name = msg.file.name
...
def fix_mp4_faststart(input_path, output_path):
    """إصلاح حاوية الفيديو لضمان التوافق مع المشغل وإظهار شريط الوقت"""
    cmd = [
        'ffmpeg', '-y', '-i', input_path,
        '-c', 'copy', '-movflags', '+faststart',
        output_path
...
def get_video_metadata(filepath):
    duration, width, height = 0, 1280, 720
    try:
        # استخدام ffprobe للتحقق الدقيق من أبعاد والمدة الزمنية للفيديو
        cmd = [
            'ffprobe', '-v', 'error', '-show_entries',
            'format=duration:stream=width,height', '-of',
            'default=noprint_wrappers=1:nokey=1', filepath
...
# --- معالج استقبال الفيديو الشامل (سواء كفيديو أو مستند أو محول) ---
@bot.on(events.NewMessage)
async def video_file_handler(event):
    if event.text and event.text.startswith('http'):
        return  # نترك الروابط لمعالج الروابط
    ...
    if msg.video:
        ...
        mime = msg.document.mime_type or ''
        if mime.startswith('video/') or mime.startswith('application/octet-stream'):
            is_video = True
...
async def start_execution(chat_id, url=None, filename_title="video.mp4", as_doc=False, quality='best', media_msg=None, target_format='mp4'):
    ...
    fixed_path = f"downloads/fixed_{status_msg.id}_{filename_title}"
    ...
    try:
        if media_msg:
            ...
            if quality in ['1080', '720', '480', 'audio_mp3', 'mp3'] and url:
                ...
                await status_msg.edit("🔧 **جاري تهيئة الحاوية لإظهار المدة الزمنية...**")
                fixed_file = await loop.run_in_executor(None, fix_mp4_faststart, file_path, fixed_path)
                if fixed_file != file_path and os.path.exists(fixed_file):
                    os.remove(file_path)
        ...
        for p in [file_path, rescaled_path, fixed_path, thumb_path]:
            if os.path.exists(p):
                try: os.remove(p)
                except: pass
