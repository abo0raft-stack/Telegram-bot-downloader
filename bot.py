...
# --- محرك الانستغرام المحدث v85 لجلب الصور والألبومات بنجاح ---
async def instagram_carousel_and_photo_engine(url, task_dir):
    try:
        # 1. المحاولة الأولى: gallery-dl المطور لجلب الصور والألبومات
        cmd = [
            "gallery-dl",
            "--directory", task_dir,
        ...
        # التأكد مما إذا تم تنزيل صور
        valid_files = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 1000 and not f.endswith('.txt')]
        if valid_files:
            return

        # 2. المحاولة الثانية: yt-dlp محدد لاستخراج صور البوست
        loop = asyncio.get_event_loop()
        ydl_opts = {
        ...
        # 3. المحاولة الثالثة: الاستعانة بـ Cobalt API المطور
        api_url = "https://api.cobalt.tools/api/json"
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
                    ...
                    for idx, media_url in enumerate(urls_to_dl, start=1):
                        ...
                        async with session.get(media_url, headers=BROWSER_HEADERS, timeout=30) as r:
                            if r.status == 200:
                                ...
                                is_vid = ".mp4" in media_url or "video" in content_type
                                ext = "mp4" if is_vid else "jpg"
    ...
    async with MAX_CONCURRENT_TASKS:
        ...
        try:
            ...
            if target_url and is_social:
                # إذا كان الرابط لمنصة إنستغرام نقوم باستخدام المحرك المخصص مباشرة لضمان الصور والفيديوهات
            ...
            for root, _, files in os.walk(task_dir):
                for file in files:
                    ...
                    if os.path.isfile(fpath) and os.path.getsize(fpath) > 300 and not file.endswith('_thumb.jpg') and not file.endswith('_clean.jpg') and not file.endswith('_final.jpg') and not file.startswith('frame_') and file != "post_caption.txt":
...
async def changelog_callback_handler(event):
    ...
    if data == "show_changelog":
        changelog_text = (
            ...
            "📸 **إصلاح صور الانستغرام:** حل جذري لمشكلة عدم تنزيل الصور وألبومات الصور عبر محرك متعدد الطبقات.\n"
