...
# --- رقم التحديث الجديد ---
VERSION = "v11.5 Screenshots & Cancel"
...
async def handle_url(event):
    ...
    status_msg = await event.respond(
        f"⏳ **جاري الاتصال بالسيرفر ({VERSION})...**",
    ...
    try:
        ...
        if os.path.exists(filename) and os.path.getsize(filename) > 0:
            await safe_edit(status_msg, f"⚙️ **جاري إلتقاط 9 لقطات شاشة ({VERSION})...**", buttons=[Button.inline("❌ إلغاء العملية", data=f"cancel_{task_id}")])
            ...
            # 3. إرسال ألبوم اللقطات إذا نجحت الدالة
            if screenshot_list and len(screenshot_list) > 0:
                try:
                    ...
                    print(f"⚠️ Album send error: {send_err}")

            if CANCELLED_TASKS.get(task_id):
                raise Exception("CANCELLED_BY_USER")

            # 4. الرفع الفعلي لملف الفيديو
