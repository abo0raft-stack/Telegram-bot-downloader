...
# --- الإعدادات ---
VERSION = "v55.0-Instagram-Carousel-Image-Fix"
...
def download_with_ytdlp(url, task_dir, fmt='mp4', quality='best'):
    ...
    if any(x in url_lower for x in ['twitter.com', 'x.com']) and os.path.exists(X_COOKIES_FILE):
        ...
        # خيارات إضافية لتحسين التعامل مع ألبومات إنستغرام عبر yt-dlp
...
def download_instagram_with_gallery_dl(url, task_dir):
    """استخدام gallery-dl كخيار قوي للصور والألبومات من إنستغرام"""
    try:
...
def fallback_instagram_download(url, task_dir):
    """طريقة احتياطية لجلب الصور والألبومات من إنستغرام عبر API مباشرة إذا فشل yt-dlp و gallery-dl"""
    try:
        ...
        if res.status_code == 200:
            ...
            try:
                ...
                if carousel:
                    for item in carousel:
                        ...
                        if node.get('is_video') or 'video_versions' in node:
                            v_url = node.get('video_url') or (node.get('video_versions', [{}])[0].get('url'))
                            if v_url: items.append(v_url)
                        else:
                            img_url = node.get('display_url') or (node.get('image_versions2', {}).get('candidates', [{}])[0].get('url'))
            ...
            for img_url in items:
                ...
                if r.status_code == 200:
                    ext = "mp4" if ".mp4" in img_url or "video" in r.headers.get('Content-Type', '') else "jpg"
...
async def start_direct_execution(chat_id, url, filename, as_doc=False, quality='best', media_msg=None, target_fmt='mp4'):
    ...
    try:
        ...
        if url and is_complex_url(url):
            ...
            # في حال لم يقم yt-dlp بإنزال صور إنستغرام أو ألبوماتها:
            ...
            if not downloaded and ('instagram.com' in url or 'instagr.am' in url):
                # المحرك الثاني: gallery-dl
                await loop.run_in_executor(None, download_instagram_with_gallery_dl, url, task_dir)
                
                downloaded = [f for f in os.listdir(task_dir) if os.path.getsize(os.path.join(task_dir, f)) > 0]
                # المحرك الثالث: API Direct Fallback
                if not downloaded:
        ...
        # إعادة تجميع الملفات من المجلد والمجلدات الفرعية إن وجد (لأن gallery-dl أحيانا يُنشئ مجلدات فرعية)
        downloaded_files = []
        for root, _, files in os.walk(task_dir):
            for f in files:
                fpath = os.path.join(root, f)
                if os.path.isfile(fpath) and os.path.getsize(fpath) > 0 and not f.endswith('.jpg.jpg'):
